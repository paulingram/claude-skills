...
- **The one-line-of-state turn-output rule** — the user's own named remedy, mechanized. Trips at `>= 3` non-empty lines OR any structural marker; never at `<= 2` lines with no marker. Both directions are pinned, because a rule that never fires is decoration and a rule that fires on a terse status update gets disabled.
...
Suite **7024 -> 7221 passing + 6 skipped, 0 failed** (+197 tests; 241 top-level test files; both encodings). Skills / agents / commands / Layer-3 tools UNCHANGED (53 / 39 / 25 / 22); hook scripts unchanged at 7; `check_separation` unaffected (26).
