...
Tests: **+18** in the new `tests/test_credit_failover.py`, one per coverage-map acceptance criterion. Both crux invariants were **mutation-tested** rather than merely asserted: deleting the `429` early return failed 3 tests (`test_classifier_truth_table`, `test_429_with_quota_wording_is_rate_limited_not_exhausted`, `test_detector_rate_limited_no_failover`), and flipping `state_data["activated"] = False` to `True` failed 2 (`test_failover_clears_recorded_activation`, `test_heal_declines_after_failover_even_on_live_port`) — so both pins are demonstrably load-bearing, not decorative. Suite 5689 → **5707 passing + 4 skipped** (203 test files), green under both Windows cp1252 AND `PYTHONUTF8=1`, with the v3.41.1 `_real_state_tripwire` silent across both runs. Skill / agent / command / hook / Layer-3-tool counts UNCHANGED (48 / 39 / 23 / 7 / 20); stdlib-only holds.
