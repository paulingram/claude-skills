...
## [3.42.0] — 2026-07-20 — credit-failover-to-login (an upstream that is reachable but cannot serve is no longer invisible)

**MINOR — new capability.** When the external-LLM gateway's upstream runs out of credits, every Claude Code session pointed at that gateway broke and stayed broken. The owner's words: *"we need a way to fail over if we are out of credits. we should fail to our login."* There was NO credit handling anywhere in the gateway surface — grep-verified: no `402`, no `insufficient_credit`, no `quota_exceeded`, no `429` classification. A credit-dead gateway keeps binding its port, keeps passing the v3.41.1 TCP liveness probe, and keeps reporting `enabled`, so every surface read healthy while every session 402'd. The class this closes: **an upstream that is REACHABLE but cannot SERVE is indistinguishable from a healthy one on every signal CT6 collected.** v3.41.1 fixed the client-side wire being silently cut; v3.42.0 fixes the upstream silently being unable to serve.

- **REQ-002 — the classifier is the crux, and a rate limit must NEVER fail over.** `classify_upstream_error(status, body)` is a PURE function returning `credit-exhausted` / `rate-limited` / `transient` / `other`. HTTP `402` and the hard-credit body vocabulary (`insufficient_credit` / `insufficient credits` / `quota_exceeded` / `credit balance is too low` / `billing` / `payment required`, case-insensitive) are `credit-exhausted`; `429` is `rate-limited`; `>= 500` or `overloaded` is `transient`. **ONLY `credit-exhausted` may trigger a failover.** The `429` status check is evaluated BEFORE the hard-credit body scan, and that order is load-bearing: some providers return `429` with quota wording for a per-minute burst limit, and misreading that as exhaustion would flap the owner's auth mode on every busy minute — the exact outcome the owner's trigger decision forbids.
- **REQ-003 — detection lives where CT6 actually observes the upstream.** The open design question (gateway-side vs installer-side) was settled with evidence, not preference: the generated launcher writes NO log by default, so passive log-scanning would silently detect nothing on a default install (REJECTED as the primary channel); a LiteLLM callback plugin would put new code inside the serving path and break the stdlib-only invariant (REJECTED); `_http_completion_probe` already POSTs to `/v1/messages` and `urllib.request.urlopen` already raises `HTTPError` carrying `.code` and the body (ACCEPTED). So detection is installer-side and probe-based — one bounded 32-token probe, negligible when credits exist and free when they do not, since the failure IS the signal. The prober is injectable, so no test opens a socket.
- **REQ-001 — the failover returns the user to Claude sign-in, and makes it stick.** `failover_to_login()` REUSES the existing primitives rather than reimplementing them: `remove_claude_env()` (merge-preserving, port-matched) strips the env block so Claude Code falls back to the EXISTING `subscription` posture — **no third auth mode is invented**; `apply_policy(agents_dir, POLICY_UNIFORM_FABLE)` reverts the role split, because login auth cannot serve the secondary alias and leaving 21 dev agents pointed at it would break every dev-class teammate spawn; and `_write_state` records `activated: false` plus a `failover` record. **The recorded-state flip is mandatory, not cosmetic:** per v3.41.1, `maybe_heal_activation()` re-applies the env block whenever state records `activated + enabled + api-key` AND the port is live — and a credit-dead gateway still binds its port and still passes that probe. A failover that stripped the wire WITHOUT clearing recorded activation would be silently undone on the very next SessionStart and re-point the owner at a gateway that cannot serve. `enabled` and the stored provider keys are LEFT INTACT, so `install --activate` is a clean one-command return that also clears the failover record.
- **REQ-004 / REQ-005 — surfaced, reachable, and automatic.** `status` gains a `credit-failover` row (when it fired, why, and the `install --activate` remediation) plus a first-class `--json` field; a machine that never failed over keeps byte-identical output. The `failover` CLI subcommand supports detect-then-act, `--check` (probe + report, mutate nothing), and `--force`. `maybe_failover_to_login()` in `hooks/sessionstart-run-continuity.py` fires automatically at session start, guarded to activated api-key machines, ordered BEFORE the activation heal so a failover that fires is not re-healed in the same hook run, and fail-open on every path. Automatic RETURN is deliberately NOT built — it would require polling a paid upstream on a timer; the owner asked for automatic failover, and the return is the existing `install --activate`.

- **The installed-copy guard — added after the feature bit the developer's own machine.** `maybe_failover_to_login` initially shipped with NO installed-copy guard, while both sibling hook functions carry one (`maybe_heal_model_split` 3 references, `maybe_heal_activation` 2, the failover **0**). A dev-checkout SessionStart therefore executed the failover against the real `~/.architect-team/gateway/gateway.json` and wrote a genuine failover record (2026-07-20T07:01:49Z). The `tests/conftest.py` tripwire did not catch it because the mutation happened OUTSIDE a pytest run — the guard, not the tripwire, is the control that matters for hook code. The detection itself was CORRECT: a read-only probe of the live gateway returned `400` with body *"Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing…"*, which the classifier matched on `credit balance is too low` — Anthropic returns 400-with-message rather than 402, which is precisely why the body-pattern arm exists alongside the status codes. So the feature worked end-to-end against genuine credit exhaustion; only its containment was missing. Fixed with the guard + explicit-injection bypass, identical in shape to the two heals. **The regression test took three attempts, and the first two were vacuous:** v1 asserted on file bytes (the blanket `except Exception: return ""` masked the outcome, so neutering the guard still passed); v2 asserted the liveness probe was never called but did not sandbox `CT6_GATEWAY_HOME`, so the state path resolved to the REAL gateway.json — which by then read `activated: false` — and returned at the STATE guard, making the test's verdict depend on the developer's machine state; v3 seeds a sandboxed `CT6_GATEWAY_HOME` with an activated api-key state AND asserts `probe_calls == []`, which neutering the guard now FAILS. Independently re-derived by the reviewer. Two questions were assessed and closed as NOT defects: the blanket fail-open matches `maybe_heal_activation` exactly and is correct for a SessionStart hook, and the `root / "agents"` resolution matches `maybe_heal_model_split`'s shipped precedent line-for-line (the unreverted split on the real machine was an artifact of the ungated dev-checkout run, where that path pointed at the already-uniform-fable checkout).

Tests: **+21** in the new `tests/test_credit_failover.py` — one per the 16 coverage-map acceptance criteria, two classifier-negative cases (rate-limited and transient must NOT fail over), the two dev-checkout containment regressions, and the hook-path end-to-end split-revert test. Both crux invariants were **mutation-tested** rather than merely asserted: deleting the `429` early return failed 3 tests, flipping `state_data["activated"] = False` to `True` failed 2, and neutering the installed-copy guard failed the dev-checkout containment test — so both pins are demonstrably load-bearing, not decorative. Suite 5689 → **5710 passing + 4 skipped** (203 test files), green under both Windows cp1252 AND `PYTHONUTF8=1`, with the v3.41.1 `_real_state_tripwire` silent across both runs. Skill / agent / command / hook / Layer-3-tool counts UNCHANGED (48 / 39 / 23 / 7 / 20); stdlib-only holds.
