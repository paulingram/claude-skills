...
## [3.63.0] — 2026-08-17 — bauplan-skills-dependency (MINOR: CT6 learns to depend on a plugin *conditionally*, and a shipped CLAUDE.md leak gets closed on the way)

CT6 had exactly one way to depend on a plugin: `REQUIRED_PLUGINS`, a hard block — a missing member exits setup 1 (`setup.py:1141`). Right for superpowers, cartographer, and ralph-loop, which every run needs. Wrong for a domain plugin most users will never touch. Bauplan Labs' `bauplan` plugin (MIT, six data-lakehouse skills) is the first dependency worth having **only where it applies**, and both halves of that needed machinery CT6 did not have: a tier that recommends without blocking, and a per-project check that arms the capability only where it belongs.

**The tier.** New `CONDITIONAL_PLUGINS`, a sibling registry to `REQUIRED_PLUGINS` and provably disjoint from it, reported at setup and routed nowhere near the exit-code branch — a missing conditional member cannot change `setup.py --check`'s exit, pinned by test in both directions. It reuses `_PLUGIN_MARKETPLACE_SOURCES` + `plugin_remediation_lines()` verbatim (the cartographer third-party-marketplace path), so remediation emits `/plugin marketplace add BauplanLabs/bauplan-skills` then `/plugin install bauplan@bauplan-skills` without a line of new string-building.

**Arming, asymmetric by signal certainty.** A `bauplan_project.yml` anywhere in the repo arms **silently** — `_has_bauplan_markers` mirrors `_has_frontend_markers` exactly, recursive glob and `_SKIP_DIR_PARTS` included, so a monorepo subdirectory marker counts and a *vendored or reference-cloned* Bauplan project does not arm its host. Stated Bauplan intent with no marker surfaces **exactly one** confirmation, then arms. Certain signals act; inferred signals ask. That second arm is not politeness — `bauplan-data-pipeline`'s own primary use is creating a project from scratch, where no marker can exist yet, so a marker-only rule would have shipped the flagship skill unreachable in the exact case it was written for. `resolve_bauplan_arming` is a pure function, so the decision is the same every time it is asked and lives in the default key-free suite.

**Where it was injected — the question the request delegated.** The whole surface was reviewed and the answer is the **lane bodies, not the architect agents**. `sync_agent_boilerplate.py` compiles shared blocks into all 39 agents unconditionally, so a Bauplan block there would ship lakehouse instructions to `visual-analyzer`, `route-mapper`, and `closeout-agent` on every run forever — the exact opposite of costing nothing when absent. Agents are role definitions; how to work with a data platform is not a property of the backend role. Each skill landed where it is genuinely the right tool, the strongest fit being `bauplan-data-quality-checks` at `data-engineering-exploration` **Stage 6**, which already mandates ≥ 1 blocker-severity validation rule per transformation and now has a skill that emits exactly that as `expectations.py`. Every pointer is gated on the arming verdict: an unarmed run reads no Bauplan instruction at all.

**Precedence, stated once.** An armed skill runs INSIDE a CT6 mandate, never instead of it — D0 still dispatches `data-engineering-exploration` VERBATIM, no gate is relaxed, and where a Bauplan platform rule governs a lakehouse operation over a CT6 default the conflict is RECORDED, never silently resolved. The rule lives in `common-pipeline-conventions` as heading + STOP pointer, its full text in `references/augment-never-replace.md`.

**A shipped defect closed on the way — the run's most portable result.** A byte-preservation test written for the new trait-keyed gate exposed a **pre-existing leak in `guidance_blocks.py`**, present in v3.62.0 and every release before it: `upsert_block` wrote a separator newline that `remove_block` never reclaimed, so an install/uninstall round trip left one byte behind and **accumulated without bound** in a file CT6 does not own — a 30-byte CLAUDE.md reached 35 after five cycles. It hit all three existing installers (MemPalace, Librarian, gateway) whenever `--claude-md` was passed, and it violated `installer-guidance-blocks`' own shipped scenario ("all other CLAUDE.md content is byte-preserved"). The first fix attempt made the suite green by rewriting the assertion to `original + b"\n"` and naming it "leaves only the separator newline" — a description that is false from cycle two onward; the review gate caught it by running the code rather than reading the report. The real fix had to make the WRITE side symmetric first: `upsert_block` appended `"\n\n"` when a file lacked a trailing newline but `"\n"` when it had one, and after the fact those two are indistinguishable from the file alone, so no reclaim rule could reverse both. Now byte-identical across LF, CRLF, no-trailing-newline, and empty files, over repeated cycles — with the repeated-cycle assertion whose absence let a green suite ship a live defect.

**Byte budget honored, not raised.** `common-pipeline-conventions/SKILL.md` sits under a hard 259290-byte cap the ledger records as deliberately unchanged, and it stood at 259287 — two bytes of headroom, so the new discipline could not be carried inline at any length. Room was made by extraction, this regime's own mechanism: the new section is heading + STOP pointer, paid for by compressing `## Local -> dev -> test-on-dev` to the same form its siblings already use, its inline paragraph having duplicated a `references/` file already on disk. No rule text lost, every heading citation intact, ledger entry APPENDED. Net **−187 bytes**: the file is smaller than before this release despite gaining a discipline.

Evidence for the behavioral claims binds to run state (`.architect-team/<lane>/<slug>/bauplan-dispatches.json`), **not** to `hooks/skill_invocation_audit.py` — that hook builds expectations from CT6's own canonical commands and would never carry a third-party dispatch, and putting an unrelated capability's evidence needs on the Stop path of every session is the wrong trade. Three live routing evals sit behind `CT6_EVALS=1`; their prompts never name `bauplan_project.yml` as a trigger, because telling the model the rule and checking it followed would test the prompt rather than the routing.

No new skill / agent / command / hook script / Layer-3 tool — counts UNCHANGED (53 / 39 / 25 / 7 / 23); `check_separation` unaffected (26). One new stdlib-only package, `scripts/bauplan/` (`arming.py`, `dispatch_record.py`).

Suite **7676 -> 7806 passing + 16 skipped** (254 test files; +130 tests across 6 NEW files: `test_bauplan_detection`, `test_bauplan_arming`, `test_bauplan_conditional_tier`, `test_bauplan_guidance_block`, `test_bauplan_dispatch_record`, `test_bauplan_eval_fixtures`). **Measured on macOS (darwin)**, where one PRE-EXISTING failure is present and unrelated to this change — `test_pretool_unilateral_override_guard.py::test_backslash_spelling_is_still_refused`, a Windows backslash-path assertion; it fails identically on a pristine `395b826` checkout, verified against a detached baseline worktree. The v3.62.0 baseline was measured on `win32`, where it passes. The skip delta (6 -> 16) is PyYAML-gated tests on this machine, not new skips.
...
**C1 — CLAUDE.md was 54,293 bytes, and the repo's own assessor said so.** `claude_md_efficiency.py assess` flagged it high/over-budget — and this is the file loaded into EVERY session's context, so its size is paid on every turn. The decomposition: the Stack section (30.7KB of per-module essays) and one 9,535-byte "Current shape" sentence duplicated content whose canonical homes already exist — `docs/CODEBASE_MAP.md` (full architecture), `docs/CAPABILITY_INDEX.md` (per-item catalog), `CHANGELOG.md`/`docs/RELEASE_HISTORY.md` (per-release narrative) — while the operative sections (Conventions at a glance, the release-order constraint) sat buried beneath ~41KB of narrative. Every distinctive fact was verified present in 2–4 canonical docs BEFORE cutting (orphan-fact sweep, ten spot checks). Now **17,345 bytes**: compact shape + capability summary + a "where the depth lives" pointer block + one-line-per-surface Stack, with the three-release digest convention and the operative conventions carried byte-identical. All 413 tests that read CLAUDE.md stay green; instruction-compliance stays 0 findings. HONEST RESIDUAL: still above the engine's 2,048-byte pointer budget — deliberately, because the digest convention and the operative conventions live inline by design; the assessor's flag is advisory for this repo (not suite-gated), and the distance is stated here rather than silently absorbed.

**C2 — the tests badge gets the pin its sibling got at v3.45.0.** The version badge has been pinned since v3.45.0; the tests badge next to it was pinned by nothing — a pin that covered one of two badges — and it drifted THREE releases stale (7375 while the repo published 7627), then drifted AGAIN during this very review: the new test's first run caught the badge at 7655 against the published 7656, a live specimen of the exact defect it was built for, created during the v3.61.0 count oscillation and sitting unnoticed since. NEW `test_readme_tests_badge_matches_the_published_count`: badge ⇒ the CHANGELOG top entry's suite-total count, which is itself measurement-backed at release — so the badge chains to the recorded artifact. Mutation witness: a wrong badge is CAUGHT, restored green.

No skill / agent / command / hook / engine behaviour change — counts UNCHANGED (53 / 39 / 25 / 7 / 23); `check_separation` unaffected (26).

Suite **7656 -> 7657 passing + 6 skipped, 0 failed** (246 test files; +1 test). **Recorded, not asserted** — `docs/measurements/2026-08-16-v3.61.1-suite.json` carries the frozen-tree bracket, taken under the v3.61.0 release-backing arm's own discipline.

## [3.61.0] — 2026-08-14 — optimize-forcing-gates (MINOR: the review of v3.60.0 asked "is this optimal?" — three named boundaries were closeable, and the fourth answer was measured restraint)

A review of the v3.60.0 implementation split its open surface into settled-by-measurement and closeable. Settled and deliberately kept: the existence arm's out-of-suite placement (red-by-construction during every authoring window otherwise), the re-measure cost (intrinsic — an artifact must describe the tree that shipped), F14 (unmeasurable at 0 of 707 transcripts), and symlink coverage (privilege-blocked; still recorded untested). Closeable, and now closed:

**O1 — the backing check stops depending on a human remembering.** `changelog_check.py --require-measurements` was the ONLY place the measurement existence arm bites, and nothing invoked it — the release's own backing check was prose, the exact defect class one level up from where v3.60.0 fixed it. NEW Stop-hook arm `_audit_release_backing`: an ACTIVE run that bumped `.claude-plugin/plugin.json` cannot end its turn while the published count is unbacked. Narrow by construction, each clause a measured lesson: non-release runs untouched (the currency comparison is meaningless against mid-cycle trees), the authoring window never blocked (the arm waits until the CHANGELOG top entry names the manifest version), foreign workspaces untouched (convention detection). The arm DELEGATES to the release CLI through one shared implementation, so hook and CLI cannot drift. Kill-switch `CT6_RELEASE_BACKING_GATE_DISABLED`.

**O2 — `TaskUpdate(status="deleted")` no longer releases the completion lock.** The headline v3.56.0 boundary, closed at PreToolUse: deletion during an ACTIVE run is refused; `completed` stays fully legitimate, so — unlike any Stop gate — this arm structurally cannot wedge a lane. Payload-only evidence (a missing transcript cannot disarm it — the F7 lesson), `escalation-pending.md` deliberately does NOT stand it down (agent-writable pause file = two-step escape, ADV-1), and it carries its OWN kill-switch `CT6_TASK_DELETE_GATE_DISABLED` — `CT6_RUN_CONTINUITY_DISABLED` does not mute it (the F12 lesson).

**O3 — measurement artifacts are immutable to agent tools.** Edit/Write/NotebookEdit under `docs/measurements/` is refused, CREATION included — a freshly hand-created artifact IS the forgery, unlike the deploy config where creation only adds a constraint. The engine writes through ordinary io, so legitimate recording is untouched. What this closes is the tool-layer half only: Bash writes remain the named residual, and a hand-written artifact remains indistinguishable from a measured one — the rule converts forgery from an available accident into a deliberate act on a named surface.

**O4 — the mixed-clock class, swept.** v3.60.0's fixture-clock defect (an absolute date compared against `time.time()`-relative ages — inverts the day the calendar passes the constant) was fixed in one file, and this run's core lesson is *a fix that covers one of two places*. The sweep measured the class count repo-wide: **exactly one** — the instance already fixed. Both candidate files inspected and cleared for stated reasons, not assumed.

**Four defects were produced while building this and caught before commit** — the honest ledger, all in `openspec/changes/archive/.../design.md`: O2's first cut early-returned and silently un-gated TaskUpdate from the arm-1 mandate check (the one-of-two-places shape, caught by the PRE-EXISTING arm-1 pin, fixed additive, then pinned from both files); O1's three subprocess calls shipped without pinned encodings (caught by the house `test_subprocess_encoding` gate, A7); one invalid witness run (CRLF vs multi-line anchors); and two more heredoc manglings (caught at collection).
...
**Three carryovers closed.** `tests/test_skill_references.py`'s byte-ledger had its v3.57.0 transition line REWRITTEN rather than appended to, attributing v3.59.0's bytes to v3.57.0 and making v3.59.0's own change invisible — the gate was never weakened (the 259290 cap untouched, both assertions enforced and green), but a ledger that misreports which release spent the bytes is the next maintainer's wrong instrument. `docs/ETHOS.md`'s "Evidence integrity (v3.47.0)" heading had come to contain a v3.59.0 rule. And the claim that all five witnesses pass `verify-check-can-fail` unchanged is true but VACUOUS for W3/W4/W5, which add no test file and fall outside that tool's jurisdiction — the demonstration carries weight for W1/W2 only, and now says so.

**Two items left OPEN, deliberately and in writing.** (1) *Hash brackets are asserted, never recorded* — five suite counts were published this thread as frozen-tree and hash-bracketed, and no bracket artifact exists on disk for any of them; the runs happened and the output was read, but the evidence is a claim, unverifiable later by anyone. The `verify-claim-instrument-binding` contract already has the field (`tree_state`); nothing writes one. (2) *The counts are machine-bound* — five committed tests hard-require gitignored `.architect-team/` fixtures and one skips without the local deploy config, so a fresh clone yields 7299/5F/7S, not the published figure (proven by predicting the pristine-worktree result in advance). Every count this repo prints describes THIS MACHINE, not the repo.

Both are the same class the v3.59.x releases addressed, still open in the process that shipped them.

Docs + one test-docstring ledger correction; no behaviour change. Counts UNCHANGED (53 / 39 / 25 / 7 / 23); `check_separation` unaffected (26).

Suite **7386 passing + 6 skipped, 0 failed** — unchanged (no test added or removed).
...
**Principle 7 sharpened — deliberately NOT an eighth principle.** All five witnesses satisfy principle 7's current letter ("state a result only after running the check and reading its output") — each check ran and was read — so the gap was in the letter, not the coverage. Principle 7's body gains a bind-the-check-to-the-claim paragraph and its compiled one-liner gains one clause: *a green check is evidence for what it measures, never for what you asserted*. The v3.47.0 Evidence-integrity section gains a **fourth** sub-rule, `### A green check proves what it measured`, anti-pattern *the borrowed green*, carrying the operational rule: name the instrument, then state the different, observable result it would have shown had the claim been false **of the object the claim names** — the same tree, the same arm, the same code path. "The same green" and "I cannot say" both mean nothing was verified. For a NEW guard, do not state the counterfactual — execute it once, with proof the falsification landed.

**The measurement that changed the fix.** The instinctive change was a new ETHOS section. It would have reached nobody: **39/39** agents carry principle 7's compiled one-liner, **39/39** carry the Evidence-integrity sub-rules as clauses inside it, and **0/39** carry any sub-rule heading. A standalone section in `docs/ETHOS.md` is read by a human maintainer and by nothing in the runtime. Editing it and declaring the principle shipped would itself have been a borrowed green — so the clause went on the compiled surface, and delivery was verified by COUNTING the agents that carry the new text (39/39, all on the Evidence-before-assertion bullet, with a negative control returning 0).

**The 23rd Layer-3 tool.** NEW `hooks/vao/claim_binding.py` exposes `verify-claim-instrument-binding` through the existing facade. It asks the question neither half of `verify-check-can-fail` asks — that tool checks whether a check read anything and whether it has ever been shown able to fail; **every one of the five witnesses passes both unchanged** — though the demonstration only carries weight for W1 and W2, since W3/W4/W5 add no test file and so fall outside `verify-check-can-fail`'s jurisdiction entirely. Claims are **agent-authored**, never inferred from evidence: inferring what was asserted would reproduce this defect one level up, measuring a reconstructed claim rather than the claim made, and a false positive against an assertion the agent never made is unanswerable. The tool refuses a mutation whose sha256 shows the file never changed, a witness whose mutated run stayed green, and a measurement whose `tree_state` moved underneath it. Where binding is genuinely undecidable — whether a needle CAN occur without running the renderer; an assertion's polarity when unlabelled — it emits a **note, never a gap**, because a Layer-3 tool that blocks on a heuristic gets its kill-switch set and a gate that is off protects nothing.
...
Layer-3 tools **22 -> 23**; skills / agents / commands / hook scripts UNCHANGED (53 / 39 / 25 / 7); `check_separation` unaffected (26).

Suite **7305 -> 7360 passing + 6 skipped, 0 failed** (+55 tests; NEW `tests/test_claim_instrument_binding.py`; 243 top-level test files, both encodings).
...
**N5b — a wedged run was unreleased AND silent.** "Unbounded" was chosen deliberately and is unchanged; the incidental second half was that *nothing told you either*, so a run wedged overnight produced no signal. The lock now emits ONE best-effort `issue_discovered` notification after `CT6_COMPLETION_LOCK_NOTIFY_AFTER` (default 5) consecutive blocks and **keeps blocking**; the counter resets when the lock stops blocking so a later wedge is not swallowed by a stale flag; `CT6_COMPLETION_LOCK_NOTIFY_DISABLED` silences the channel without weakening the gate. The doc's explicit prohibition is honoured and pinned: the continuation guard's no-progress counter is NOT advanced and `escalation-pending.md` is not raised (ADV-1 — the marker is agent-written, so that route would appear to escalate without releasing anything and would mis-fire the guard once work closed). The emission path is wrapped in a bare `except`, so its test observes a **sentinel notifier from outside the hook** — the first cut called a nonexistent `_utc_now` and the swallow hid it completely.

**F7 — and the first fix was ineffective.** Neutralization is applied PER FIELD in `_lock_task_text`, not to the assembled line: `_lock_task_text` prefixes `[<id>] `, so a subject's forged bullet is never at position 0 and a leading-shape strip in `_lock_clip` never reached it. **The test asserted a rendering that could not occur and passed vacuously; the mutation witness is what exposed it.**

**N3 + N2b — the two named guard boundaries, closed.** The guard protects the UNION of the real default task root and any `CT6_TASKS_ROOT` override — an environment variable may ADD to the protected set, never remove the real store from it. And `(st_dev, st_ino)` identity catches a hardlink under any name, failing safe throughout (compared only where both paths exist, `st_ino == 0` yields no identity, indeterminate falls back to resolved-path comparison, the arm can only ever ADD a block, and it is skipped entirely for `st_nlink == 1` so an ordinary write pays one stat).

**The `prose_lines` / `line_count` naming trap — and this repo's own record was WRONG about it.** The finding said one counter was load-bearing and the other cosmetic. T4 made that true; **G1 then put the ceiling arm back on the full count** and the comment was never corrected — by v3.56.0 there were THREE counters and all three were load-bearing. Renamed to `all_nonempty_lines` / `unfenced_lines` / `report_length_lines` with the nesting relation and a pick-by-meaning rule at the call site. Equivalence was **proved**: a 40-input corpus captured from `9750728` before the edit, full returned-dict comparison, **0 mismatches**. The corpus landed as a permanent parametrized test — that regression net, not the rename, is the deliverable; all three counter substitutions turn it red.

**G2 — ANSWERED, and the precondition is FALSE.** Scanned **679 transcripts across 23 projects, 1120.3 MB**; of 331 matching records, 8 were `role: user` strings and **all 8 dissolved** (7 `<teammate-message>` envelopes, 1 compact summary). **Zero** harness-written blocked-Stop records. The decisive evidence is the OLD sample — the v3.30.0 continuation guard has used the same mechanism for months and contributes 64 records, none of them user-role feedback — so this is a harness property, not a thin sample. G2's boundary-advance is therefore **inert**. Consequence bounded: `turn_output` is evaluated only when work is already open, so G2 can never CAUSE a block; it produced a misleading message. Fixed by making the message honest rather than by adding state — the `note_continuation_block` objection still stands (empty in exactly the plain sessions this lock exists for; writing it from the lock would pollute every repo the user types in). The message no longer claims "the last turn tripped it"; it states that output **since the last user prompt** is a narrative and discloses that this covers the whole turn. The rule keeps its teeth.

**Mutation-harness classification converted.** `mutation-checks-lock-wiring.txt` is regenerated by an EXIT-CODE harness: anchor-matched, file-changed-by-sha256 before running anything, classified on pytest's exit code (1 caught, 0 ESCAPED, 2..5 `[ERROR]` so a collection error can never read as caught), restored-sha verified plus a green re-run before scoring. All mutations re-run for real: **22 caught, 0 escaped, 0 no-op, 0 broken.**

**IN-FLIGHT SCOPE AMENDMENT — the frontend-E2E loop-exit gate stopped asking.** Reported live: *"25 of the 27 flagged slices are pre-existing debt"* — a commit blocked over work the run never touched, with the user asked to choose. Three defects, all the pipeline's: (1) **retroactive scope** — `.architect-team/reviews/` is cumulative, so a v3.55.0 gate demanded live evidence from slices predating it; `_audit_frontend_e2e` now scopes to slices this run OWNS (claimed by a teammate manifest OR written after the run marker's `started_at`) and prints how many it excluded, while keeping today's behaviour when NEITHER signal exists because under-blocking is the defect v3.55.0 removed; (2) **no path to a live environment** — `common-pipeline-conventions` gains *"Local -> dev -> test-on-dev is the pipeline's job, not the user's"* (bulk in `references/local-dev-test-discipline.md`): bring the environment up or run `deploy_command`, seed through the application's own create path, then run the flow; (3) **it asked** — *"how do you want to proceed?"* with a recommended option attached is not an escalation, it is the run asking permission to do its own job. **HONEST BOUNDARY:** the inherited debt still exists — scoping stops it blocking the next commit, it does not close it.

Suite **7222 -> 7284 passing + 6 skipped, 0 failed** (+62 tests; 241 top-level test files, both encodings; no new test file — every property landed in the file that already owned its subject).

## [3.56.0] — 2026-08-12 — turn-boundary-completion-lock (a session cannot end its turn while registered work is open, and the condition is read from disk rather than asserted)

**MINOR — new capability.** Closes a user-reported failure the instruction tier could not fix: Agent Teams sessions ending their turn with work still open. The reporting agent diagnosed its own mechanism exactly — *"every time my turn is about to end, I fill it with a summary; writing a summary forces me to decide what's done enough to describe, and that decision is me drawing the boundary again"* — and then, in the same turn, produced two formatted reports while seventeen items sat open. The user had already tried the instruction-tier fixes (*"make a list of my outstanding asks"*, *"you cannot consider anything complete until you have finished all my asks"*). What was left was machinery.

**The decisive property: the stopping condition is READ FROM DISK, never asserted by the agent.** Three facts made it buildable — the harness persists the shared task list to `~/.claude/tasks/session-<first-8-of-session-id>/<taskId>.json` (`status` ∈ `pending | in_progress | completed`, with an `owner`); the Stop payload already carries `session_id` (`pipeline-completion-audit.py`); and CT6's Stop hook is already wired at `matcher: "*"` in every session. What was missing was that no arm in `hooks/` read the aggregate list at all, and `_is_real_run` kept the hook deliberately inert outside a CT6 run — which is exactly why the reported 17-item session sailed through.

**ralph-loop is deliberately NOT adopted at the top level.** The user asked directly. Its exit condition is a literal `<promise>` string the model types, compared by shell string equality, with nothing verifying it (its own system message says *"do not lie to exit!"*). Wrapping the top-level run in it would relocate the reported failure rather than remove it. ralph-loop keeps its place in the convergence sub-loops of the twelve skills that use it. The rationale is written into `docs/ETHOS.md` so a later run does not re-adopt it.

- **NEW `hooks/open_work.py`** (stdlib-only) — the substrate: `read_harness_tasks` (read failures are DATA in an `unreadable[]` partition, never exceptions — the property all of REQ-6 rests on), `open_task_items` (unknown/missing status counts as OPEN; the asymmetry fails safe), the durably-accumulated ask-ledger, `classify_turn_output`, `teammate_name`, and the single entry point `evaluate_completion_lock`.
- **The lock in `hooks/pipeline-completion-audit.py::main()`** — evaluated ABOVE the non-engaged early return, the no-progress budget's `return 0`, the escalation-marker return, AND `_in_progress_is_fresh`. Placement is the requirement, not an implementation detail, and it is pinned by tests that fail when the lock is moved. It COMPOSES the continuation guard's block (`CONTINUE`, the worklist, the post-compact skill-reload directive) and heartbeats the marker, so an engaged run loses nothing — while deliberately NOT calling `note_continuation_block`, because advancing that counter would burn the guard's auto-escalation budget on a run that was in fact progressing.
- **Unbounded, with four per-source kill-switches** — `CT6_COMPLETION_LOCK_DISABLED` (master), `CT6_TASK_LIST_GATE_DISABLED`, `CT6_ASK_LEDGER_GATE_DISABLED`, `CT6_TURN_OUTPUT_GATE_DISABLED`. Disabling one source leaves the others enforcing. The pre-existing arms keep `CT6_MAX_NO_PROGRESS_STOPS` unchanged. "Unbounded" means the AGENT can never decide to stop; the human always can.
- **Only a source that can verify its own release condition may refuse a stop.** The task list may block (the harness writes `status`, so "done" is a fact the gate reads) and so may the turn-output rule (narrative shape is decidable from the text). The **ask-ledger is ADVISORY** — it knows a directive was *given*, never that it was *met*. Shipped as a blocking source it made an ordinary session — one request, no tasks, no CT6 run — unexitable **forever**, because nothing ever closed an entry; found by testing the live plugin before trusting it. Asks are still recorded involuntarily and surfaced whenever another source blocks, so nothing the user asked for goes unmentioned; they simply do not hold the turn. Opt back in with `CT6_ASK_LEDGER_BLOCKING=1`. An advisory ask also cannot satisfy the turn-output rule's open-work precondition, so the ledger cannot block by proxy.
- **A short turn is never a narrative.** The rule was `>= 3` lines **OR** any structural marker, which fired on `**Status:** still on task 1 of 9.` — one line, the exact terse shape the block message demands. The block asked for one line of state and then refused one: an unbreakable loop whose only exit was a kill-switch. Markers are now detected and reported but not decisive; the predicate is `>= 3` non-empty lines OR `> 600` chars of unbroken prose, which catches the single-paragraph report that merely lacks newlines.
- **The auto-derived ask-ledger**, at `.architect-team/ask-ledger.json` — derived from the harness-written transcript, not a model-initiated registration call, so an agent cannot decline to register an ask. **Accumulation is load-bearing**: the transcript reader is tail-capped, so re-deriving from a truncated window on a long session would drop the earliest directives — the ones most likely still unfinished — and silently allow the stop. Re-derivation only ever ADDS. Ambiguous stays open.
- **Teammate owner-scoping** — a teammate is held only for tasks whose `owner` matches it, never wedged on lanes it cannot close, and the standdown requires a GENUINE `CT6-TEAMMATE` token rather than a heuristic.
- **The one-line-of-state turn-output rule** — the user's own named remedy, mechanized. Trips on a structural marker (heading, bullet — ASCII **or** Unicode, numbered item, `**Label:**` block, table row) at `>= 2` lines, on `>= 6` report-length lines, on `>= 12` lines of any length, or on `> 600` raw chars of unbroken prose. NEVER on a one-line turn whatever markers it carries, and it measures ALL assistant text since the last genuine user prompt — not the last block, which is what closes the summary-then-sign-off evasion. Both directions are pinned, because a rule that never fires is decoration and a rule that fires on a terse status update gets disabled.
- **Split failure semantics** — an own-code crash fails OPEN; an unreadable SOURCE BLOCKS and names what it could not read. A blanket fail-open there would reproduce the reported bug via one malformed file.
- **`hooks/pretool_unilateral_override_guard.py` extended** — refuses agent `Edit`/`Write`/`NotebookEdit` against the ask-ledger and the harness task store (ADV-3 / ADV-5).

**Adversarial review found TWELVE executed escapes across three rounds.** The accurate tally — replacing an earlier "eight closed" claim that overstated it: **five closed and verified**, **four narrowed with a named residual** (the ground-truth guard is closed against `Edit`/`Write`/`NotebookEdit` but Bash remains open by design; the substrate import is closed against every `BaseException` but `os._exit()` remains; `CT6_TASKS_ROOT` moves both the ground truth and the guard protecting it; deleting the ledger file erases accumulated asks), and **one accepted boundary** (`TaskUpdate(status="deleted")`). A further finding, N5, is a behaviour cost rather than an escape: because the lock returns above the continuation guard and does not advance its counter, a genuinely wedged run no longer auto-escalates to the human the way it did pre-v3.56.0 — the kill-switches are the exit. Two reversed architectural calls the architect had already endorsed, and both reversals are recorded in the change's `design.md` rather than quietly amended: (1) `_in_progress_is_fresh` was above the lock, but `in-progress.md` is written by the AGENT, so it was a self-asserted exit admitted through a side door — the corrected general rule is *the question is never what a file means, it is who writes it*; (2) the teammate standdown fell back on a `>= 1500`-char heuristic reachable by the **user's own long prompts**, which silently disarmed the whole gate. An independent review separately measured that exempting slash-command prompts from ledger derivation dropped the user's directive itself in **25 of 51** command-carrying prompts in their real history — including the prompt that commissioned this feature.

**HONEST BOUNDARY — the gate is not undefeatable and the docs say so.** `TaskUpdate(status="deleted")` unlinks the task file (verified on a live task), so the lock reads a clean-empty dir and releases; deletion is a legitimate harness operation no hook tier can forbid. The ask-ledger does NOT mitigate this: it was demoted to advisory, so it never blocks on its own — and the advisory FYI listing renders only INSIDE a block, so at the exact moment every lane has been deleted there is no block and the recorded asks are neither blocking nor shown. Only `CT6_ASK_LEDGER_BLOCKING=1` restores the mitigation. More broadly, an agent with unrestricted `Bash` can write any file, including these records; the guard raises the cost and makes a bypass deliberate and visible rather than accidental. Stated precisely: **an agent can no longer end its turn early by deciding it is finished — it can still end early by destroying the evidence that it is not, which is a louder and far more detectable act than writing a summary.**

Suite **7024 -> 7222 passing + 6 skipped, 0 failed** (+198 tests; 241 top-level test files; both encodings). Skills / agents / commands / Layer-3 tools UNCHANGED (53 / 39 / 25 / 22); hook scripts unchanged at 7; `check_separation` unaffected (26).
...
**PATCH — TWO small fixes, each red-first tested; NO new skill / agent / command / hook script / Layer-3 tool — `check_separation` unaffected (26).** Fix 1 closes the v3.55.2 KNOWN FOLLOW-UP in code: the run-continuity constants gain the data-eng lane, so a `/architect-team:data-eng` run is now BOTH real-time gated (v3.55.2) AND tracked by the active-run marker / continuation guard / sticky arm (this release). Fix 2 sweeps the last recorded residue of the bounded-at-3-cycles anti-pattern class (v3.9.3 cleared the skills tier, v3.55.3 the agent bodies): the ux-test-builder SPEC-of-record still capped the U7 consensus loop its own skill had already dropped.

- **Run-continuity for the data-eng lane.** `hooks/run_continuity.py`'s `RUN_DRIVING_SKILLS` gains `data-eng-pipeline` (**4 → 5 members**: the 4 playbooks + `data-eng-pipeline`); `ENGAGEMENT_SKILLS` (= `RUN_DRIVING_SKILLS | {proposal-refiner}`) picks it up automatically (**5 → 6**). This RESTORES the `_PIPELINE_SKILLS == ENGAGEMENT_SKILLS` invariant at 6 (both now include `data-eng-pipeline`) — a `/architect-team:data-eng` run now ENGAGES the `active-run.json` marker at the `PostToolUse(Skill)` wiring and is recognized as engaged, so the continuation guard and the sticky arm track it like every other pipeline run — closing the v3.50.0 omission that was the run-continuity sibling of the v3.55.2 real-time-gate fix. Red-first test `test_data_eng_pipeline_engages_and_is_recognized` in the existing `tests/test_run_continuity.py`.
- **The ux-test-builder SPEC-of-record swept.** `openspec/specs/ux-test-builder/spec.md`'s U7 requirement + scenario still specced the "bounded at 3 re-examination cycles" consensus loop the ux-test-builder SKILL already dropped (the skill's no-cap framing was already pinned by `test_u7_loop_until_converged_no_cap`) — the spec-of-record contradicted the implementation. The U7 requirement + scenario now state the unbounded framing: no fixed cycle cap, the re-examination loop runs until the executors converge, and persistent genuine divergence (a real product ambiguity only the owner can adjudicate) surfaces to the user as required input (a domain gate; fires regardless of `--proposal-first`) while all other work continues — it never halts on cycle count. Red-first pin `test_u7_spec_of_record_matches_skill_no_cap` in the existing `tests/test_ux_test_builder_skill.py`.

Tests: **2 new test functions** in EXISTING files (no new test file). Suite **7022 → 7024 passing + 6 skipped, 0 failed** (239 top-level test files — disk-anchored via `git ls-files 'tests/test_*.py'`; Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1`; +2 tests, zero new failures); `check_separation` clean (checked 26, unaffected). Skills / agents / commands / hook scripts / Layer-3 tools UNCHANGED (**53 / 39 / 25 / 7 / 22**); `tests/test_dispatch_banner.py`'s version pin moved in lockstep with the bump.

## [3.55.3] — 2026-08-09 — debug-3cycle-fix (the bounded-at-3-cycles loop-cap anti-pattern swept from every agent body — 3 files / 4 locations; the debugging-skill review recorded, the multi-bug build queued)

**PATCH — docs-and-discipline: a consistency defect-CLASS sweep across 3 agent files / 4 locations (+ two red-first regression pins in the existing `tests/test_diagnostic_research_team.py` — one per-file, one repo-wide) and a recorded review (`docs/proposals/DEBUGGING_MULTI_BUG_ENHANCEMENT.md`); NO new skill / agent / command / hook script / Layer-3 tool — `check_separation` unaffected (26).** Origin: the user's "review how we handle debugging as a skill" request (2026-08-08) with an 11-point target spec. The review found the single-bug debugging discipline genuinely strong (7/11 points covered) and one consistency defect — which the paired audit and a new repo-wide pin then widened to a defect CLASS: the `bounded at 3 cycles` loop cap survived in THREE agent bodies, each directly contradicting its OWN skill's unbounded loop. All swept here; the spec's remaining delta — the multi-bug session layer — is captured as a build plan for a fresh run, not built in this PATCH.

- **The sweep (3 files / 4 locations).** `agents/diagnostic-researcher.md` capped the re-dispatch loop ("Loop is bounded at 3 cycles total — after that, surface to the human…") against the `diagnostic-research-team` skill's no-cap loop; `agents/system-architect.md` capped BOTH its editability and interaction Round-3 review loops ("bounded at 3 cycles…") against `editability-completeness` / `interaction-completeness`; `agents/oracle-deriver.md` capped the user-confirmation gate ("Bounded at 3 cycles per `common-pipeline-conventions`'s domain-gate convention" — a convention that does not exist) while its own skill `verified-agent-output` says "no fixed cycle cap … until the user accepts" (the same bounded-at-3 anti-pattern the v3.9.3 C3 residue sweep removed from `ux-test-builder` and the pipeline skills; the agent bodies were the missed residue). All four locations now document the unbounded loop (per `common-pipeline-conventions` `## Unbounded solving discipline`): the three architect loops run until the architect converges — cycle count never halts them, the ONLY interruption a genuine missing-required-input pause (a stale CODEBASE_MAP, an absent expectation file, a redacted log), surfaced loudly while the rest of the run continues, then resumed once supplied; the oracle confirmation gate repeats until the user accepts — the user's own confirmation IS the deliverable.
- **The regression pins (red-first).** TWO new tests in the existing `tests/test_diagnostic_research_team.py`: `test_researcher_agent_re_dispatch_loop_is_unbounded` (the per-file pin — no fixed cycle cap remains in the diagnostic-researcher AND the unbounded until-the-architect-converges loop is stated) and the repo-wide, case-INSENSITIVE `test_no_bounded_at_3_cycles_loop_cap_in_any_agent` (scans every `agents/*.md` body and forbids the anti-pattern's re-introduction anywhere). Detection honesty: the review found the diagnostic-researcher instance; the paired audit found the two system-architect locations; the repo-wide case-insensitive pin caught the capital-B oracle-deriver instance that a case-sensitive grep and the audit had both missed.
- **The review recorded.** NEW `docs/proposals/DEBUGGING_MULTI_BUG_ENHANCEMENT.md` — the debugging-skill review as a standing proposal: the 11-point coverage table with per-point evidence (7/11 covered), the prioritized gaps (P1 — multi-bug orchestration f/g/h/i: per-bug parallel confirm/research, a pooled per-session research index, the architect bug-grouping review, per-group solve→test→commit→publish release gates; P2 — hypothesis-exhaustion formalization; P3 — report-origin triage; P4 — minor hardening), the reuse-first E1–E9 enhancement plan, and the already-well-covered inventory (do NOT rebuild). The multi-bug orchestration build (E1–E5) is QUEUED as a fresh, focused `/architect-team` run per the user's decision (2026-08-08) — recorded, not built here.

Tests: **2 new test functions** in the existing `tests/test_diagnostic_research_team.py` (no new test file). Suite **7020 → 7022 passing + 6 skipped, 0 failed** (239 top-level test files — disk-anchored via `git ls-files 'tests/test_*.py'`; Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1`; +2 tests, zero new failures); `check_separation` clean (checked 26, unaffected). Skills / agents / commands / hook scripts / Layer-3 tools UNCHANGED (**53 / 39 / 25 / 7 / 22**); `tests/test_dispatch_banner.py`'s version pin moved in lockstep with the bump.

## [3.55.2] — 2026-08-08 — data-eng-realtime-gate-fix (the data-eng lane joins the real-time PreToolUse skill-gate — the v3.50.0 real-time-gating gap closed)

**PATCH — a one-constant bug fix in `hooks/pretool_skill_gate.py` (+ a red-first regression test): `data-eng-pipeline` joins `_PIPELINE_SKILLS`, so `/architect-team:data-eng` is now REAL-TIME gated exactly like the other five pipeline commands; NO new skill / agent / command / hook script / Layer-3 tool — `check_separation` unaffected (26).** The v3.50.0 data-eng lane wired `/architect-team:data-eng` into the Layer-6 Stop-time skill-invocation auditor (`hooks/skill_invocation_audit.py`'s `_PIPELINE_COMMAND_SKILLS` / `COMMAND_TO_SKILLS`) but omitted `data-eng-pipeline` from the real-time PreToolUse gate's `_PIPELINE_SKILLS` — so a `/architect-team:data-eng` request could reach build/dispatch tools (Edit / Write / NotebookEdit / Agent / Task*) without the `data-eng-pipeline` Skill ever being engaged, with only the after-the-fact Stop-time audit to notice. The v3.55.1 independent doc-audit had documented the five-member reality as a current-state note (its finding B1); this PATCH closes the underlying asymmetry in code.

- **The fix.** `data-eng-pipeline` is added to `hooks/pretool_skill_gate.py::_PIPELINE_SKILLS` (**5 → 6 members**: the 4 run-driving playbooks + `data-eng-pipeline` + `proposal-refiner`). An unsatisfied `/architect-team:data-eng` request now BLOCKS (exit 2) the first build/dispatch tool call until the `data-eng-pipeline` Skill is engaged — prevention, not just detection, identical to the other five pipeline commands. Read-only investigation + the command wrapper's own `Bash` setup stay un-gated (the v3.15.1 narrowing, unchanged).
- **The regression test (red-first).** `test_data_eng_command_gated_and_satisfied_by_dataeng_skill` in the existing `tests/test_pretool_skill_gate.py`: an unsatisfied `/architect-team:data-eng` request blocks `Edit` (exit 2); engaging `data-eng-pipeline` satisfies the gate (exit 0).
- **The docs brought current.** `docs/INTEGRATION_MAP.md`'s gate-scope parenthetical ("five of these six" → all six) and its three-skill-set-constants paragraph, plus `docs/CODEBASE_MAP.md`'s `pretool_skill_gate.py` bullet, now state the six-member reality — and the deliberate non-uniformity below.

HONEST BOUNDARY / KNOWN FOLLOW-UP: the run-continuity constants (`hooks/run_continuity.py`'s `RUN_DRIVING_SKILLS` / `ENGAGEMENT_SKILLS`) STILL omit `data-eng-pipeline` — `_PIPELINE_SKILLS` (6) now deliberately differs from `ENGAGEMENT_SKILLS` (5) by exactly `data-eng-pipeline`. A `/architect-team:data-eng` run is therefore real-time gated but NOT yet tracked by the `active-run.json` marker / continuation guard / sticky arm. That scope boundary is documented (`docs/INTEGRATION_MAP.md`'s three-constants paragraph), not hidden; extending the run-continuity constants to the data-eng lane is a known follow-up.

Tests: **1 new test function** in the existing `tests/test_pretool_skill_gate.py` (no new test file). Suite **7019 → 7020 passing + 6 skipped, 0 failed** (239 top-level test files — disk-anchored via `git ls-files 'tests/test_*.py'`; Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1`; +1 test, zero new failures); `check_separation` clean (checked 26, unaffected). Skills / agents / commands / hook scripts / Layer-3 tools UNCHANGED (**53 / 39 / 25 / 7 / 22**); `tests/test_dispatch_banner.py`'s version pin moved in lockstep with the bump.

## [3.55.1] — 2026-08-07 — compact-readme-status-timeline (docs-only PATCH: the README carries only the current release, the full timeline moves to docs)

**PATCH — docs-only: relocates the README's embedded release timeline to `docs/RELEASE_HISTORY.md`, refreshes the MemPalace, and sweeps the docs current; NO source / test-behavior change, no new skill / agent / command / hook / Layer-3 tool — `check_separation` unaffected (26).** Finishes the README compaction v3.47.1 began. v3.47.1 moved the accumulated per-release narrative to `docs/RELEASE_HISTORY.md` and kept a swapped-per-release spotlight, but it left the README's `STATUS` section carrying a one-line-per-release timeline (`v0.1.0 → v3.55.0`, ~135 lines that grew by one every release). The user's mandate: a more compact README — only the most recent release's changes, with the accumulated updates in their own file in docs and a pointer to the full history.

- **REQ-1 — the README carries no accumulated release timeline.** The `STATUS` section's `v0.1.0 → v3.55.0` one-line timeline is removed from `README.md`; the section keeps the house `▰▰▰` frame but names only the current release + a house-style pointer to `docs/RELEASE_HISTORY.md`. The README keeps exactly one `NEW IN` spotlight (swapped to v3.55.1) and its existing top `RELEASE HISTORY` pointer block. README **1193 → 1066 lines**.
- **REQ-2 — the full timeline lives in its own docs file.** The one-line timeline moves into `docs/RELEASE_HISTORY.md` as a newest-first **Release timeline at a glance** index above the detailed per-release narrative — no release content lost (`test_release_history.py`'s completeness floor is unchanged at ≥55; the file now names 66 release identities and still carries the current release + the `CARRIED FROM v2.3.0` earliest anchor).
- **REQ-3 — MemPalace refresh + documentation-currency sweep.** The MemPalace was refreshed with the latest changes (the data-engineering lane Runs A–F v3.49.0–v3.54.0, the v3.55.0 frontend-E2E loop-exit gate, and this compaction). A comprehensive documentation-currency sweep corrected the `docs/CODEBASE_MAP.md` "Currency note" that still pinned the v3.49.0 inventory (50 skills / 24 commands / 21 Layer-3 tools / 6878 tests) → v3.55.1 (53 / 25 / 22 / 7019), plus any other stale current-state references the per-release sweeps missed. An independent system-architect Documentation Currency Audit (producer ≠ checker) re-verified the whole change.

HONEST BOUNDARY: docs-only — this relocates documentation and refreshes currency; it changes no runtime behavior. The previous release's headline (v3.55.0 frontend-e2e-loop-exit-gate) and every earlier release remain in full at `docs/RELEASE_HISTORY.md`.

Tests: no new test files (docs-only). Suite **7019 passing + 6 skipped, 0 failed** (239 top-level test files — disk-anchored via `git ls-files 'tests/test_*.py'`; the release-notes' historical "235" undercounts this basis, kept byte-stable in the older entries — Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1`; unchanged — the `test_release_history.py` / `test_readme_styling.py` / `test_dispatch_banner.py` version pins re-derive from `plugin.json` and stay green after the lockstep bump); `check_separation` clean (checked 26, unaffected). **Layer-3 tools unchanged at 22**; skills / agents / commands / hooks UNCHANGED (53 / 39 / 25 / 7).

## [3.55.0] — 2026-08-07 — frontend-e2e-loop-exit-gate (click-driven, as-the-user, full E2E Playwright becomes a HARD run-level loop-exit criterion)

**MINOR — additive within `hooks/` (stdlib-only): a NEW completion-audit arm + the 22nd Layer-3 falsifiability tool + a schema hardening; NO new services module / skill / command / agent — `check_separation` unaffected (26).** Commits were landing from CT6 runs that touched the frontend but were never Playwright-tested as a real user. The v3.44.0 machinery existed (`hooks/frontend_impact.py::detect_frontend_impact` + the conditionally-required `frontend_impact_e2e_review` field) but had two escape hatches: the gate was **per-task** (checked at the Phase 3 review gate, never at the completion audit that actually blocks the commit), and its skip was a **self-authored note**. This run makes genuine click-driven, as-the-user, full end-to-end Playwright testing a **run-level, non-bypassable loop-exit criterion**.

- **REQ-1 — the run-level loop-exit gate.** A NEW `_audit_frontend_e2e` arm in `hooks/pipeline-completion-audit.py` (registered in `audit()`, on the `_audit_bug_fix_testing` template). It aggregates `files_changed` across the run's `.architect-team/reviews/*.json`, runs the EXISTING `changed_files_touch_frontend`, and — for a frontend-impacting run — REQUIRES, per frontend slice, a genuine passing E2E verdict at `.architect-team/frontend-e2e/<slice>-verdict.json` (`verdict=="passed"`, `executed_against_live_env`, ≥1 `user_driven_actions`, a `trace_path` that EXISTS, ≥1 `visible_state_assertions`). A missing / non-passing verdict — or a slice that touched a real frontend UI file but produced only a review-gate note — **BLOCKS the run's completion**. Kill-switch `CT6_FRONTEND_E2E_GATE_DISABLED`; no-op for a run that touched no frontend; fail-open outside an active run.
- **REQ-2 — the genuineness verifier (the 22nd Layer-3 tool).** `verify-frontend-e2e-loop-exit` in `hooks/vao/` (dispatched through the `hooks/vao_tools.py` facade — Layer-3 tools **21 → 22**). It validates an E2E verdict artifact is GENUINE as-the-user testing and bites on the four escape modes: `e2e-described-not-executed` (not run against a live env / no trace), `e2e-api-only-no-user-actions` (only `page.request.*`/`fetch`/direct-API, no `page.click`/`fill`/`getByRole`/`check`/`selectOption`), `e2e-vacuous-navigate-assert` (no assertion on visible end-to-end state), `e2e-trace-claimed-but-absent` (`trace_path` set but the file does not exist). A real click-driven artifact with an existing trace + a visible-state assertion returns `valid:true`, 0 gaps. Deterministic, stdlib-only; each escape has a red fixture proving the bite.
- **REQ-3 — escape-hardening + wiring.** `hooks/review_evidence_schema.py` keeps `frontend_impact_e2e_review` conditionally required on a frontend-touching diff, and documents that the `n/a`/note escape is legitimate ONLY for a change with no runnable UI surface — the run-level arm is the backstop that no per-task note can escape (a producer can no longer self-authorize away the most important gate). Wired into `skills/architect-team-pipeline/SKILL.md` (Phase 3 references it; Phase 5 writes the verdict artifact when it runs Playwright; Phase 8 lists `_audit_frontend_e2e` in the completion-audit worklist), `skills/playwright-user-flows/SKILL.md` (the verdict artifact is the deliverable of a frontend flow run), and `docs/ETHOS.md` (the loop-exit principle: a frontend change is not done until a real user clicked through it).

The paired review earned its keep — on the gate itself. The adversary (concurred by the independent reviewer) caught a BLOCKING hole (B1): the enforced arm's genuineness check was a shallow COUNT (`len(user_driven_actions) >= 1` / `len(visible_state_assertions) >= 1`), so a POPULATED-but-fake `passed` verdict — actions present but all `page.request`, or assertions present but only a title/navigate — slipped past the run-level gate (reproduced through the real hook, EXIT 0), while the deep genuineness tool that catches it was wired to no enforced gate. Closed fix-forward (TDD red-first, with arm-level populated-fake reds the original suite lacked): `_audit_frontend_e2e` now imports and CALLS `verify_frontend_e2e_loop_exit` per slice and blocks on not-valid — the deep genuineness IS the enforcement, not a shallow count a crafted verdict slips past. Both reviewers re-verified to PASS on the fixed arm.

HONEST BOUNDARY: this run ships the ENFORCEMENT — it does not itself run a browser. It makes the ABSENCE of a real, executed, click-driven, live-environment E2E test a hard blocker on the commit for any frontend-impacting run; the actual Playwright execution + the verdict artifact are the frontend/integration agent's job downstream. Behavior is verified over fixture verdict artifacts (a genuine one + each of the four escape modes) + the arm's block/no-op/kill-switch paths + a captured demo.

Tests: **1 new test file** (`test_frontend_e2e_loop_exit.py`, 37 tests) + fixtures under `tests/fixtures/vao/` (genuine + four escape-mode E2E verdicts). Suite **6982 → 7019 passing + 6 skipped, 0 failed** (235 test files, Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1`; +37 tests, zero new failures); `check_separation` clean (checked 26, unaffected — hooks tier). **Layer-3 tools 21 → 22**; skills / agents / commands / hooks-scripts UNCHANGED; `tests/test_dispatch_banner.py`'s version pin moved in lockstep with the bump.

## [3.54.0] — 2026-08-03 — jsonld-emitter (Run F: the data-engineering lane's LAST run — the catalog + lineage sidecars become standard JSON-LD, corroboration thread closed)

**MINOR — additive: a NEW `scripts/data_dictionary/jsonld_export.py` deterministic stdlib serializer (+ two small per-field corroboration hardening edits); stdlib-only (`json` only — NO rdflib/pyld); NO new services module / skill / command / agent — `check_separation` unaffected (26).** Run F of `docs/proposals/DATA_ENG_LANE_AND_CROSS_POLLINATION.md` §7.6 (R6) — the final run of the six-run build sequence. It emits the two machine-complete sidecars CT6 already produces — `docs/data-dictionary.json` (the catalog) + `lineage-graph.json` (the lineage) — as **JSON-LD** for the external catalog / lineage ecosystems (OpenLineage / Marquez / DataHub / dbt) Stage 6 of `data-engineering-exploration` names.

- **The emitter — standard vocabularies, stdlib-only.** `export_dictionary_jsonld` maps each dictionary table to a `schema:Dataset` (a `dcat:Catalog` wraps them); each field becomes a `schema:variableMeasured` `PropertyValue` carrying the definition + `ct6:provenance` + `ct6:confidence` (+ `ct6:usage` when a Run E usage block is present) — verbatim. `export_lineage_jsonld` maps the lineage graph to **PROV-O**: a `data_asset` → `prov:Entity`, a `function`/`endpoint` → `prov:Activity`, a `reads` edge → `prov:used`, a `writes` edge → `prov:wasGeneratedBy` — walking ONLY the edges present in the graph (an unknown edge kind is skipped, never invented). `export_combined` emits one JSON-LD document (`@context` binding the real vocabulary URLs + `@graph`).
- **Validation is the correctness bar (no live consumer).** `validate_jsonld` asserts a resolvable `@context`, every `@graph` node carries an `@id` + `@type`, `@id`s are unique, and every referenced `@id` (edge endpoints, `variableMeasured` refs) resolves to a node (referential closure). A round-trip (serialize → parse → re-derive the source facts) proves no data is lost or fabricated. The emitter emits ONLY what the sidecars contain — an empty/absent input yields an empty but VALID `@graph`, never fabricated triples (the emitter's analogue of the corroboration discipline).
- **The corroboration thread closed.** Run F folds in the two same-class fast-follows Run E's adversary logged, so the marathon ends with NO known corroboration edges open: (F1) `data_dictionary.py::_usage_block` now returns `None` when every usage sub-value is `None` (so a misbehaving adapter returning a non-`None` empty mapping OMITs the block instead of attaching an all-`None` one — engine-enforced, not only adapter-enforced); (F2) `sql_mining.py::corroborate_mined_claim`'s rows-path now carries the `non_null_sampled == 0` guard its two sibling gates (annotations F-A5, `_corroborate_correction`) already have. Both fixed red-first; Run C's 29 + Run E's + Run D's tests stay green.
- **`skills/data-dictionary/SKILL.md`** gains a short JSON-LD-export note (the emitter, the shapes, the no-consumer boundary). No skill-count change — R6 "moves no count except tests."

HONEST BOUNDARY (the D7 conditional, adjudicated): §7.6 marks R6 "build ONLY when a named external consumer exists; otherwise backlog deliberately." **No external consumer is wired in this repo.** It is built anyway to complete every run of the proposal, and ships with the boundary stated plainly: the emitter produces JSON-LD conforming to the schema.org / DCAT / PROV-O SHAPES, structurally validated + round-trip-parsed — **NO live external-consumer (OpenLineage/Marquez/DataHub) ingestion is verified; none is wired.** A consumer wired later needs only to point at the output. It emits only what the sidecars contain — no fabricated triples, provenance preserved.
...
Tests: **2 new test files** (`test_jsonld_export.py`, 17 tests — the dataset/PROV-O mapping, `validate_jsonld`, the round-trip, the empty-input/no-fabrication guard; `test_data_dictionary_usage.py`, 6 tests — the F1 usage-block OMIT) + 4 F2 tests in `test_sql_mining.py` (29 → 33) + fixtures under `tests/fixtures/jsonld/` (a sample dictionary + lineage graph, since the CT6 repo ships no sidecars). Suite **6955 → 6982 passing + 6 skipped, 0 failed** (235 test files, Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1`; +27 tests, zero new failures); `check_separation` clean (checked 26, unaffected). Skills / agents / commands / hooks / Layer-3 tools UNCHANGED (53 / 39 / 25 / 7 / 21); `tests/test_dispatch_banner.py`'s version pin moved in lockstep with the bump.

## [3.53.0] — 2026-08-03 — usage-stats-review-roundtrip (Run E: the catalog gets importance-aware + human-correctable — measured usage, an offline stakeholder review round-trip, corroboration hardening)

**MINOR — additive within `scripts/data_dictionary/data_dictionary.py` (+ two small per-field hardening edits); stdlib-only; NO new services module / skill / command / agent — `check_separation` unaffected (26). A dictionary built without a usage source, and a repo that never runs the review, are byte-unchanged.** Run E of `docs/proposals/DATA_ENG_LANE_AND_CROSS_POLLINATION.md` §7.5 (R4 + R5, "small enough to pair") — the standing catalog stops guessing importance and gains a human-in-the-loop correction channel, and the two same-class corroboration follow-ups logged in Runs C/D are closed on the same surface.

- **R4 — usage-grounded importance, MEASURED or OMITTED (never fabricated).** An optional injected `UsageStatsSource` seam (following the `build_from_sqlite` adapter shape) attaches a per-table `usage` block (`read_recency` / `write_recency` / `row_volume`) at provenance `live-data` — because it IS measured. SQL Server (`sys.dm_db_*`) + Postgres (`pg_stat_user_tables`) shapes are described; `SqliteUsageStats` honestly returns none. **Absent stats OMIT the block entirely — never zeros, never `{}`** (the deng silently-single-target anti-pattern, refused). `PROVENANCE_TYPES` is unchanged (usage rides `live-data`); a no-source build is byte-identical to before.
- **R5 — the offline stakeholder review round-trip, with the pinned test deng lacks.** `generate-review` / `apply-review` subcommands (stdlib `csv`): `generate-review` writes the fields most needing human eyes (`confidence == "low"` OR a ⚠ corroboration conflict), sorted by R4 `usage.row_volume` when present, redacted through `scripts/helpdesk/logit.py::redact_evidence` (default `summary` — sample-derived detail dropped; `--privacy full` opt-in). `apply-review` reads the edited CSV and ingests each non-blank `proposed_definition` as a `provided_defs` entry **through `corroborate_definition`** — a human correction is corroborated like any other claim (a conflict is ⚠/downgraded, NOT blessed). The PINNED round-trip test (write → edit one cell → read → assert the correction lands on the EXACT `table.field`, no column drift) is the test deng's column-drifted sheets prove the need for.
- **Corroboration hardening — the two logged follow-ups closed on the same surface.** (1) `scripts/sql_mining/sql_mining.py::corroborate_mined_claim` is now PER-FIELD: a candidate whose field is absent from the `corroborated_defs` map with `rows=None` stays `inference` / not-accepted (the Run C R1 residual). (2) `scripts/data_dictionary/annotations.py` closes the Run D F-A5 residual: on the `--db` rows path, a factual claim whose composed corroboration reports `non_null_sampled == 0` is treated as not-checked — which covers both an all-NULL column and a column absent from the sampled rows (an all-NULL / ghost column does not corroborate a TEXT-family claim), while higher-evidence types (boolean/integer/date/key) keep flagging correctly. Both fixed red-first; Run D's 44 annotation tests + Run C's 29 sql-mining tests stay green.
- **`skills/data-dictionary/SKILL.md`** gains the `usage` block + the `generate-review` / `apply-review` round-trip contract (redaction + corroborate-on-apply). No skill-count change — it extends an existing skill.
...
Tests: **1 new test file** (`test_usage_review.py`, 29 tests) covering the R4 measured/omitted split, the R5 pinned round-trip + corroborate-on-apply + default-summary redaction, and both hardening reds. Suite **6926 → 6955 passing + 6 skipped, 0 failed** (235 test files, Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1`; +29 tests, zero new failures); `check_separation` clean (checked 26, unaffected). Skills / agents / commands / hooks / Layer-3 tools UNCHANGED (53 / 39 / 25 / 7 / 21); `tests/test_dispatch_banner.py`'s version pin moved in lockstep with the bump.

## [3.52.0] — 2026-08-03 — data-annotations (Run D: the warm catalog gets a team memory — per-user annotations on data objects, corroborated on ingest, merged at query, gate-integrity preserved)

**MINOR — additive: a NEW `scripts/data_dictionary/annotations.py` deterministic stdlib engine + a NEW `data-annotations` skill + an ADDITIVE merge in `services/knowledge_server/dictionary_source.py`; a repo with no annotations behaves EXACTLY as before; scripts-tier stdlib + additive-within-DictionarySource so `check_separation` is unaffected.** Run D of `docs/proposals/DATA_ENG_LANE_AND_CROSS_POLLINATION.md` §3b(R3)/§7.4 — the lane's team-shared memory on data objects. deng-toolkit's genuinely good pattern (per-user, git-shared annotation files that never merge-conflict) is adopted for its INTERFACE while CT6's SEMANTICS keep it honest: a factual annotation is a CLAIM that must be corroborated, never transcribed as truth, and served annotation memory INFORMS a per-run gate, it never skips or auto-satisfies one. Decision D5 (user-delegated): data objects first; persisting interaction/bulk-verify domain-gate confirmations as annotations is a documented, deferred second step (a hook, not built).

- **The two load-bearing disciplines, each proof-carried.** (1) **Corroborate-on-ingest, PER FIELD** — an annotation carrying a factual claim (`claims_key` / `expected_type` / a definition) routes through `scripts/data_dictionary/data_dictionary.py::corroborate_definition`; it is accepted as corroborated truth ONLY when its EXACT `table.field` was actually inspected (rows sampled for that column, OR a corroborated definition for that field compared) — a field absent from every corroboration source is served `uncorroborated` / not-accepted / `needs_corroboration=true`, never as established truth. (2) **Gate integrity** — `inform_gate` re-derives a gate's `satisfied` from the gate's OWN evidence only; an adversarial annotation carrying `satisfied=true` or an accepted claim cannot flip it (the guard mutated → a test reds). Non-factual annotations (`note` / `quality_flag ∈ {TRUSTED, STALE, INCOMPLETE, EXPERIMENTAL}` / `deprecation`) are stored as-authored (opinions), and a `quality_flag` travels with the served field.
- **NEW `scripts/data_dictionary/annotations.py`** — per-user `docs/data-annotations/<user>.json` files anchored to `table` / `table.field` ids, validate-on-write + atomic (the house pattern), CLI-driven (mirrors `data_dictionary.py`), NO new services module. Per-user = never-merge-conflict; usernames are validated (injective username→file mapping — collision-prone / unsafe names refused).
- **ADDITIVE server merge-at-query** — `services/knowledge_server/dictionary_source.py` `get_table_details` (+ `search_dictionary`) merge per-user annotations into the response with each field's `corroboration_status` + `quality_flag`, the existing `{verdict, basis}` freshness envelope still applied. A no-annotations repo response is BYTE-UNCHANGED (additive; the closed output contract gains no new top-level field).
- **NEW `data-annotations` skill** — the engine's contract (store shape + vocabulary, the corroborate-on-ingest inversion, the server merge-at-query, the mine-to-MemPalace recall path, and the GATE-INTEGRITY rule) → skills 52 → **53**. Recalled annotations INFORM, never auto-satisfy, a gate; the deferred second D5 step is documented as a hook.
- **The paired review earned its keep — again.** The first review returned FAIL from the adversary with **one blocking corroboration-integrity finding (F1)**: a factual claim on a field in NO corroboration source (absent from a supplied `corroborated_defs` map, `rows=None`) was stamped `accepted` / `corroborated` WITHOUT `corroborate_definition` ever running for that field — CLI-reachable (`annotate --anchor ghost.col --dictionary <dict>` without `--db`). Root cause: a not-per-field `ran = rows is not None or bool(corroborated_defs)`. Closed fix-forward (TDD red-first): corroboration is now PER-FIELD (`checked` true only when THIS column was inspected) + username validation (F2) + anchor-existence (F3). Both reviewers re-verified to PASS (each confirmed the fix bites). Same class as Run C's F-findings — the discipline caught it in every code-heavy run.

HONEST BOUNDARY: Run D ships the annotation ENGINE + its corroborate-on-ingest + the additive server merge + the skill. It stores + corroborates + serves + mines annotations on DATA OBJECTS; persisting interaction/bulk-verify confirmations as annotations is the deferred D5 second step (documented, not built). It does NOT connect to a live warehouse. A cross-run follow-up is logged: apply the same per-field-acceptance fix to sql-mining's `corroborate_mined_claim` (the R1 residual) AND treat `corroboration.non_null_sampled==0` as not-checked at the annotations layer (a reviewer-flagged, non-blocking, non-CLI-reachable text-family-on-all-NULL-column edge rooted in the composed `corroborate_definition`'s pre-existing DD-14 semantic) — both fold into Run E, which touches the dictionary engine. Usage-stats + review round-trip (R4/R5) remain Run E; the JSON-LD emitter (R6) Run F (deferred per D7 pending a named consumer).

Tests: **1 new test file** (`test_data_annotations.py`, 44 tests) incl. the F1 acceptance tests (a factual claim on an absent / uncorroborated field is NOT accepted; the server surfaces it as uncorroborated, not truth), the F2 distinct-users no-collision test, and the gate-integrity mutation test. Suite **6878 → 6926 passing + 6 skipped, 0 failed** (235 test files, Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1`; +48 tests, zero new failures); `check_separation` clean (checked 26, unaffected — additive within DictionarySource, no new module); `tests/test_dispatch_banner.py`'s version pin moved in lockstep with the bump. Skills 52 → **53**; agents / commands / hooks / Layer-3 tools UNCHANGED (39 / 25 / 7 / 21).

## [3.51.0] — 2026-08-03 — warehouse-sql-mining (Run C: the standing warm catalog gets evidence — join/metric/relationship mining from SQL objects, corroboration-gated)

**MINOR — additive: a NEW `scripts/sql_mining/sql_mining.py` deterministic stdlib engine + a NEW `warehouse-sql-mining` skill; no existing behavior changes; scripts-tier stdlib so `check_separation` is unaffected.** Run C of `docs/proposals/DATA_ENG_LANE_AND_CROSS_POLLINATION.md` §3b(R2)/§7.3 — the largest independent engine. It extracts the join / filter / aggregate-ratio metric shapes and the table-to-table read/write relationships latent in a warehouse's stored procedures / views, so the lane's D0 exploration and the data-dictionary have *evidence*, not guesses — and everything it produces is **corroboration-gated** (the CT6 inversion of deng's fidelity problem). Decision D6 (user-delegated): a **vendored minimal stdlib T-SQL/ANSI extractor**, NOT `sqlglot` — it recognizes the shapes the consumers use, and everything it can't recognize is coverage-stat'd, not silently mis-parsed.

- **The three load-bearing disciplines, each proof-carried.** (1) **Stdlib-only / no sqlglot** — no 3rd-party parser import (an AST import-scan test bites); scripts-tier, `check_separation` clean at 26. (2) **Parse-coverage honesty** — the artifact carries `{parsed, skipped:[{object,reason}], failed:[{object,reason}]}`; a dialect-specific / malformed / undecodable object lands in skipped/failed WITH a reason, never silently dropped or mis-parsed, and the run never crashes. (3) **Corroboration is the gate** — mined field/metric candidates enter the dictionary at provenance `inference` (never stronger) and pass `scripts/data_dictionary/data_dictionary.py::corroborate_definition`; a mined claim conflicting with a corroborated definition is flagged ⚠ + downgraded, NOT accepted (a mutation proof: bypassing the gate reds a test). Mined read/write relationships become `hooks/lineage_graph.py` `data_asset` node + `reads`/`writes` edge evidence using ONLY existing kinds (`validate_lineage_graph == []`).
- **NEW `warehouse-sql-mining` skill** — the engine's contract (extraction surface, corroboration-gating rule, parse-coverage-honesty rule, CLI, the mining→dictionary/lineage/exploration wiring) → skills 51 → **52**.
- **Additive lane wiring** — `skills/data-eng-pipeline/SKILL.md` D0 gains a one-paragraph step: when SQL objects are in scope, invoke the miner + feed its evidence to `data-engineering-exploration` Stages 2–3; a data-eng run without SQL objects is unchanged.
- **The paired review earned its keep.** The first review returned FAIL from BOTH reviewers with **five findings** (two blocking): a crash on a UTF-16 `.sql` (SSMS's DEFAULT export encoding) that falsified the engine's own "never crashes" claim; a nested-block-comment mask that stopped at the first `*/` and leaked a phantom table into an UNGATED lineage edge; a substring aggregate check ("count" ⊂ "Discount") mis-parsing `Discount/Total` as a ratio; a frontmatter over-claim that no-context candidates "pass corroborate_definition"; and a stale demo. All five closed fix-forward (TDD red-first): comment-DEPTH tracking, a `(OSError, UnicodeDecodeError)` catch with `utf-8-sig` BOM tolerance, a word-boundary aggregate-call regex, and machine `not-accepted` markers on uncorroborated candidates with the skill claim corrected to the accurate conditional. Both reviewers re-verified to PASS (each confirmed the fixes bite by reverting them in-memory).

HONEST BOUNDARY: Run C ships the ENGINE + its corroboration-gated wiring. It is T-SQL-first (ANSI shapes covered; dialect exotica land in `skipped` with a reason). It mines SQL *text* (stored-procedure / view definitions) — it does NOT connect to a live warehouse (execution-stats mining, R4, is Run E). Nothing mined is presented as stronger than `inference` until corroborated. A logged non-blocking follow-up (R1): a narrow `corroborate_mined_claim` edge marks a candidate accepted when `corroborated_defs` is supplied but `rows=None` and the candidate isn't in the map — not a provenance bypass (stays `inference`), not CLI-reachable. The JSON-LD emitter (R6) remains Run F (deferred per D7); annotations (R3) Run D; usage-stats + review (R4/R5) Run E.

Tests: **1 new test file** (`test_sql_mining.py`, 29 tests) + fixtures under `tests/fixtures/sql_mining/` (incl. a committed UTF-16 SSMS-export fixture + nested-comment + Discount/Total regressions). Suite **6845 → 6878 passing + 6 skipped, 0 failed** (234 test files, Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1`; +33 tests, zero new failures); `check_separation` clean (checked 26, unaffected); `tests/test_dispatch_banner.py`'s version pin moved in lockstep with the bump. Skills 51 → **52**; agents / commands / hooks / Layer-3 tools UNCHANGED (39 / 25 / 7 / 21).

## [3.50.0] — 2026-08-03 — data-eng-lane (Run B: data-engineering becomes a first-class sibling lane at the front door)

**MINOR — additive: a NEW `skills/data-eng-pipeline/SKILL.md` sibling lane + a NEW `/architect-team:data-eng` command + a classifier-contract extension + additive routing; a non-data-eng ask behaves EXACTLY as before.** Run B of `docs/proposals/DATA_ENG_LANE_AND_CROSS_POLLINATION.md` §3/§7.2. Run A shipped the generic knowledge server (v3.49.0); Run B gives data-engineering the exact shape the `bug-fix` lane already has — a purpose-built lane a data-eng-primary ask routes to at Phase −2, rather than being handled as a generic feature. The lane COMPOSES what exists (`data-engineering-exploration` verbatim, `intake-and-mapping`, the Run A knowledge server, the `data_dictionary` engine) and adds exactly TWO new disciplines.

- **A fifth classifier verdict, moved honestly.** `agents/bug-classifier.md` gains `kind: data-eng` + a `data_eng_portion` field; the agent's own "exactly four kinds / five fields" pins MOVE to five/six in LOCKSTEP (the enum, the field list, all six examples, the hard rules, and the structural test `tests/test_bug_classifier_agent.py` — which gains an explicit half-moved-contract guard forbidding every stale four/five phrasing). A non-data-eng ask still classifies exactly as before — the fifth kind fires only on a data-eng-PRIMARY signal; plain bug/feature/mixed/unclear fall through untouched. The codebase-markers detection arm (which needs mapping output absent at −2 time) documents graceful deferral to Phase 0c.
- **`--data-eng` flag + `/architect-team:data-eng` command.** A third override bullet alongside `--bug-fix` / `--feature-only` (with natural-language equivalents), and a NEW `commands/data-eng.md` on the `commands/bug-fix.md` template invoking the lane by bare name → command count 24 → **25** (canonical-command pin + `COMMAND_TO_SKILLS` + the frozen fallback + `CAPABILITY_INDEX` all moved in lockstep; instruction-compliance zero findings, no `": "` in the description).
- **NEW `skills/data-eng-pipeline/SKILL.md` — the lane orchestrator (D−1…D8), reuse over duplication.** MemPalace wake-up precedes everything; **D0 dispatches `data-engineering-exploration` VERBATIM** (the lane becomes its THIRD documented caller — verified compose-not-fork: none of the exploration's seven `## Stage N` bodies appear in the lane); D1 = Phase 1 semantics; D2–D6 = Phases 2–6 verbatim (the full evidence stack + the data bar: every transformation ships ≥1 blocker-severity validation rule + lineage emission); D8 = Phase 8 verbatim.
- **The two new disciplines, wired to real Run A infra.** **D−1 warm-catalog-first**: query the knowledge server's `get_dictionary_status` (a real contract at `services/knowledge_server/contracts.py`) for the dictionary + its `{verdict, basis}` freshness BEFORE a rebuild — the server verdict INFORMS, the per-run gate DECIDES, and DB currency is honestly `unknowable` without a connection (carried from Run A). **D7 catalog-refresh**: rebuild the affected dictionary tables via `scripts/data_dictionary/data_dictionary.py` → re-corroborate → re-index the knowledge server → mine to MemPalace, leaving the catalog warm for the next run (with graceful no-server degradation pointing at `/architect-team:knowledge-install`).
- **Front-door-vs-mid-flow precedence, additive.** `skills/architect-team-pipeline/SKILL.md` gains a `kind: data-eng` routing bullet (mirroring `kind: bug`, incl. the low-confidence soft-route) + a `## Data-eng lane precedence` section: the lane WINS at the front door (a data-eng-primary run routes to `data-eng-pipeline` at Phase −2 and NEVER reaches Phase 0c); Phase 0c KEEPS winning mid-flow (a feature-primary run with a data-eng surface is byte-unchanged, its `## Phase 0c` heading preserved); `mixed` with a data-eng portion parallel-spawns with `triage_done: true` bounding recursion at depth 1. No existing routing bullet's semantics change for a non-data-eng run.

Both slices passed the evidence-schema-v7 paired review (independent task-reviewer + adversarial reviewer, producer != checker, `validate_evidence` 0 gaps on both). HONEST BOUNDARY: Run B ships the LANE (entry + orchestration + the two disciplines wired to Run A's server) — the heavy SQL-mining engine (R2) is Run C, annotations (R3) Run D, usage-stats + review round-trip (R4/R5) Run E, the JSON-LD emitter (R6) Run F. The lane is a documentation/orchestration surface (a skill + wiring), not a running data service — nothing is described as "deployed"; no new `services/` module, so `check_separation` is unaffected.

Tests: **2 new test files** (`test_data_eng_entry_surfaces.py` + `test_data_eng_pipeline_skill.py`) + the moved pins (bug-classifier five-kinds/six-fields + the half-moved guard, commands, canonical count, instruction-compliance files_checked 116 → 118, `test_skills` EXPECTED_SKILLS). Suite **6795 → 6845 passing + 6 skipped, 0 failed** (233 test files, Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1`; +50 tests, zero new failures); `check_separation` unaffected (no services module); `tests/test_dispatch_banner.py`'s version pin moved in lockstep with the bump. Skills 50 → **51**; commands 24 → **25**; agents / hooks / Layer-3 tools UNCHANGED (39 / 7 / 21).

## [3.49.0] — 2026-08-03 — knowledge-server-foundation (Run A: the generic standing, staleness-aware, MCP-queryable knowledge server both legs of the data-engineering proposal ride on)

**MINOR — additive: a NEW `services/knowledge_server/` server-tier member + a NEW `/architect-team:knowledge-install` command; ZERO behavior change to any existing surface.** Run A of `docs/proposals/DATA_ENG_LANE_AND_CROSS_POLLINATION.md` §7.1 — the shared foundation the lane's catalog steps and the map-server both depend on. The proposal's unifying thesis: leg-1's warm data-catalog and leg-2's map-server are ONE generic server pointed at two source families, built once. It adopts deng-toolkit's INTERFACE (warm, structured, one-tool-call, team-shared) while keeping CT6's SEMANTICS (change-driven freshness, corroborated content, per-run gates still run at consumption). Decisions taken per §8's marked recommendations (user-delegated via "deploy your build plan"): D2 per-repo-first, D3 a new `services/knowledge_server/` composing the Librarian's index, D4 hand-rolled stdlib JSON-RPC-over-stdio (no `mcp` SDK), D8 a separate install command.

- **NEW `services/knowledge_server/` — a stdlib-only MCP server** speaking JSON-RPC 2.0 over stdio (`initialize` / `tools/list` / `tools/call` hand-implemented, NO `mcp` SDK), import-clean so `services/separation.py::check_separation()` stays green (checked 26), no LLM anywhere (fully deterministic). A pluggable READ-ONLY source-adapter seam (`adapter_base.py::SourceAdapter`) registers tools from one or more sources; a 6-tool sweep mutates no source sidecar (sha256 before==after).
- **Two adapters shipped this run (NO scope cut).** `DictionarySource` over `docs/data-dictionary.json` (+ `docs/data-annotations/`) serving `search_dictionary` / `get_table_details` / `find_relations` / `get_dictionary_status`; `MapSource` over `lineage-graph.json` + the `*_MAP.md` frontmatter serving `search_map` / `get_route_details` / `find_call_paths` / `get_map_status` — `find_call_paths` the generalization of deng's `find_join_paths` over the lineage edge vocabulary (`calls` / `reads` / `writes` / `modifies` / `serves` / `originates` / `serves_route`), walking ONLY edges present in the graph (a poisoned/invented edge is refused).
- **Freshness on EVERY response — the deng anti-pattern refused.** Every tool response (not just the status tools) carries `{verdict: current|stale|unknowable, basis:[]}`: map staleness via `hooks/lineage_graph.py::transitive_stale_nodes` + the `last_mapped`/`last_traced` stamps + `map_invalidated`; dictionary via reference-file git-drift since `built_at`; DB-side currency honestly `unknowable` without a live connection — never a fabricated `current`, never a bare wall clock.
- **Composition over greenfield.** Ranked search composes the Librarian's `LibraryIndex` (concept×3/keyword×2/text×1) via the sibling-import bootstrap the daemon uses (verified by module identity — NOT a fork); re-index on a `bg_runtime.Scheduler` tick when a sidecar hash changes (bytes+mtime_ns); closed output contracts per tool via `scripts/mcp_design/output_contract.py` (the server eats CT6's own dog food — a malformed response FAILS validation through the real dispatch, 3-way falsifiable).
- **NEW `scripts/setup/install_knowledge_server.py`** on the `install_librarian.py` template: provisions `~/.architect-team/knowledge/`, GENERATES + PRINTS the MCP registration (never auto-edits the user's MCP config), `--check-only`/`--json`/`status`/`print-registration`/`uninstall --purge` parity, and a live-serving confirmation in the `confirm_gateway_serving` mold — the word "serving" is printed ONLY after a real `tools/call` round-trip against the actually-started server, described "serving on this machine", NEVER "deployed".
- **PORTABLE `--repo-root` registration.** The registration launches the server with `python <repo>/services/knowledge_server/server.py --repo-root <repo>` (no brittle hardcoded absolute paths); `server.py::discover_sidecars` auto-discovers the conventional CT6 sidecars (`docs/data-dictionary.json`, `lineage-graph.json`, `docs/*_MAP.md`, `docs/data-annotations/`) under that root, and explicit `--dictionary`/`--graph`/`--map` override/augment. At least one resolvable source is REQUIRED else a clean nonzero exit (no silent empty serve). Repo-root resolves `--repo-root` flag > recorded config > the git root of the install context.
- **The launch-dead-server class STRUCTURALLY refused (the whole point).** The entry point is exercised end-to-end at THREE layers — in-process `main()`, a suite subprocess co-exercise (`test_real_server_repo_root_serves` / `_via_full_default_path` spawn the ACTUAL `server.py` through the installer's REAL emitted argv), and the reviewers' own paste-simulation of the printed registration against the live server. The probe argv is DERIVED from the registration argv from ONE function (`_probe_argv == _server_argv + --max-messages`), so what the installer probes is byte-identical to what it ships — drift impossible by construction (`test_probe_and_registration_share_argv`). The paired review caught the seam first-hand: the installer's original `--base-dir` registration would have launched a DEAD server (the real server takes source sidecars); the test-fidelity root cause was an in-test fake that ignored unknown args — closed by `test_fake_server_rejects_unknown_args` (an argv drift back to `--base-dir` now goes RED), then the slice was retargeted red-first to the portable `--repo-root` form and independently re-verified live by both the task-reviewer and the adversary.
- **NEW `commands/knowledge-install.md`** (`/architect-team:knowledge-install`) on the librarian-install shape → command count 23 → **24**; the canonical-command pin (`tests/test_skill_invocation_audit_canonical.py`) + the directory-derived `COMMAND_TO_SKILLS` move in lockstep, and the detached-hook frozen fallback in `hooks/skill_invocation_audit.py` was refreshed to list librarian-install + knowledge-install.

Both slices passed the full evidence-schema-v7 paired review (independent task-reviewer + adversarial reviewer, producer != checker, `validate_evidence` 0 gaps on both), plus an independent Phase-7 master review (PASS — its own suite run, `check_separation`, evidence-gate re-check, and a live paste-simulation, having built none of the surface).

HONEST BOUNDARY: the installer confirms live serving on THIS machine via a real `tools/call`; nothing is described as "deployed". A bare `install` (no `--confirm`) in a repo lacking discoverable sidecars — the CT6 repo itself is such a repo — resolves repo-root to that git root and prints an accurate green (provisioned + registration generated, both true) whose registration is DORMANT until sidecars are generated; this is not a false serving claim (only `confirm-serving` makes the serving claim, and it honestly reports NOT-serving there), and the git-root-before-fixture precedence is the deliberately more-honest choice over silently serving the bundled DEMO fixture as a real catalog. A cheap static discoverability WARN on the default-install path is a LOGGED fast-follow, not shipped this run. Runs B–F of the proposal (the data-eng lane, warehouse SQL mining, data annotations, usage-stats + review round-trip, JSON-LD emitter) remain future runs.

Tests: **8 new test files** (`test_knowledge_server_core` / `_e2e` / `_contracts` / `_freshness` / `_dictionary_source` / `_map_source` / `_discovery` + `test_install_knowledge_server`, plus the `tests/fixtures/knowledge_server/` fixture set) + the moved count pins. Suite **6690 → 6795 passing + 6 skipped, 0 failed** (231 test files, Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1`; +105 tests — 100 from the 8 new files, +5 parametrized from the 24th command — zero new failures, measured first-hand by the orchestrator and re-verified by the independent Phase-7 master review); `check_separation` {clean:true, checked:26}; `tests/test_dispatch_banner.py`'s version pin moved in lockstep with the bump. Skills / agents / hooks / Layer-3-tool counts UNCHANGED (50 / 39 / 7 / 21); commands 23 → **24**; **NEW `services/knowledge_server/` server-tier member**.

## [3.48.1] — 2026-08-02 — harden-contract-first-parallelism (the foreign v3.48.0 release held to house standards after FAILING its first CT6 paired review)

**PATCH — a review-and-harden pass over the foreign v3.48.0 contract-first-parallelism release; NO new skill / agent / command / hook / Layer-3 tool, and the inventory holds at 50 / 39 / 23 / 7 / 21.** v3.48.0 merged from a foreign (non-CT6) session and its first CT6 paired review returned **FAIL from both reviewers** — the adversary and the independent reviewer converging FIRST-HAND, before reading each other, on the same central defect: the retirement ledger's reader was fail-OPEN. A hand-typed `mock_serving` state, a `{"entries": [...]}` wrapper, or any unrecognized entry shape read as an EMPTY ledger, so the CFP-6 close-out gate reported green — with an all-zero summary indistinguishable from a clean run — while the ledger carried an unretired mock; and the "non-closable" claim repeated across four docs was enforced by nothing. This release closes all **13 findings** (9 adversarial breaks B1–B9 + 4 independent IR-1…IR-4) fix-forward in `scripts/contract/interface_contract.py` + `skills/contract-first-parallelism/SKILL.md` + `tests/test_contract_first_parallelism.py`, with finding-forced edits to `agents/backend.md`, `agents/system-architect.md`, both pipeline bodies, `tests/test_evidence_integrity_instruction_surfaces.py`, and `CLAUDE.md` — nothing reverted, no finding resolved by weakening its claim — and both fix-pass verdicts are **PASS**.

- **The two CRITICAL ledger fail-opens made fail-CLOSED as a module invariant (B1/B2/B7).** New `normalize_ledger` centralizes every ledger read: an unrecognized state, a non-mapping entry, and an unknown container shape each BLOCK with a named reason (`unknown-ledger-state` / `unrecognized-ledger-shape`) instead of reading as an empty ledger; `ledger_summary` gains an `unknown` bucket so its counts always sum to the ledger, and an unreadable ledger prints `ledger: UNREADABLE`, never zeros. An ABSENT ledger is the deliberate exception — an explicit clean no-op (`no contracts recorded`, exit 0), so a run that created no contracts is never blocked.
- **The "non-closable" claim wired into real machinery (B9).** New `declared_gate_entry` / `declare_gate` / `satisfy_gate` write one `cfp-retirement-<contract-id>` entry per contract into the EXISTING v3.47.0 `.architect-team/declared-gates.json` registry, so the EXISTING Stop-hook `_audit_declared_gates` arm is what makes a run with an unretired mock non-closable — `hooks/pipeline-completion-audit.py` byte-UNCHANGED (the fix-pass reviewer drove the declare → audit-blocks → satisfy → audit-clean loop through the real arm). Registration is wired at backend provisioning (CFP-4), `architect-team-pipeline` Phase 2 + the Phase 8 satisfy-after-gate step, and mini M4/M7; the skill's `## Honest boundary` states plainly that a contract provisioned without `declare-gate` is a gate nothing enforces.
- **The advertised marker check made real (B4).** The skill claimed a provisional-marker check no code performed. `contract_drift(..., retirement=True)` — CLI `drift --retirement` — now REFUSES a payload still carrying its provisional marker at ANY depth (`items[]._mock`, named by path, error-severity `provisional-marker-still-served`); the same payload WITHOUT `--retirement` stays advisory, preserving the CFP-5 mid-build reading. `validate_contract` additionally refuses a contract that DECLARES its own provisional marker as a response field.
- **Correctness + nested-type binding (B3/B6).** A top-level JSON-array observed payload draws the clean named error `observed-payload-not-an-object` instead of a `TypeError` traceback; `array` / `object` response fields declare element / member shapes under `items` / `fields` (the same list-of-declarations form as `response_fields`), validated recursively and drift-adjudicated by payload path (`items[].total`, `meta.count`); an unshaped container validates as a spoken advisory, and the `system-architect` approval mode is instructed to usually refuse it.
- **Tests that could not fail, fixed (B5).** Three adversary mutations SURVIVED the foreign release's 41 greens — `REQUIRED_CONTRACT_FIELDS` gutted 8 → 1, `_HTTP_METHODS` narrowed, the contract doc's New column inverted — all three now killed (re-executed in fresh scratch mutants: 8 / 3 / 1 tests fail respectively, unmutated baseline green). The file grew **41 → 149 cases** red-first under both encodings, and a valid `verify-check-can-fail` verdict now covers the release's test file — the foreign branch shipped none.
- **The CFP mock reconciled with `verify-no-fake-data` (B8) — in the STRICTER direction.** No exemption was carved: the provisional handler is production backend code, so the v3.47.0 sweep applies to it in full and `Jane Doe` / lorem-ipsum / `$1,234` are correctly flagged in a backend diff. The skill + `agents/backend.md` now direct plausible domain-vocabulary values and name the verifier's REAL categories, test-pinned against `hooks/vao/fake_data.py::_FAKE_DATA_PATTERNS` — `fake_data.py` itself byte-UNCHANGED.
- **The independent findings (IR-1…IR-4).** The v3.48.0 entry's PRE-EXISTING-RED note corrected in place to the composed-HEAD truth (the cited 5 `test_report_claims_citation.py` reds are not reproducible from this checkout — the file measures 124 passed / 0 failed; the correction is dated and names its measurements); `docs/CODEBASE_MAP.md`'s stale current-state lines refreshed; the two Phase 8 sections no longer both claim to run first; the split ordered list + stray blank line fixed. The evidence-integrity instruction-surface pin was WIDENED to scan every `declared-gates.json` mention rather than only the first — proven necessary (the old logic goes red on the correct body, since CFP legitimately names the registry at provisioning, before the ship step) AND still-biting (stripping every `satisf` token turns it red).
- **Bump-lockstep.** `plugin.json` + `marketplace.json` → 3.48.1 with `tests/test_dispatch_banner.py`'s version pin moved in the same edit; this release's documentation-currency pass closed every remaining version-bearing surface (README banner + version badge + tests badge + spotlight + timeline, the history's new current section, both maps' ledgers + stamps, `CLAUDE.md`).

HONEST BOUNDARY: v3.48.0 itself is NOT rewritten — it shipped, and its entry above stands (with only the IR-1 red-note correction, dated in place); this release hardens what it shipped. The declared-gates registry write remains instruction-invoked (an agent that skips `declare-gate` leaves a gate nothing enforces — stated in the skill's own honest boundary), and the engine still cannot prove a captured payload came from the live endpoint (the observed payload is injected by the agent that hit it).

Tests: **0 new test files** — `tests/test_contract_first_parallelism.py` **41 → 149** (+108: the fail-closed ledger matrix, the retirement-reading drift cases, nested-binding positives + negatives, the declared-gates round trip, the fake-data-category cross-pin, and the mutation-killing constant-set / method-set / doc-renderer pins) + the widened `tests/test_evidence_integrity_instruction_surfaces.py` pin. Suite **6582 → 6690 passing + 6 skipped, 0 failed** (223 test files, Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1` — measured first-hand by the fix-pass reviewer's full run and re-verified by the orchestrator's authoritative post-bump capture). Skill / agent / command / hook / Layer-3-tool counts UNCHANGED (50 / 39 / 23 / 7 / 21); **NO new skill / agent / command / hook / Layer-3 tool**.

## [3.48.0] — 2026-08-01 — contract-first parallelism (the frontend stops waiting on the backend's build)

**MINOR — the architect's parallelism protocol for full-stack work.** The owner directive (verbatim intent): when developing full-stack apps from scratch or integrating with existing services, if backend services must be updated and the frontend needs the inbound surfaces to properly visualize the data, the frontend and backend agents coordinate on inbound-surface needs and the architect approves; once defined the backend IMMEDIATELY provisions the endpoint along with a mock data payload, so the frontend can integrate with this live endpoint while the backend fixes/modifies anything under the hood — the abstraction lets the frontend build a fully integrated interface instead of waiting for the backend to finish, maximizing parallelism; the same theory holds when the surface exists but needs new attributes. The goal is a working connect point first, then development completed around it.

**Why the old flow serialized.** The frontend never needed the backend's IMPLEMENTATION — only its INTERFACE, and those become available at very different times. Pre-v3.48.0 the frontend hit a missing endpoint, authored a `missing-api-for-frontend-element` SR, PAUSED that element, and waited a full backend round trip (design → implement → test → report → re-dispatch) before wiring. It was only ever waiting on the *shape*.

**The protocol — new 50th skill `contract-first-parallelism` (CFP-1…CFP-6).** CFP-1 **detect** at Phase 2 for every `both`-layer slice needing surfaces — two shapes, one protocol: `new-surface` (the endpoint does not exist) and `amend-surface` (it exists but lacks attributes the UI must render; NOT a lesser case — a frontend blocked on three fields is as blocked as one waiting on a whole endpoint). CFP-2 **co-design**: the frontend states what it must render (fields + meanings + empty/loading/error states), the backend states what is authoritative and feasible — a conversation with a written artifact, not a handoff. CFP-3 **architect approval** binds the contract; neither side changes it unilaterally, amendments return through the architect who relays them. CFP-4 **immediate provisioning**: the backend's FIRST action with the approved contract is the endpoint AT ITS REAL PATH (never a temporary path or separate mock server — re-pointing later is the rework the protocol eliminates), serving a contract-conforming payload with every declared error state reachable, marked provisional and registered in the ledger. CFP-5 **parallel build**: the frontend builds its COMPLETE integration against the live surface while the backend replaces the internals underneath, free to change anything the contract does not name. CFP-6 **retirement**, mandatory before close.

**Why this is not the forbidden fake-data pattern.** The provisional payload lives SERVER-SIDE behind the real path, so the frontend holds no fixture, no `page.route` intercept, and no hardcoded shape — it performs real HTTP against a real endpoint and renders what comes back, which is strictly MORE live-wired than the alternative. The four forbidden frontend-side anti-patterns are unchanged; `## Frontend missing-API discipline` is narrowed to the surface the architect has NOT contracted (and its SR is the cheapest entry point INTO the protocol, since it already documents the shape).

**The debt is always repaid.** Every provisioned mock is tracked in a forward-only ledger (`proposed → approved → mock-serving → live`; a live surface never reverts to serving its mock). The Phase 8 / M7 gate makes the run NON-CLOSABLE while any surface is still `mock-serving` — "retire it next run" is precisely the end-of-run deferral the existing discipline forbids, and a mock that outlives its run is the debt the no-fake-data disciplines exist to prevent. Retirement is verified end-to-end through the SAME live path the frontend already calls; the frontend needing no change at retirement is the proof the abstraction held.

**The engine — `scripts/contract/interface_contract.py`** (stdlib-only, no import-time side effects): `validate_contract` (the approval gate — an untyped response field, a missing error-state set, an un-rooted path, a duplicate field, or an `amend-surface` that does not name its added attributes all BLOCK; `any`-typed fields are advisory), `build_contract_doc`, `transition` (forward-only; backward raises), `unretired_mocks` / `retirement_gate` / `ledger_summary`, `contract_drift` (approved shape vs an observed payload: missing and mistyped fields are errors, undeclared extras advisory, integer satisfies number), and the `validate` / `doc` / `gate` / `drift` CLI. HONEST BOUNDARY: it never performs HTTP — the observed payload is supplied by the agent that actually hit the endpoint.

**Wiring.** `architect-team-pipeline` Phase 2 engages the protocol BEFORE teams spawn (so surfaces are provisioned as the teams start, not after they block) and Phase 8 runs the retirement gate before the doc-currency gate and the commit; `mini-architect-team-pipeline` mirrors it at M4/M7; `system-architect` gains a TENTH mode (`## Interface Contract Approval` — engine gate first, then reuse-first judgment: prefer amending a real endpoint over minting a new one); `agents/backend.md` gains `## Contract-first provisioning` (provision first, build second, retire before close); `agents/frontend.md` gains `## Contract-first parallelism` (build against live, hold no fallback of your own, never adapt around a drift); `common-pipeline-conventions` carries the canonical statement.

Tests: new `tests/test_contract_first_parallelism.py` (41 — the approval gate per blocker kind, the forward-only state machine incl. the refused backward transition and input non-mutation, the retirement gate across every unretired state + the empty-ledger no-op + the live-without-evidence advisory, drift adjudication, the four CLI verbs, and the wiring pins for both pipelines / both implementer agents / the architect mode / the CPC section / the narrowed missing-API scope). `tests/test_skills.py` `EXPECTED_SKILLS` + `tests/test_instruction_compliance.py` in-scope 114 → 115; `docs/CAPABILITY_INDEX.md` regenerated (50 / 23 / 39); the `tests/test_skill_references.py` CPC byte pin advanced 256080 → 258442 with its rationale recorded (a NEW cross-pipeline discipline, which the extraction mission's own "keep every gate/discipline inline" rule requires to stay in the body — its full protocol lives in the skill). Suite **6537 → 6580 passing + 3 skipped** (223 test files, Linux-measured; the 5 pre-existing `test_report_claims_citation.py` reds noted below are excluded from the passing figure and unchanged by this release). Skill count **49 → 50**; agents / commands / hooks / Layer-3 tools UNCHANGED (39 / 23 / 7 / 21); **NO new agent / command / hook / Layer-3 tool**.

PRE-EXISTING RED, not introduced here: 5 failures in `tests/test_report_claims_citation.py` were reported against this branch's own working state. CORRECTED ON THE COMPOSED HEAD (post-merge review, 2026-08-02): the file is GREEN — `python -m pytest tests/test_report_claims_citation.py -q` measures 124 passed, 0 failed, and `git diff 13f7a6b..HEAD` over that file and `hooks/` is empty, so the cited reds are not reproducible from this checkout at either endpoint and no reader of HEAD should hunt for them. The observation was local to the foreign session's environment; the release itself neither introduced nor inherited a red.

## [3.47.1] — 2026-08-01 — compact-readme-release-history (the README goes compact; the complete release narrative moves to docs/RELEASE_HISTORY.md under a pinned append-and-swap convention)

**PATCH — a docs-only restructure; NO code / skill / agent / command / hook / Layer-3-tool change, and the inventory holds at 49 / 39 / 23 / 7 / 21.** The README had accumulated ~610 lines of per-release "NEW IN" narrative — 55 distinct release identities across 2 regions — above its durable content, reaching 1785 lines; every release appended another section. This release moves the narrative out, compacts the README to 1182 lines, and institutionalizes the convention that keeps it compact.

- **The move.** The full release narrative moved BYTE-IDENTICAL to the NEW `docs/RELEASE_HISTORY.md`, verified by a three-verdict extraction check with negative controls: release identities named by the pre-change README **55** / reachable after the move (README + history) **55** / lost **NONE**. The convention is COMPLETE history including the current release — each release APPENDS its section to the history and SWAPS the README's single spotlight, so the current release appears in both places (this release's own section is byte-identical in both, mechanically extracted from the README at append time).
- **The compact README.** 1785 → 1182 lines (the restructure landed at 1184; this release's own spotlight swap shed 2 more): one swapped-per-release NEW-IN spotlight, the house-style RELEASE HISTORY pointer block where the narrative began, every durable region untouched, the ASCII banner fixed, and `### Bumping versions` gains **step 5** documenting append-and-swap.
- **The pins — NEW `tests/test_release_history.py` (13 tests).** Adversarially hardened after a 4-break review loop (pins that could not fail were made falsifiable; the extraction generator made atomic + idempotent): exactly-one-spotlight; the spotlight version DERIVED from `plugin.json`, never hardcoded — a bump that misses the swap goes red rather than stale; the pointer pin anchored to the block's STRUCTURE (break B2: a bare substring check passed with the entire pointer block deleted, because the Bumping-versions step also names the path); a 55-release-identity completeness floor across all three naming grammars (break B1: a history gutted from 157 KB to ~6 KB passed the earliest-anchor and current-version pins); and standing mutation tests that prove each pin bites against the real shipped files.
- **The bump-lockstep lesson, institutionalized.** The 3.47.1 version bump moved `tests/test_dispatch_banner.py`'s version pin in the SAME edit as the two plugin JSONs — closing, in-release, the inherited red the post-restructure suite capture surfaced — and this release's doc-currency pass closed every remaining version-bearing surface (README banner + version badge + tests badge + spotlight, the history's current-release section, both maps' stamps, `CLAUDE.md`) in one sweep. The readme-styling and release-history pins all derive from `plugin.json`, so this failure class now surfaces as a red test, not silent staleness.

Tests: **1 new test file** (`tests/test_release_history.py` — 13 tests: 9 convention pins + 4 pin-bites mutation tests). The run's post-restructure suite capture measured **6536 passing + 6 skipped, 1 failed** — the sole red being `test_dispatch_banner.py`'s version pin against the just-bumped JSONs, closed by the lockstep pin move above. Post-bump, with every doc surface current: Suite **6524 → 6537 passing + 6 skipped, 0 failed** (222 test files by the top-level `tests/test_*.py` disk-count basis; 6543 collected; Windows-measured — the macOS-without-PyYAML basis, 5966 + 16 at v3.46.0, remains unmeasured since). Skill / agent / command / hook / Layer-3-tool counts UNCHANGED (49 / 39 / 23 / 7 / 21); **NO new skill / agent / command / hook / Layer-3 tool**.

## [3.47.0] — 2026-07-31 — harden-evidence-integrity (a check is not evidence until it has been shown able to fail)

**MINOR — the banking-app postmortem's nine prevention rules become structural enforcement: the NEW 21st Layer-3 tool `verify-check-can-fail`, three new Stop-audit arms, the active-run unmanifested-task completion gate, five report-claims citation severities, mid-run spec fingerprinting, and the evidence-integrity ETHOS — and the Windows suite reaches ZERO failures for the first time since the v3.42.x-era macOS merges.** The trigger (2026-07-30): a CT6-orchestrated run (the banking-app FDS fix-list release — 2 docs, 26 items, 9 agents, 3 deploys) approved and reported fixes that were not real — an inert column-config feature, a typecheck gate that examined zero files, "deployed and verified" claimed off status codes, task-board `completed` statuses repeated to the user while a route white-screened for every persona, and absence findings manufactured from narrow greps. The postmortem distilled nine prevention rules; the gap analysis mapped them against v3.46.0 with file:line evidence and found two rules unenforced, one unenforced for flows, and six with named structural holes. The mandate: make this failure class structurally impossible, not exhortatively discouraged.

- **The 21st Layer-3 tool — `verify-check-can-fail` (`hooks/vao/check_integrity.py` + the facade/CLI), forged through SIX adversarial rounds.** A cited verification-command output must prove it could have failed. The zero-work signature registry uses anchored / count-aware signature kinds + repo-state predicates — pytest `collected 0 items`, Playwright / jest / vitest no-tests-found, a `tsc --noEmit` aimed at a solution-shaped tsconfig — never raw substrings, so a green run that *quotes* a signature, or a test whose *name* contains `failed`, cannot false-positive. Red-run-first proof binds every diff-added test file to one of the three acceptable red sources (`tdd-red` before implementation / `pre-change-checkout` / `assertion-inversion`), with red-output correlation (`output-does-not-reference-test`, `shared-anonymous-red`, excerpt-required-when-indeterminate). Scanning is REGION-AWARE: a `reporting_region` partitions runner reporting from relayed text, the terminal verdict wins with an aggregated trailing block, an ambiguous multi-run output is refused rather than guessed, and a degraded decode basis abstains rather than convicts. A decode ladder (BOM sniff + NUL-ratio UTF-16 handling) keeps a UTF-16LE `collected 0 items` from hiding inside a UTF-8 read; typecheck-intent gating exempts `tsc -b` build-mode and emits the `typecheck-tsconfig-indeterminate` note instead of overclaiming. Severities: `vacuous-check` / `new-guard-never-shown-red` / `red-run-not-red`.
- **The evidence schema + the review gate (`hooks/review_evidence_schema.py` + `hooks/review-gate-task.py`).** Optional `check_integrity_review` joins the contract — `OPTIONAL_VAO_FIELDS` goes 3 → **4** on the established v2.1.0 / v2.2.0 / v3.14.0 additive-optional pattern (the 17 required fields untouched; every existing evidence file still validates). The schema module also gains `normalize_task_id`, isinstance guards, and a fail-closed crash wrapper via the gate hook. And the **unmanifested-task completion hole is closed**: during an ACTIVE run, the orchestrator session's `TaskUpdate(completed)` / `TaskCompleted` on a task registered in NO teammate manifest (`expected_review_evidence` / `shared_task_id` / `task_ids`) is BLOCKED with remediation — scoped to the run's own session (teams mode shares ONE session id, recorded at engagement by `hooks/run_continuity.py --engage [--session-id]`, primary env `CLAUDE_CODE_SESSION_ID` with the `CT6_SESSION_ID` operator override ahead of it), kill-switch `CT6_TASK_GATE_DISABLED`, fail-open outside active runs, skip-with-warn on a non-dict manifest, and path-shaped evidence ids flagged as unusable entries rather than silently matched.
- **Three new Stop-audit arms (`hooks/pipeline-completion-audit.py`).** `_audit_check_integrity` — a diff that adds tests requires EVERY check-can-fail verdict in the verdict directory to pass (latest-wins removed; notes surface informationally). `_audit_declared_gates` — a gate the orchestrator NAMES is a gate it records in `.architect-team/declared-gates.json`, and an unsatisfied recorded gate blocks completion QUOTING its `declaration_text`, with containment + non-whitespace evidence required to satisfy it. `_audit_spec_currency` — a SHA-256 spec fingerprint (NEW `hooks/spec_fingerprint.py`: `compute_spec_fingerprint` over the sorted posix-relpath + content of the active change dir) is stamped into every teammate manifest at dispatch; an in-flight teammate briefed against a superseded spec with no re-brief record (explicit `teammate` field; token-boundary attribution) is flagged, and `_audit_manifest_id_hygiene` rides the same arm. New SR `origin.kind: spec-drift` (direct-dispatch — deliberately NOT a `TEST_FAILURE_ORIGINS` member), and the code-wins-spec-amended discipline is now written text.
- **Report-claims citation (`hooks/vao/deferral.py` + NEW `deferral_b.py` + NEW `mention_context.py`).** Five claims-citation severities over `final_report` + `progress_reports[]` — `uncited-completion-claim`, `uncited-deploy-claim`, `absence-claim-uncited`, `stalled-agent-claim-uncited`, `undeclared-gate-language` — with an extended citation-token vocabulary, a per-occurrence mention-vs-use guard (quote-enclosure AND an attribution cue AND (prose context OR an own-family id); forward-only clause-scoped citations bounded by neighbor claims), and stated-boundary handling (B-8 token overlap; report-surface token-form citations). The module ceiling is honored by two splits: `deferral.py` 513 / `deferral_b.py` 846 / `mention_context.py` 174 / `core.py` 185 lines (the `hooks/vao/` package is now **14 modules** + `__init__`). `scripts/delivery/delivery_manifest.py` gains ERROR-severity citation findings — `uncited-verified-claim` (per-step 'FOR IT' scoping) + `uncited-element-claim`, with junk evidence values refused: evidence must NAME an artifact.
- **The instruction tier.** `docs/ETHOS.md` gains `## Evidence integrity (v3.47.0)` — *the grep absence* (grep proves presence, never absence; an absence claim requires an executed enumeration: the runner's collected list, the owner's answer, or the catalog), *the silence conversion* (an unreported agent is in-flight, not stalled; exactly three knowable states), *the relayed claim* (relay claims as claims, verdicts as facts, naming the verdict) — with the principle-7 clause recompiled across all 39 agents + the 5 pipeline skills via the existing compilers. `team-spawning-and-review-gates` (the `spec_fingerprint` manifest field, the `spec-drift` SR, red-first's three sources, `## Reading teammate state`, 4 `OPTIONAL_VAO_FIELDS`), `verified-agent-output` (the 21st tool fully described post-round-6 + the `check_integrity_review` citation contract), `dev-api-integration-testing` (the API read-back chain POST-echo → GET → PUT-echo → GET-after-PUT; the DB side-effect demoted to necessary-not-sufficient), `playwright-user-flows` (shared-state hygiene: unique-or-restore, every flow runnable twice, never assert a literal the test itself wrote), `common-pipeline-conventions` (`## Declared-gates discipline (v3.47.0)` + `## Spec currency discipline` + `gate_id` guidance + verdict hygiene), `delivery-manifest` (the citation bar + its honest boundary), the 4 pipeline bodies' declared-gates ship checks, and `task-reviewer` + `test-completeness-verifier` (capture-and-scan, the `readback_audit` finding, fixture-hygiene step 3g).
- **The adversarial loop, honestly.** All four implementation lanes' FIRST passes FAILED adversarial review; ~20 review rounds ran in total; six spec disagreements resolved code-wins with the spec amended in lockstep; five run-protocol lessons were recorded for the next run. The two long-standing Windows baseline failures are CLOSED — one was a real product bug (`scripts/setup/guidance_blocks.py::remove_block` accreted a blank line per uninstall on CRLF files — fixed with the CRLF-aware boundary-newline repair), and one was a falsifiability repair (the `test_skill_references.py` byte pin re-derived LF-normalized so it measures content, not line endings).

**Honest boundary:** the citation gate binds PERSISTED report artifacts (`final_report`, `progress_reports[]`, manifests, verdict files) — mid-run chat remains instruction-governed, not hook-verified. The declared-gates and spec-currency arms fail open when their registries are absent, so foreign workflows and non-CT6 repos are untouched.

Tests: **7 new test files** — `test_vao_check_can_fail.py`, `test_completion_status_integrity.py`, `test_declared_gates_audit.py`, `test_spec_currency_audit.py`, `test_spec_fingerprint.py`, `test_report_claims_citation.py`, `test_evidence_integrity_instruction_surfaces.py` — plus extensions across the review-gate / deferral / delivery-manifest / ethos-injection / facade-reexport / verifier families and every 20 → 21 count pin (the facade identity-check now covers 169 re-exports, 21 of them `verify_*` functions). Suite **5979 → 6524 passing + 6 skipped, 0 failed** (221 test files) on the **Windows-measured basis** — the first zero-failure Windows run since the two v3.42.x-era environmental failures appeared; the macOS-without-PyYAML basis (5966 + 16 at v3.46.0) was not re-measured this release. Living capability specs **68 → 76** (8 new: `check-falsifiability`, `declared-gates-registry`, `completion-status-integrity`, `report-claims-citation`, `spec-currency-mid-run`, `api-readback-discipline`, `flow-fixture-hygiene`, `evidence-integrity-ethos`; `verified-agent-output` amended). Skill / agent / command / hook counts UNCHANGED (49 / 39 / 23 / 7); Layer-3 tools **20 → 21**; `OPTIONAL_VAO_FIELDS` **3 → 4**; **NO new skill / agent / command / hook**.

## [3.46.0] — 2026-07-26 — delivery-manifest (the end-of-run bill of sale)

**MINOR — every completed run now ends with a DELIVERY MANIFEST: a stakeholder-readable bill of sale, email-ready.** The owner directive (verbatim intent): at the end of any completion, create a manifest — what was the problem solved in basic speak, and what testing criteria someone can use to validate; if it is a new feature, outline the locations of all the new elements and the new functionality by element released; the goal is a fully documented bill of sale that can simply be copied into an email and deployed; and if documents are provided, the agent attempts to match the vocab and layout.

**The skill — `skills/delivery-manifest/SKILL.md` (the 49th).** Fires automatically at every pipeline close-out (Phase 8 / B8 / M7 / U9, after `--mark-complete`, before `run_complete`) and on demand for any finished work. The three pillars every manifest MUST carry: (1) the problem in PLAIN SPEAK — no paths, no code markup, written for the person paying for the work; (2) testing criteria a person outside the build team can execute, EVERY step with an explicit expected result; (3) for a feature, a table locating every new element (location / name / what it does). **Template matching:** if the user provided an example document (a prior delivery note, a company template), the manifest matches its VOCABULARY AND LAYOUT — same section names, ordering, tone — with the three pillars mapped into the template's own sections and the matched source recorded as `template_source`.

**The engine — `scripts/delivery/delivery_manifest.py` (stdlib-only, no import-time side effects, the house engine shape).** `validate_manifest` is the publishing gate: required fields per delivery type (`feature`/`bug-fix`/`improvement`/`docs`/`infrastructure`), at least one validation step and per-step `expected`, per-element `location`+`name`+`functionality` for features, and ZERO unresolved placeholders (TBD/TODO/FIXME/…) anywhere — all error-severity blockers; `jargon-in-plain-speak` (path/code markup inside the plain-speak section) and `element-location-unresolved` (a repo-path-shaped location that does not resolve; URLs never checked) are advisories. `build_manifest` serializes the DEFAULT layout; `validate_text` gates a TEMPLATE-SHAPED rendering (substance + zero placeholders — layout/vocab matching is LLM judgment, the honest boundary); `render_email` returns the copy-paste `{subject, body}`; `build`/`validate` CLI.

**The wiring — all FOUR pipelines.** Each close-out gains the manifest step (assemble the data JSON → engine-validate to zero errors → build to `<workspace>/.architect-team/delivery/<slug>-manifest.md`) and each `run_complete` notifier invocation now embeds the manifest into the run's FINAL email via `--plan-file` (both polyglot halves) — the recipient's inbox carries the complete bill of sale the moment the run closes.

Tests: new `tests/test_delivery_manifest.py` (27 — the gate's per-kind blockers + advisories, default-layout pillars, template-source passthrough, `validate_text`, email render, CLI exit codes, the four-pipeline wiring pins incl. the `--plan-file` embed on every `run_complete` half, and the skill-contract anchors); `tests/test_skills.py` `EXPECTED_SKILLS` + `tests/test_instruction_compliance.py` in-scope count 113 → 114. Suite **5939 → 5966 passing + 16 skipped** (214 test files) on the recorded macOS-without-PyYAML basis. Skill count **48 → 49**; agents 39 / commands 23 UNCHANGED; NO new agent / command / hook / Layer-3 tool.

## [3.45.0] — 2026-07-25 — opus-5-model-upgrade (the plugin's named Opus generation becomes Claude Opus 5)

**MINOR — a model-policy + documentation-currency change; NO new skill / agent / command / hook / Layer-3 tool, and the inventory holds at 48 / 39 / 23 / 7 / 20.** Claude Opus 5 shipped. The v3.43.0 delivery-adversarial split already ran 12 agents on Opus, but every version-bearing Opus reference in the plugin still named **Opus 4.8** or **4.7** — two generations behind — including a hard-coded dated id in the service tier and a gateway route list with no Opus 5 entry.

- **The real id, confirmed from the platform docs, is `claude-opus-5`** — simultaneously the Claude API ID *and* alias under the 4.6-generation-and-later dateless scheme `claude-{name}-{major}[-{minor}]`, major releases omitting the minor segment. There is no `claude-opus-5-0`, no dated `claude-opus-5-YYYYMMDD`, no `-v1` suffix. Bedrock `anthropic.claude-opus-5`; Google Cloud `claude-opus-5`. Two facts drove design decisions rather than merely informing them: **a dateless id is a pinned snapshot, not an evergreen pointer** (the opposite of pre-4.6 aliases like `claude-sonnet-4-5`), and **`claude-fable-5` remains the documented most-capable widely-released model** — so Opus 5's arrival does not argue for dissolving the Fable-plans / Opus-delivers partition.
- **Agent frontmatter is deliberately UNCHANGED.** All 39 `agents/*.md` stay byte-identical (12 `model: opus`, 27 `model: fable`); the 12 delivery + adversarial agents inherit Opus 5 through the floating bare harness alias with **zero file edits**. Pinning `claude-opus-5` was considered and rejected: because the dateless id is a pinned snapshot, pinning would trade the alias's auto-follow for reproducibility this instruction surface does not need, and would drag `VALID_MODELS`, the lever's `OPUS_MODEL`, `apply_delivery_split`, and 10 lever tests along with it. `agents/` + `tests/test_agents.py` served as the run's **negative control**, verified untouched four independent ways.
- **Service-tier fallback** (`services/common/service_config.py`): `FALLBACK_MODEL` → `claude-opus-5`, with `resolve_model`'s docstring prose moved in the same edit (constant and docstring can no longer drift apart), the living `fable-default-setup-fixes` requirement + scenario amended in lockstep so the `openspec validate --all --strict` Stop-hook gate stays green, and an in-code record of a trade-off the owner was shown and overrode: Opus 5 (2026-07) is **newer** than Fable 5 (GA 2026-06-09), so a harness too old to resolve the `fable` alias is certainly too old to resolve `claude-opus-5` — this fallback is strictly less likely to resolve than the primary it backs. Shipped as directed; substituting a "safer" laddered or unchanged value would be **invented caution** per `docs/ETHOS.md` `## Fidelity to human-configured policy`.
- **Gateway routing** (`scripts/setup/install_gateway.py`): `claude-opus-5` added to `ANTHROPIC_EXPLICIT_MODELS` (necessary because the `*` catch-all was observed non-functional on a real LiteLLM install — SR-gateway-wildcard-route), every legacy id **retained** (the tuple is an allow-list of routable ids, not a currency statement), and the provenance comment **split by source**: the dated `/v1/models on 2026-07-16` listing claim now covers only the ids actually observed then, stating outright that Opus 5 did not exist on that date.
- **7 commit trailers** across 4 `commands/*.md` + 3 `skills/*/SKILL.md`: `Claude Opus 4.7 (1M context)` → `Claude Opus 5 (1M context)`, preserving the literal `Co-Authored-By: Claude Opus ` prefix that `tests/test_dispatch_banner.py` uses as its trailer-block anchor.
- **Current-state docs corrected, release history frozen.** 6 current-state assertions updated (`CLAUDE.md` ×2, `docs/CODEBASE_MAP.md` ×2, `docs/INTEGRATION_MAP.md` — which was doubly stale and still described uniform `model: fable` "as of v3.32.0" — and the README badge); 6 historical per-release lines left **byte-identical**, because past releases genuinely shipped on Opus 4.8 and rewriting a release section would falsify the record. The discriminator is tense and function, not file or proximity: `CLAUDE.md:9` moved while `:60` froze; `docs/CODEBASE_MAP.md:293`/`:360` moved while `:36` froze.
- **A v3.43.0 wording-debt sweep folded in** (a disclosed scope extension, adjacent to the Opus mandate): ~13 lines across `scripts/setup/set_default_model.py`, `scripts/setup/setup.py`, `commands/architect-team-setup.md` (user-facing), the two renamed lever tests, and the living `codex-role-split` spec asserted that the plugin runs "uniform `fable` on all 39 agents" or that uniform-fable IS "the current operating model" — false since v3.43.0, and invisible to any Opus-version sweep because they carry no version string. Corrected; behavior untouched. Descriptions of what an action *does* ("`--no-codex` applies the uniform fable policy") were verified accurate and preserved, so the sweep did not over-reach into true statements.

Scope was held to a **countable contract** rather than reviewer judgment: a case-insensitive, hyphen-tolerant sweep returning exactly 36 pre-change lines, partitioned three ways — **27 updated / 3 retained-unchanged / 6 frozen** — so 9 of the original 36 still match, plus **5 lines this change introduces**, reconciling in both directions to exactly **14** post-change matches, each identified by artifact + identity rather than by line number. Three of the 5 introduced are these release notes themselves (the README v3.45.0 section ×2 and the `CLAUDE.md` digest), which necessarily name the generation they describe removing; the other two are an OpenSpec scenario quoting the forbidden strings as the subject of a negative assertion, and the comment explaining why the legacy gateway routes are deliberately retained.

**Honest boundary:** this release does NOT verify that a live Anthropic endpoint answers to `claude-opus-5` from a given machine, nor that the Opus 5 gateway route works against a real running LiteLLM install — both require a keyed live gateway this run did not provision. The id is confirmed from the published platform docs; the route is verified as present in the emitted gateway config, not as serving.

Tests: **0 new test functions, 0 new test files** — every change lands as an added assertion, a re-pinned literal, or a rename inside a test that already existed, which is what makes the unchanged suite total below meaningful. Two assertions are net-new: `tests/test_install_gateway.py` (Opus 5 routable alongside the retained 4.8) and `tests/test_readme_styling.py` (the shields.io version badge pinned to `plugin.json` — added because this release's own version bump missed that badge three times, and no default-suite test covered it). Two lever test functions were renamed where their names asserted the claim their rewritten docstrings retract, with assertion-set MD5 digests proven identical before and after; the release version pin advanced. *(A finer-grained "N literals re-pinned across M files" tally was deliberately dropped from these notes: it carried no honesty the two claims above do not already carry, had to be hand-re-derived after every subsequent edit, and was wrong twice during this run — the same defect class as pinning a contract to line numbers a change moves.)* Suite **5939 passing + 16 skipped** (213 test files) on the recorded macOS-without-PyYAML basis — **unchanged**, as expected for a change that adds no test function. Measured on the Windows checkout this ran on: **5948 passed, 2 failed, 5 skipped**, identical to the pre-change baseline; the 2 failures are pre-existing CRLF byte-count artifacts (`tests/test_installer_guidance_blocks.py::test_remove_deletes_exactly_and_byte_preserves`, `tests/test_skill_references.py::test_target_skill_byte_count_reduced_vs_baseline[common-pipeline-conventions]`) that this change does not touch and did not "fix" by editing recorded counts. Skill / agent / command / hook / Layer-3-tool counts UNCHANGED (48 / 39 / 23 / 7 / 20).

## [3.44.0] — 2026-07-24 — harden-verification-dev-test-prod (never "done" off a unit test; dev → test-on-dev → prod as an agent-immutable opt-in)

**MINOR — the verification gates get teeth and a prod-deploy discipline lands, all as additive, backward-compatible enforcement; NO new skill / agent / command / hook / Layer-3 tool, and the inventory holds at 48 / 39 / 23 / 7 / 20.** The trigger: agents claiming "done" off a passing unit test, inventing unasked-for caution, and not verifying backend changes end-to-end when they carry frontend impact. This release makes that class of failure structurally impossible where it matters, and makes `dev → test-on-dev → prod` a real, opt-in, human-owned discipline the agent cannot route around.

- **The end-to-end gate (`hooks/frontend_impact.py` + `review_evidence_schema.py`).** New `detect_frontend_impact()` — the canonical frontend-file extension set + the either-signal detector: a change has frontend impact when EITHER the diff touches frontend files OR a changed backend endpoint is consumed by a frontend route (per ROUTE_MAP / INTEGRATION_MAP). The evidence schema gains a **conditionally-required** `frontend_impact_e2e_review` field: when `files_changed` shows frontend impact, a real-frontend end-to-end verdict ('pass') is MANDATORY — a passing unit test can never open the gate. Backward-compatible (the field is optional with no impact — the established optional-VAO-field pattern), so **zero blast radius** across 590 evidence-touching tests; the 17-field required-count pin is untouched.
- **The dev → test-on-dev → prod opt-in (`hooks/deploy_config.py` + `.architect-team-deploy.json`).** A per-project, human-authored, gitignored config (schema template committed at `.architect-team-deploy.example.json`, mirroring the `.architect-team-notify.json` pattern) opts a project into the discipline: `prod_deploy.enabled` gates it, `always_merge_to_prod_on_complete` makes every clean run auto-merge to `prod_branch` on completion, `deploy_command` (null = "prod" is just the merge to `prod_branch`) runs a real external deploy after dev is green. The reader is pure + fail-safe — an absent / malformed config resolves to "not enabled" (a broken config never triggers a deploy).
- **Immutability — the agent can never override (`pretool_unilateral_override_guard.py`).** Once `.architect-team-deploy.json` exists it is IMMUTABLE to agents: the PreToolUse guard BLOCKS any agent Edit / Write / NotebookEdit targeting an existing deploy config, unconditionally (regardless of run state). Creating a fresh config is allowed — it only ADDS the prod constraint. Only a human changes a human's policy — by editing the file, or passing `--no-prod` / `CT6_NO_PROD` for a single run. An agent deciding to disable or skip the prod mandate "to be safe" is the buck-the-command override this forbids.
- **The ETHOS gains fidelity (`docs/ETHOS.md`).** A new `## Fidelity to human-configured policy` section (outside the pinned 7-principle fence — the count and every agent's compiled block are untouched): a human's explicit configuration is binding; the agent obeys it and never self-grants an exception, with **invented caution** — the unasked-for "PHI safeguard just in case", the silent scope-narrowing — named as the anti-pattern.

Tests: 4 new test files (`test_frontend_impact` [14], `test_frontend_impact_gate` [8], `test_deploy_config` [8], `test_deploy_config_immutability` [8, incl. the ETHOS-fidelity pin]) — 38 tests, all TDD (red before green). `tests/test_dispatch_banner.py`'s version pin advanced 3.43.0 → 3.44.0. Suite **5901 → 5939 passing + 16 skipped** (213 test files), on the recorded macOS-without-PyYAML basis (+38; 16 skips = 14 PyYAML-gated + 2 archived-coverage-map). Skill / agent / command / hook / Layer-3-tool counts UNCHANGED (48 / 39 / 23 / 7 / 20); **NO new skill / agent / command / hook / Layer-3 tool** — the new pieces are shared hooks modules + a config + one conditionally-required evidence field.

## [3.43.0] — 2026-07-23 — deliver-adversarial-opus-split (the strongest model where code is written and attacked; Fable where it is planned and reviewed)

**MINOR — a model-policy change: the shipped ship state flips from uniform Fable to the delivery-adversarial Opus split; NO new skill / agent / command / hook / Layer-3 tool, and the inventory holds at 48 / 39 / 23 / 7 / 20.** Since v3.32.0 every agent shipped `model: fable` (the deliberate uniform default). Owner directive (2026-07-23): run the agents that DELIVER and ATTACK on the strongest coding model, and keep the agents that PLAN and JUDGE on Fable. This release makes that split the ship state and adds the deterministic lever mode that reproduces it.

- **The split.** The **12 delivery + adversarial agents ship `model: opus`** (Opus 4.8): the four implementer / merge agents — `backend`, `frontend`, `integration`, `reconciler` — and the eight that attack / refute / reproduce / execute-to-surface-failures — `adversarial-reviewer`, `structure-adversary`, `fix-sensibility-checker`, `bug-replicator`, `qa-replayer`, `flow-executor`, `visual-analyzer`, `mini-qa`. The **27 planning / validation / review agents stay `model: fable`** — architects, deep planners, researchers, mappers, and the reviewers that adjudicate adversarial output (`system-architect`, `task-reviewer`, `test-completeness-verifier`, `flow-explorer`, `visual-capture`, `reference-tracer`, `monitor-synthesizer`, `doc-updater`, and the rest). Rationale, per the directive: Opus is the doer/attacker; Fable is the planner/judge.
- **The lever.** New mode `scripts/setup/set_default_model.py --split delivery` — canonical partition `DELIVERY_ADVERSARIAL_AGENTS` (a strict subset of the gateway development bucket, so the two splits never disagree on whether a stem is a "doer"), applied via `apply_deliver_split` / `deliver_split_targets` / `deliver_role_model`, recorded as policy `deliver-opus-split` (`POLICY_DELIVER_OPUS_SPLIT`), recognized by `policy_state` and reported by `--check`. It writes a **real `model: opus` id** — a DIFFERENT axis than the gateway secondary split, with **no gateway impersonation** — and an unclassified (e.g. newly scaffolded) stem **fails safe to Fable** (Opus is never the silent default). Idempotent, line-ending-preserving, only the frontmatter `model:` line is ever touched.
- **The pins.** The master `tests/test_agents.py` pin (formerly `test_all_agents_uniform_fable`) now asserts the split off the canonical set (`test_agents_follow_the_delivery_opus_split`), and the per-agent model pins for the 12 flipped agents assert `opus` with the delivery/adversarial rationale. The gateway-install and setup pins that read the real repo `agents/` now expect the `deliver-opus-split` ship state — the gateway secondary split still targets only the INSTALLED plugin copy, never the committed tree.

Tests: 10 new lever tests in `tests/test_set_default_model.py` (the delivery split's targets / apply / idempotence / model-line-only / fail-safe / policy-state / CLI / distinctness-from-secondary). `tests/test_dispatch_banner.py`'s version pin advanced 3.42.1 → 3.43.0 (the standing per-release pin update). Suite **5891 → 5901 passing + 16 skipped** (209 test files), on the recorded macOS-without-PyYAML basis (+10 lever tests; the 16 skips = 14 PyYAML-gated + 2 archived-coverage-map). Living capability specs UNCHANGED at 68; skill / agent / command / hook / Layer-3-tool counts UNCHANGED (48 / 39 / 23 / 7 / 20); **NO new skill / agent / command / hook / Layer-3 tool**.

## [3.42.1] — 2026-07-21 — docs-currency-v3-42-1 (a full documentation-currency sweep: 207 docs walked, 202 verified current, exactly one updated, zero dead pointers)

**PATCH — a documentation-currency sweep of the plugin's own doc surface; docs-only, with no code / skill / agent / command / hook / test-behavior change.** The walked surface was all 207 tracked `*.md` files outside the frozen zones. Outcome: **202 docs verified current** against the code and against each other, **4 dispositioned as frozen-feature** (byte-preserved), and **exactly ONE doc updated** — `commands/architect-team-setup.md`, brought current with two source-verified fixes: a superseded secondary-route dialect claim corrected to the shipped behavior, and the one-call confirmation section augmented to describe the current completion + serving-deployment-identity + auth-enforcement probe that now layers on top of the models listing. Every walked doc was given an explicit disposition in a refreshed ledger; all **500+ cross-references were verified live by three independent checkers with zero dead pointers**; and the historical narrative + the frozen zones were byte-preserved. An independent widened-surface audit returned **PASS with zero findings**.

- **Scope.** Only the documentation-currency inventory (README, CHANGELOG, CLAUDE.md, and the two maps) carries the v3.42.1 stamp; the substantive plugin state is unchanged from v3.42.0. The maps' pure currency stamps advance to v3.42.1 (per the v3.39.1 docs-only precedent), while each version's feature attributions stay recorded as history.

Tests: unchanged — docs-only. Suite **5891 passing + 16 skipped** (209 test files), measured on macOS WITHOUT PyYAML (the 16 skips = 14 PyYAML-gated + 2 archived-coverage-map), green under both encodings (the default env AND `PYTHONUTF8=1`) — identical to v3.42.0. Living capability specs UNCHANGED at 68; skill / agent / command / hook / Layer-3-tool counts UNCHANGED (48 / 39 / 23 / 7 / 20); `tests/test_dispatch_banner.py`'s version pin advanced 3.42.0 → 3.42.1 (the standing per-release pin update); **NO new skill / agent / command / hook / Layer-3 tool**.

## [3.42.0] — 2026-07-21 — quality-upgrades-v3-42 (twelve self-improvements to CT6's own machinery: a quotable principles doc compiled into every agent, a recall data-envelope, generated instruction-surface tooling, single-source skill boilerplate, self-removing installer guidance, and an opt-in behavioral eval tier)

**MINOR — twelve improvements to the plugin's own machinery in one combined release, each closing a real gap; NO new skill / agent / command / hook / Layer-3 tool, and the inventory holds at 48 / 39 / 23 / 7 / 20.** Where the recent releases hardened the gateway and the first-install path, this release hardens the *instructions* CT6 runs on: it gives the load-bearing principles a single quotable home and puts them in front of every agent by construction, marks recalled memory as data at the point it enters a session, generates a cheap machine-readable capability catalog and a changelog-shape gate, single-sources the shared skill boilerplate the lint used to merely police, makes installer guidance self-removing when its capability is absent, and adds the first tier that verifies a real model ACTS correctly on the instructions rather than only that they are well-formed.

- **Principles injection (`principles-injection`).** New `docs/ETHOS.md` states CT6's seven operating principles — reuse-before-build, the-producer-is-never-its-own-checker, honest-boundary, unbounded-solving, default-to-action, documentation-currency, evidence-before-assertion — each paired with its named anti-pattern, so the tie-breaker a reviewer reaches for is written down once instead of scattered across 48 skill bodies. The principles are compiled into a marker-fenced block in all 39 `agents/*.md` (via an extended `scripts/setup/sync_agent_boilerplate.py` + `scripts/setup/agent_boilerplate_blocks.py`) and into the 5 pipeline-driving skills via the new single-source compiler below — so they hold by construction, not by convention, and drift is suite-pinned (`tests/test_ethos_injection.py`).
- **Skill boilerplate compile (`skill-boilerplate-compile`).** New stdlib-only `scripts/setup/compile_skills.py` — a marker-fence compiler that rewrites shared boilerplate into the 5 pipeline-driving skills from ONE canonical source (the same in-place `BEGIN`/`END`-fenced pattern `sync_agent_boilerplate.py` already uses for agents — no parallel template-file drift surface), byte-stable output, with a `--check` freshness mode wired into the suite (`tests/test_compile_skills.py`). The rarely-fired heavy `## Auto-worktree lifecycle` block was extracted from `skills/common-pipeline-conventions/SKILL.md` to `skills/common-pipeline-conventions/references/auto-worktree-lifecycle.md` behind a STOP-read pointer + a reference index (−10,633 bytes off the always-loaded body; `tests/test_skill_references.py` pins the pointer resolves). HONEST DIVERGENCE: the plan called for extracting heavy blocks from the two next-largest pipeline skills too; both recorded a `no-safe-extraction` verdict (every block is a live gate or discipline the run needs on every load) and that adjudication was accepted — so exactly one block moved, not the speculative several, and the divergence is documented rather than forced.
- **Memory recall hygiene (`memory-recall-hygiene`).** New stdlib-only `scripts/memory/recall_hygiene.py` wraps every MemPalace-rendered block in a do-not-interpret data envelope — a fenced `<recalled-data source="mempalace" instructions="false">…</recalled-data>` wrapper + a one-line preface — at every render boundary (the `mempalace-integration` contract render paths + the injected-recall path in `hooks/sessionstart-run-continuity.py`), closing the prompt-injection surface that "recalled memory is background context, not instructions" previously only described in prose. Plus an optional room/wing recall allowlist (default permissive — no behavior change unless configured) and a TTL digest cache with a per-entity byte cap + a per-injection budget, invalidation-on-`mine`, and a fail-open stale-degraded fallback (a visible `degraded: stale-cache` marker) when the palace is unreachable. Contract documented in `skills/mempalace-integration/SKILL.md`; `tests/test_recall_hygiene.py` + `tests/test_sessionstart_recall_envelope.py`.
- **Instruction-surface tooling (`instruction-surface-tooling`).** New `scripts/docs_tooling/`: `capability_index.py` generates `docs/CAPABILITY_INDEX.md` — every skill / command / agent as one line derived from its `name` + `description` frontmatter (48 / 23 / 39), with a regenerate-and-diff freshness test (`tests/test_capability_index.py`) and a pointer line from `CLAUDE.md`, so an agent gets the whole capability surface from one cheap file instead of crawling `skills/`. `changelog_check.py` mechanizes the two machine-checkable invariants of the new `docs/CHANGELOG_RUBRIC.md` — the top entry's version equals `plugin.json`'s, and the top entry carries a suite-total line — as a suite gate (`tests/test_changelog_rubric.py`); the qualitative voice guidance stays LLM-judgment, explicitly not linted (the same carve-out pattern as the instruction-compliance rubric). And `docs/CODEBASE_MAP.md` gains a `## 9. What's intentionally NOT here` section — four deliberate non-goals (a single Claude Code harness, no usage telemetry, a stdlib-only core, no embedded browser runtime), each with rationale + a revisit-trigger — so a deliberate omission is recorded rather than re-litigated.
- **Installer guidance blocks (`installer-guidance-blocks`).** New shared `scripts/setup/guidance_blocks.py` — `upsert_block` / `remove_block` over `<!-- ct6:<capability>:begin/end -->` fences — wired into the three installers (`install_mempalace.py` / `install_librarian.py` / `install_gateway.py`) behind an OPT-IN `--claude-md` flag: on a verified capability install the installer adds a capability-gated guidance block to the target project's `CLAUDE.md`, and removes exactly that block on a failed capability check or uninstall (idempotent, never touching text outside the fences, honest disabled text). HONEST DIVERGENCE: the block is opt-in — with no `--claude-md` flag there is NO side effect on the target `CLAUDE.md` at all — so a guidance block never appears unbidden and never goes stale silently.
- **Behavioral eval tier (`behavioral-evals`).** New `scripts/evals/` (`runner` / `judge` / `collector` / `budget`) + `tests/evals/` — the first tier that verifies a real model ACTS correctly on CT6's instructions, not only that they are well-formed. The runner shells to the `claude` CLI (stream-json parse, stdlib `subprocess`, `--max-turns` bounds); routing evals assert a prose prompt invokes the right pipeline skill, and a planted-defect outcome eval runs a bounded fixture repo and judges the result against ground truth. Deterministic pass thresholds live in the fixture, never the judge (the eval never grades a self-claim); the collector writes a per-run verdict + cost JSON and compares to the prior run; and a warn-first budget-regression gate flags >2× tool/turn growth (with noise floors; the failing mode behind `CT6_EVALS_BUDGET_STRICT` until baselines stabilize). The whole tier is OPT-IN via `CT6_EVALS=1` and excluded from the default suite — the default suite stays key-free and deterministic, with the engine logic (parser, collector math, budget-gate, fixture integrity) verified offline in `tests/test_evals_offline.py`. Model resolution is explicit (`CT6_EVALS_MODEL` → `services/common/service_config.FALLBACK_MODEL`) with a child-env scrub. One LIVE smoke was executed this release to prove the harness end-to-end: 4/4 pass (3 routing + 1 outcome) at the recorded real cost.
- **Frontmatter helper — YAML block scalars.** `tests/helpers/frontmatter.py`'s fallback parser now handles YAML block scalars (`>`, `>-`, `>+`, `|`, `|-`, `|+`) with PyYAML value-parity, so the folded `note:` scalars in the two maps parse identically with or without PyYAML installed (`tests/test_frontmatter_helper.py`).

Tests: 10 new default-suite test files (`test_ethos_injection`, `test_compile_skills`, `test_skill_references`, `test_recall_hygiene`, `test_sessionstart_recall_envelope`, `test_capability_index`, `test_changelog_rubric`, `test_installer_guidance_blocks`, `test_evals_offline`, `test_frontmatter_helper`); the opt-in eval tier under `tests/evals/` is excluded from the default collection. `tests/test_dispatch_banner.py`'s version pin advanced 3.41.1 → 3.42.0 (the standing per-release pin update). Suite **5891 passing + 16 skipped** (209 test files, `tests/test_*.py` disk-count basis) — measured on macOS WITHOUT PyYAML, where the 16 skips are 14 PyYAML-gated cases + 2 archived-coverage-map cases; identical counts under the default env AND `PYTHONUTF8=1`. Living capability specs 62 → **68** (the six folded capabilities above: `principles-injection`, `memory-recall-hygiene`, `instruction-surface-tooling`, `skill-boilerplate-compile`, `installer-guidance-blocks`, `behavioral-evals`). Skill / agent / command / hook / Layer-3-tool counts UNCHANGED (48 / 39 / 23 / 7 / 20); **NO new skill / agent / command / hook / Layer-3 tool**; the stdlib-only core holds (the eval runner shells to the already-required `claude` CLI).

## [3.41.1] — 2026-07-19 — gateway-activation-drift (the recorded-consent flag is never ground truth: drift detection on status, verify-then-heal on the install carry-forward, a symmetric SessionStart self-heal)

**PATCH — a bug-CLASS fix, not an incident patch.** A non-merge-preserving rewrite of `~/.claude/settings.json` (observed 2026-07-18→19) silently dropped the gateway env block (`ANTHROPIC_BASE_URL` + `ANTHROPIC_AUTH_TOKEN`) while `~/.architect-team/gateway/gateway.json` still recorded `activated: true` — and every consuming surface stayed green. The class: **a recorded-consent flag (`gateway.json.activated`) is treated as ground truth for the live client-side wiring it merely remembers, on every surface that reports or depends on it, with no verify-against-reality step and no heal.** v3.41.1 fixes the whole class — every consumer of `activated` either VERIFIES against `claude_env_applied()` or HEALS from the recorded consent, never trusts the flag bare. The replication artifact `tests/bug-fix-gateway-activation-drift/test_replication.py` (5 tests, `5 failed` pre-fix) flips green and remains the frozen regression contract.

- **REQ-001 — status names the drift (report-only; status observes, install repairs).** `_cmd_status` now computes `report.activation_drift` = recorded `activated` truthy AND the settings.json env block not FULLY applied. The fully-applied predicate (`activation_fully_applied`, new) is BASE_URL equal to the gateway URL AND a present `ANTHROPIC_AUTH_TOKEN` — a token-only loss is half-drift (B4 supplement-2) and counts; `claude_env_applied()` keeps its BASE_URL-only semantics for its other callers. The predicate is shared by BOTH the status drift computation AND the carry-forward verification (the B4 non-blocking observation: token-only half-drift is detected — and healed — on the install path too, not just at SessionStart). On drift: the summary carries `activated=False (DRIFTED)`, a dedicated `activation-drift` fail row names the recorded-vs-settings contradiction + the remediation (re-run `install --activate`, or start a new session for the SessionStart self-heal), and the footer prints `Claude Code activation: DRIFTED (recorded activated; env block missing)` instead of the generic `not applied`. The `--json` payload (B4 supplement-1) gains a first-class `activation_drift` field — the dict is hand-built, so drift is explicit on the machine-readable surface, not implicit in `steps[]`. A machine whose recorded `activated` is falsy keeps today's output byte-identical — drift text appears ONLY on genuine drift (the replication tests' markers deliberately exclude the generic `not applied` / `activated=False` strings).
- **REQ-002 — the install carry-forward verifies and heals from recorded consent.** The `elif prior_state.get("activated"):` branch no longer trusts the flag bare. It resolves the settings path + uses `args.port` (B4 gap-3: the port THIS install serves and records into the state write — never a stale prior-state port that would print green "verified" against a port the new install no longer serves). Verified (`activation_fully_applied`) → the carry-forward ok row names the verification ("activation carried forward (verified against settings.json)"). Drifted → a corrupt settings.json ABORTS first (B4 gap-4: if settings.json EXISTS but is unparseable JSON, a FAIL row names the corruption and `apply_claude_env` is NOT called — its `_read_settings` would silently treat corrupt as `{}` and overwrite; only an explicit `--activate` may overwrite a corrupt file). Otherwise the persisted master key is read from `gateway.env` (`read_env_file(...).get(MASTER_KEY_VAR)`); present → `apply_claude_env()` re-applies the env block from the served port + the persisted key (the recorded `activated: true` IS the prior consent; the key is never re-derived), with a heal row + `report.activated = True`. Absent/unreadable → a FAIL row names the drift + the `install --activate` remediation, `report.activation_carried` stays False, and no surface prints a green carried-forward. The state-write (`activated = report.activated OR prior.activated`) is unchanged in shape — consent stays recorded so the SessionStart heal stays armed. `setup_entry`'s display follows: `carried-forward` prints ONLY when verified; the healed path prints `activated=True`-shaped output via `report.activated`; the unhealable path surfaces the fail row via the existing failed-steps branch (degrades the setup row to warn).
- **REQ-003 — SessionStart self-heals activation drift.** A new `maybe_heal_activation()` in `hooks/sessionstart-run-continuity.py`, mirroring `maybe_heal_model_split`'s structure and guards. Installed-copy guard with explicit-injection BYPASS (B4 gap-1i): the hook must run from an INSTALLED plugin copy (a dev checkout is never rewritten), BUT when BOTH `gateway_state_path` AND `settings_path` are explicitly passed (the programmatic/test seam), the copy guard is bypassed — explicit injection of both paths is itself the consent to operate on those sandbox paths, and the real `main()` wiring never injects. State guards: `gateway.json` must record `activated` truthy AND `enabled` truthy AND `auth_mode == "api-key"`. Gateway-liveness guard (B4 gap-2 — the audited class must not recur inside the fix): the recorded `enabled` flag is NOT trusted bare; before any write, `127.0.0.1:<port>` is probed via an injectable `port_probe` seam (default: a real short-timeout `socket.create_connection`; tests always inject). Nothing listening → fail-open no-op (re-pointing sessions at a dead gateway would convert silent drift into hard-broken sessions). Settings guard: heal when `ANTHROPIC_BASE_URL` is ABSENT, or when it EQUALS the recorded gateway URL but `ANTHROPIC_AUTH_TOKEN` is absent (half-drift — the URL is provably ours, so completing the pair is safe); present-and-equal with token → no-op (healthy); present-but-DIFFERENT → no-op (a user-customized BASE_URL is never clobbered — the same posture as `remove_claude_env`). Corrupt-existing settings.json → abort fail-open (never overwrite a file we cannot parse). Heal: port from `state.get("port", 4000)`; master key = the PERSISTED `CT6_GATEWAY_MASTER_KEY` parsed from `<state dir>/gateway.env` (a tiny local KEY=VALUE parser — the hook keeps local copies per its existing `_SPLIT_POLICY_STRINGS` convention; the key is never re-derived). Merge-preserving write. Every failure path (missing state, corrupt JSON, absent key, dead port, corrupt settings, unwritable target) returns `""` — a session start can never wedge on this. The heal note names the re-applied URL + the restart caveat for already-running sessions. `main()` invokes the activation heal FIRST, then the split heal (B4 supplement-3 — order test-pinned so the notes read coherently on a machine recovering from both drifts).
- **REQ-004 — the root clobberer: the repo's own test suite was deactivating the developer's machine.** Found mid-run, and the actual origin of BOTH drift incidents. `tests/test_install_gateway.py::test_uninstall_purge_removes_state_dir` ran a REAL `uninstall` with `--base-dir` and `--agents-dir` sandboxed but **no `--settings-path`**; `_cmd_uninstall` then resolved the DEFAULT `~/.claude/settings.json` and `remove_claude_env` stripped `ANTHROPIC_BASE_URL` + `ANTHROPIC_AUTH_TOKEN` merge-preservingly whenever the recorded BASE_URL matched port 4000. On CI and on never-activated machines this silently no-ops — which is exactly why it survived undetected — but on the owner's ACTIVATED machine **every full `pytest` run deactivated the gateway**, leaving `gateway.json` recording `activated: true` against a settings.json with no env block: the precise drift shape REQ-001…003 exist to detect and heal. Both incidents are mtime-correlated to suite runs (2026-07-18→19, the v3.41.0 run's seven iterations; 2026-07-19 ~10:04Z, this run's own B5 suite runs — the change was reproducing its own bug). Note the product was never at fault: `uninstall` resolving the real settings.json is correct behavior for a real user running a real uninstall. The defect was entirely that a TEST invoked it unsandboxed. Fixed in three independent layers, deliberately redundant because any one of them can be forgotten by a future author: **(a)** the historical leaker now names an explicit `--settings-path`; **(b)** a module-wide autouse `_sandbox_default_settings_path` fixture redirects `_setup.DEFAULT_USER_SETTINGS_PATH` to a per-test sentinel, so NO test in `tests/test_install_gateway.py` — present or future, whether or not its author remembers the flag — can reach the real file, with a `test_default_settings_path_fallback_is_absorbed_by_the_sentinel` probe proving the redirect actually absorbs the fallback rather than merely existing; **(c)** a session-scoped `_real_state_tripwire` in `tests/conftest.py` takes SHA-256 content digests of the real `settings.json` + `gateway.json` + `gateway.env` at suite start and raises a loud, named `REQ-004 TRIPWIRE` failure at suite end on any mutation — converting a future leak of this class, in ANY test file through ANY code path, from silent damage into a named suite failure that also prints the recovery path. Content digests rather than mtimes: a byte-identical rewrite is not a mutation and a bare touch should not cry wolf. The tripwire was verified to actually FIRE (a simulated leaky test in an isolated pytest session: the leaky test itself reports `passed` while the session ERRORs with the named tripwire message — the whole point, since the historical leak also "passed"). **(d)** `tests/test_install_librarian.py` audited: every test invoking `install` is sandboxed by `--base-dir` or `CT6_LIBRARIAN_HOME`, and `install_librarian.py` never touches `settings.json`, so it cannot cause this class — no change needed.
- **Forward-compatibility pin for credit-exhaustion failover.** `maybe_heal_activation`'s state guards are read BEFORE the liveness probe, and that ordering is now load-bearing and test-pinned: `test_heal_suppressed_by_recorded_state_even_on_a_live_port` (parametrized over both `activated: false` and `auth_mode: subscription`) asserts the heal stays silent on a LIVE port purely on the strength of recorded state, and that the probe is never even called. This makes RECORDED CONSENT the single suppression seam, so a future failover that un-points Claude Code when the upstream runs out of credits suppresses this heal by flipping recorded state, with zero change to the guard logic — rather than being silently fought and undone on the next session start. The docstring additionally records that TCP port-liveness is explicitly NOT a credit/upstream-health signal: a credit-dead gateway still binds its port and still passes the probe.
- **Docs.** README gateway section: a v3.41.1 capability table (drift detection + the carry-forward verify/heal + the SessionStart activation self-heal, matching the section's existing voice/format). CHANGELOG: this entry. `pytest.ini` adds `--import-mode=importlib` so the two `tests/bug-fix-*/test_replication.py` files (this run's + the v3.41.0 run's) coexist without a module-name collision.

Tests: +43 hermetic tests — 8 replication in `tests/bug-fix-gateway-activation-drift/test_replication.py` (5 for REQ-001…003, the frozen regression contract, UNMODIFIED; + 3 for REQ-004, all three RED before the isolation fix and green after) + 33 unit in `test_fix_units.py` + 1 sentinel-absorption probe in `tests/test_install_gateway.py` + the session tripwire in `tests/conftest.py`, covering every `coverage-map.json` acceptance criterion: status clean-machine output unchanged (no drift text), the explicit `activation_drift` `--json` field, token-only half-drift detection, the drifted footer qualifier; carry-forward verified-wording (against `args.port`), the served-port-not-stale-prior-port pin, drifted+key heal (merge-preserving + `report.activated=True`), drifted+no-key FAIL row (via a `read_env_file` monkeypatch — the absent-key branch is a defensive safety net, unreachable through the normal install flow because `resolve_keys` always regenerates + persists a master key), corrupt-settings abort (byte-preserved, both the install path + `setup_entry`'s warn degradation), `setup_entry` carried-forward-only-on-verified + the healed path's no-carried-forward; the SessionStart heal guards — dev-checkout no-op WITHOUT full injection, explicit-injection bypass, enabled-false / subscription-mode / activated-false no-ops, dead-port no-op via injected `port_probe`, custom-BASE_URL never clobbered, healthy-machine no-op, half-drift token completion, merge-preservation of unrelated keys (top-level + env + the agent-teams flag), fail-open on absent/corrupt state + absent master key + corrupt settings (byte-preserved abort), missing-settings creates-it, recorded-port flows into both the BASE_URL + the probe argument, and `main()` heal order (activation before split) + the heal note reaching stdout. Suite **5646 → 5689 passing + 4 skipped** (202 test files by the `tests/test_*.py` disk-count basis — the prior 200 plus this run's `test_replication.py` and `test_fix_units.py`), green under both Windows cp1252 AND `PYTHONUTF8=1` — and the REQ-004 tripwire stays silent across a full run, which is now itself the standing evidence that the suite no longer mutates real machine state. Skill / agent / command / hook / Layer-3-tool counts UNCHANGED (48 / 39 / 23 / 7 / 20); **NO new skill / agent / command / hook / Layer-3 tool**; stdlib-only holds (`socket` for the TCP liveness probe).

## [3.41.0] — 2026-07-18 — glm-secondary-route-fix (registry route dialects + the spawn-compatible impersonation alias + the spawn-mandatory, upstream-verified, served-state-aware /v1/messages confirm + the port-holder-stopping, bind-verified restart + the exactly-one-instance, ownership-verified, auth-enforcing install + the never-dark verify-then-swap deploy with the launcher port guard + tree-aware instance identity with route-contradiction staleness)

**MINOR — the v3.40.0 GLM secondary becomes actually USABLE: the gateway route speaks each provider's real API dialect, the role split writes a harness-spawnable Claude id the gateway impersonates (disclosed), and "CONFIRMED live" now requires a real completion.** Two independent, stacked defects, both live-verified 2026-07-17 (`.architect-team/bug-isolation/glm-secondary-route-fix/`); the replication artifact `tests/bug-fix-glm-secondary-route-fix/test_replication.py` (4 tests, `4 failed` pre-fix) flips green and remains the frozen regression contract.

- **BUG-A — gateway route dialect (every `ct6-secondary` call 404'd).** The registry hard-coded the `openai/` provider prefix for every entry, and LiteLLM's *proxy* drives Anthropic-format (`/v1/messages`) traffic for `openai/*` models through the OpenAI **Responses API**; api.z.ai implements only `/chat/completions` → live `404 {"path":"/v4/responses"}` on every call (invisible to non-proxy tests — SDK and Router paths 200 on both prefixes). Fix: each `SECONDARY_PROVIDERS` entry REQUIRES `route_dialect` (`openai` → `openai`; `zai` → `hosted_vllm`, verified live with thinking blocks intact); the route is DERIVED as `<route_dialect>/<model>` and the redundant `route_model` field is REMOVED (an entry structurally cannot carry a route inconsistent with its dialect); `_secondary_route_model`/`build_gateway_config` consume the registry field — zero hard-coded prefixes remain; the extensibility pin requires the field + the exact key-set.
- **BUG-B — harness spawn gate (a spawn on the alias emitted ZERO HTTP).** Claude Code's Agent-Teams spawn path validates teammate model ids client-side — any frontmatter alias that is not a known Claude id dies before the gateway (gateway-log verified). Fix: the exported, lever-single-sourced **`SPAWN_ALIAS_MODEL_ID = "claude-haiku-4-5"`** (test-pinned; changing it is a deliberate act) becomes the split's frontmatter target — `split_targets`/`apply_split`/`apply_policy` write it, `policy_state` recognizes all three alias generations, `migrate_legacy_split` moves BOTH superseded frontmatter values (`codex-5.6-sol` AND raw `ct6-secondary`, which is spawn-dead) to it on every config-regenerating install, and the uniform lever refuses it. The generator emits the explicit impersonation route (`claude-haiku-4-5` → `<dialect>/<model>`) in BOTH auth modes, AHEAD of the anthropic catch-all, and SKIPS the real-Anthropic route for that one id (a duplicate `model_name` would form a LiteLLM load-balancing group mixing GLM with real Haiku). **Disclosure triple:** `gateway.json` records `spawn_alias`/`spawn_alias_maps_to`; `status` prints "claude-haiku-4-5 -> glm-5.2 (impersonated secondary)"; the README gateway section states the trade-off plainly. `ct6-secondary` REMAINS served for direct/scripted callers; the SessionStart self-heal heals to the state-recorded alias with the new priority `spawn_alias` > `secondary_alias` > legacy `codex_alias` (a pre-spawn-alias state still heals to what its config actually routes, with an upgrade note).
- **Wrong-hop confirmation — FO-1 supersedure.** The v3.40.1 entry's FO-1 "client-side" attribution was half the story: the harness gate WAS real (BUG-B), but the gateway completion hop was ALSO broken (BUG-A), and the v3.39.0 confirm probed `/v1/models` only — model LISTING, not completion SERVING — so setup printed "CONFIRMED live — CT6 runs the split" over a 404ing `/v1/messages` path. `confirm_gateway_serving` now requires a bounded `/v1/messages` completion (max_tokens 32, non-empty content block — thinking blocks count; spawn alias first, `ct6-secondary` fallback) through a new injectable `_default_completion_prober` seam beside the existing models-prober seam; the one-restart repair cycle covers the completion hop; the success report is two-hop honest (gateway completion CONFIRMED + the spawn-hop is verifiable only from a fresh session); `--check-only`/`--no-register` never probe (unchanged); `status --live` probes ONLY state-recorded aliases (legacy installs stay green).
- **Iteration 2 (SR-glm-fix-iter2) — the first deploy exposed two further wrong-hop class members, both live-verified at B6 and closed here.** (1) **Stale process (restart-on-regeneration):** the installer's register/start step is start-if-not-running — after `config.yaml` regeneration the RUNNING pre-deploy process kept serving the prior config (stale marker `claude-ct6-secondary` in `/model/info`), and the `anthropic/*` wildcard dynamically instantiated a REAL-Haiku deployment for `claude-haiku-4-5` (6/6 probes on deployment `883c743106d3`, real-Haiku pricing in `x-litellm-response-cost`). A config-REgenerating install now RESTARTS a running gateway before confirming; the trigger is ANY byte-level config-content change — an api_base or dialect edit keeps the model-group SET identical, so set-shape detection would miss exactly that class — while a byte-identical re-install never churns a healthy process and a fresh install is covered by register's own start. (2) **Upstream-blind probe:** the new completion probe accepted the wildcard-served real-Haiku completion — it proved A completion, not the deployment. The probe now verifies the SERVING DEPLOYMENT: the response's `x-litellm-model-id` is resolved via GET `/model/info` (same auth) and the deployment's `litellm_params.model` must equal the registry-derived dialect route computed at probe time (never a constant); a mismatch FAILS naming the observed upstream — "served by anthropic/claude-haiku-4-5, expected hosted_vllm/glm-5.2" — closing the probe's last dishonesty window (silently billing Anthropic while reporting the split live). Both fixes ride the existing machinery (`restart_gateway` + the injectable `_default_completion_prober` seam, extended with `expected_upstream`); `status --live` derives the same expectation from the recorded provider/model, and the one-restart repair cycle now also fires on a wrong-upstream completion (the stale-process incident self-repairs even without the proactive restart).
- **Iteration 3 (SR-glm-fix-iter3) — the second deploy went green on a STILL-stale listener; two escapes, both live-verified at B6 and closed here.** The RCA is now three iterations of ONE class — each layer of the confirm proved something true but not the thing CT6 depends on: model LISTING (v3.39.0) → A completion (iteration 1) → an identity-verified completion on SOME alias (iteration 2) → **on the RIGHT alias** (this iteration) → **against the SERVED state, not the file** (this iteration). (1) **Fallback green:** the confirm probed spawn-first but fell back to `ct6-secondary` — whose upstream is correct in BOTH the old and new configs — so it printed "completion CONFIRMED via ct6-secondary (upstream deployment verified = hosted_vllm/glm-5.2)", TRUE but IRRELEVANT while the spawn alias (the id the harness actually spawns) stayed anthropic-served. The confirm is now **spawn-alias-MANDATORY**: `confirm_gateway_serving` gains `mandatory_completion_model` — the mandatory candidate's failure fails the whole confirm without trying any later candidate, and a completion that arrived via a different alias is rejected by name; install and spawn-recorded `status --live` declare the spawn alias mandatory, and fallback green survives ONLY for legacy states with no recorded `spawn_alias`. (2) **File-vs-served staleness:** the iteration-2 restart trigger byte-diffed the generated config against the PRIOR FILE — but iteration 1 had already regenerated the file while the pre-deploy PROCESS kept serving the older config (file-current-process-stale matches no byte diff, and no restart fired). Staleness is now judged against the **SERVED state**: the new pure `_served_state_staleness` compares the generated config's model-group set (parsed by `_config_model_groups`, coupling-test-pinned against the real generator) against GET `/model/info` in BOTH directions (a served group the config does not route IS the stale-marker signature) plus the secondary/spawn routes' expected upstreams (group-name sets can be IDENTICAL — the wildcard instantiates the same name — while the upstream is wrong); any mismatch restarts via the existing `restart_gateway` + confirm poll. The byte diff stays as the cheap first check; the served-state probe rides a new injectable `_default_model_info_prober` seam (keyless pytest hermetic); an UNREACHABLE `/model/info` cannot judge — no proactive restart (a cold-starting fresh gateway looks exactly like that), with the spawn-mandatory confirm as the fail-closed backstop whose repair cycle still restarts on a wrong-upstream completion.
- **Iteration 4 (SR-glm-fix-iter4) — the third deploy: iteration-3's machinery behaved exactly as designed (staleness named, confirm failed closed on the mandatory contract) — but the "restart" was mechanically a NO-OP, extending the RCA ladder one rung deeper: the verified claim must also be EXECUTED against the actual port-holder, with the bind verified.** Port 4000 was held by an UNTRACKED, manually-started diagnostic process (PID 68724) across TWO "restarts": `restart_gateway`'s tracked stop (`schtasks /end`) only reaches the task-managed process, and each blocked start leaked a doomed litellm instance (5 zombies observed, none holding the port). Two fixes, both inside the VERIFIED restart the install call sites now invoke (the legacy no-context call shape is byte-identical): (10a) **stop-by-port** — ONLY after the tracked stop fails to free the gateway's OWN configured port through a bounded grace poll (the B4 auditor's ordering pin: the fallback must never drift into being the primary path) is the LISTENING pid resolved (stdlib `netstat -ano` parse on Windows / `lsof -ti` elsewhere; pure parsers test-pinned, near-miss ports never match) and THAT pid stopped — **image-agnostic by design** (no litellm name guard: port ownership is the class boundary — auditor note 3), with the resolved pid+image named honestly on SUCCESS (the audit trail for stopping a process the tracker did not own — auditor note 1) AND on refusal (access-denied ⇒ a named failure + manual remediation + NO start attempted); cannot-resolve stays fail-open (no kill fires on a guess), with 10b as the fail-closed backstop. (10b) **bind verification** — "restarted" is reported ONLY after the new instance answers `/model/info` with a state matching the generated config (the iteration-3 served-state seam, reused verbatim, on the same bounded-poll discipline as the confirm — four call-time-resolved `RESTART_PORT_*`/`RESTART_BIND_*` constants mirroring `CONFIRM_*`); a still-held port or a still-stale serve is a NAMED failure after at most ONE start attempt — never a doomed-instance spawn loop.
- **Iteration 5 (SR-glm-fix-iter5) — a USER scope amendment ("make sure that never happens on install") after the fourth deploy's aftermath broke live sessions, completing the install-honesty class at the PROCESS-POPULATION level.** The confirm was legitimately green at its moment — then the accumulated state bit: 8 gateway instances had piled up (~12 GB; register's start-now half ran `schtasks /run` on EVERY install), and after the iteration-4 port-holder kill an ENV-BROKEN zombie (master key unresolvable) stole port 4000 in the race window — the state-match bind-verify PASSED on it (same config file) while every real-token request failed into LiteLLM's no-DB 400 path (`no_db_connection` on `claude-fable-5`), breaking the user's fable session traffic. Four closures, each identity- or invariant-based, none incident-pattern-matched: (11a) **exactly-one-instance** — before its single start, install stops EVERY process whose cmdline carries THIS state dir's config path (an ownership PROOF — every generated launcher passes `--config <state>/config.yaml`; image- and launcher-agnostic, so other state dirs' gateways and unrelated litellm never match; Windows cmdlines normalized for case, separators, and the 8.3 short-name form per the B4 auditor note, via an injectable `GetShortPathNameW` seam), and register NEVER blind-starts over a held port — an owned holder is KEPT serving (its zombie siblings swept), a foreign holder defers to the verified restart's port-holder stop; the at-most-one-owned-instance result invariant is test-pinned end-to-end, the legacy no-context `register_gateway` call shape stays byte-identical, and sweep rows are honest (pid+image on success AND refusal, with remediation); (11b) **start-ownership** — after the one start, the port-holding pid must BE the instance the restart just launched: the per-launcher pid seam where the launcher exposes it (`systemctl show -p MainPID` / `launchctl print`), the owned-instance LAUNCH-WINDOW match where pid lineage is opaque (schtasks / the detached shim — the B4 auditor's documented seam), judged by the pure `_judge_bound_instance`; a race winner is a NAMED failure ("a PRE-EXISTING owned instance, not the one this restart just launched", with the deploy-4 cross-reference and the kill hint — never auto-killed into a kill/start loop), unresolvable identity stays fail-open with the honest note (the served-state match remains the fail-closed backstop); (11c) **auth-enforcement probe** — the confirm's ladder gains its first NEGATIVE rung: a deliberately-invalid key (random, NEVER echoed in any detail — the auditor's never-echo note) must be DENIED SERVICE. Arms corrected in **iteration 5b** (captured evidence: a DB-less LiteLLM — every healthy CT6 install — ALWAYS answers an invalid key with the prisma crash-500; it cannot emit a clean 4xx, so a 5xx-FAIL arm would fail-close every healthy install): a 4xx is clean enforcement; a crash-5xx is enforcement-with-a-NAMED-WARNING cross-referencing the LiteLLM no-DB prisma quirk (the confirm PROCEEDS, the warning riding the success detail); only a 200 — service GRANTED to a bad key, exactly the env-broken-zombie state — FAILS naming auth-not-enforced — on the same injectable-seam discipline (`_default_auth_prober`; keyless pytest hermetic), firing only AFTER a passed real-key completion rung (the 5b auditor pin, test-pinned: a down gateway can never surface the 5xx-warn green; the listing-only legacy shape byte-identical), failing fast on the 200 arm (a broken auth env is per-instance-deterministic, and the install's one restart repair cycle — a fresh instance re-reading gateway.env — is the remediation); (11d) **master-key never-rotate pin** — a re-install preserves `CT6_GATEWAY_MASTER_KEY` byte-identically (plain re-run, provider switch, and the pure resolver all pinned; `preserved_env_keys` covers it): rotation would invalidate the settings.json `ANTHROPIC_AUTH_TOKEN` every live session depends on, which is what makes the Lead's deploy replace-the-hand-started-instance safe by construction. Rotation exists only as a hypothetical future explicit flag — none today.
- **Iteration 6 (SR-glm-fix-iter6) — a SECOND USER mandate ("figure this out and ensure it never fucking happens again") after the 2026-07-18 OFFLINE incident, completing the deploy-honesty class at the AVAILABILITY level.** The user's live Claude sessions went dark: every request through the gateway returned `400 no_db_connection`. RCA (incident report + LiteLLM source `user_api_key_auth.py:1537`): the error is misleading — a failed master-key check on a DB-less deployment falls into the `prisma_client is None` branch, and the port-4000 winner among **8 piled-up instances** (~12 GB; every one launched through `run_gateway.bat`, timeline 16:02–21:23) had an UNRESOLVED `CT6_GATEWAY_MASTER_KEY`. The deploy shape itself was the root: stop-then-start on the LIVE port — the one every active session routes through — so each deploy opened a dark window and a port race, nothing verified an instance's ENV before it was allowed to serve, and the launcher happily piled onto an already-serving port. Three closures: (12a) **verify-then-swap, never dark** — before ANY live-instance stop, a STAGING instance of the identical generated config (only the launch port swapped; launched through the SAME env-sourcing launcher template as the cutover start, per the B4 launch-path-fidelity note) must pass the FULL accumulated ladder on the staging port (`STAGING_PORT_OFFSET`): bind + served-state match, staging-bind ownership (the iteration-5 rung re-aimed — a stale staging-port squatter cannot answer for a dead staged instance; an owned squatter is leaked residue cleared by pid, a foreign one an honest failure never killed), the REAL-key completion with upstream identity (an unresolved master key DENIES the real key service and fails exactly here — the incident state, caught structurally by rung conjunction, no `no_db_connection` matcher anywhere in the mechanism), then the auth-enforcement rung (after the completion, the 5b ordering preserved). ONLY on staging-green: staging is torn down and the low-risk cutover runs through the EXISTING iteration-4/5 machinery (port-holder stop, one start, bind verify, ownership judge), followed by a post-cutover completion+auth re-verify with ONE bounded retry for a port-race winner (remediated via the iteration-4 port stop); retry exhaustion is the one honest-fail path that can leave the box dark and its message EMBEDS the recovery procedure. ANY staging failure leaves the old instance serving (no tracked stop, no kill, no start — pinned per failure kind) and fails the install honestly naming the rung. THE B4-REQUIRED PIN: the staged instance is stopped on ALL exit paths — green cutover, every ladder-rung failure, and exception (try/except/finally) — via a two-sweep teardown (staging-port holder by pid + owned launch-window residue, never the live holder), so verify-then-swap can never reintroduce the iteration-5 accumulation class on the staging port; the iteration-5 next-install sweep stays the backstop, never the mechanism. (12b) **the launcher port guard + masked key log** — `run_gateway.bat` (and the posix twin) now REFUSES to launch when the port THIS launch binds is already LISTENING, naming the holder pid (keyed on its OWN embedded port per the B4 note, so a staging launcher is never refused by a busy live port; posix is lsof-best-effort, standing down to the installer machinery when absent) — closing the pile-up vector at the single chokepoint all 8 incident instances entered through — and logs masked master-key presence after sourcing gateway.env and before exec: the first 8 chars exactly when resolved, an explicit MISSING when not (never the full key, never wider — the incident's undiagnosable was-the-env-ever-loaded question, answered in the log from now on). (12c) the recovery procedure is codified (see Docs) and embedded in the confirm failure hints + the retry-exhaustion message.
- **Iteration 7 (SR-glm-fix-iter7, CRITICAL) — deploy-6 darkened the port a SECOND time; the RCA lands one level deeper than any prior rung: the two IDENTITY PRIMITIVES the whole discipline stack stands on were wrong.** Live forensics: (A) the exactly-one sweep kept listener pid 54032 (`python.exe`) and stopped pid 30828 (`litellm.exe`) as a "non-holder" — but they were the SAME instance (`cmd` → `litellm.exe` → `python.exe`); killing the parent killed the listener; port dark; the user's sessions offline again (second occurrence — the iteration-5 deploy's kill of 81780/9716 was the same class); and (B) the staging served-state rung FALSE-FAILED a fresh staged instance: `/model/info` advertises the catch-all's wildcard expansions (23 `anthropic/<id>` groups), which the name-set diff read as unrouted extras — the false-staleness accelerant behind the forced restarts and their race windows all along — while bare NAME matching would instead blind the true marker (`anthropic/*` matches `claude-ct6-secondary`'s NAME; only its serving ROUTE contradicts). Two closures: (13a) **an owned INSTANCE is a process TREE, not a pid** — the CIM listing now carries `ParentProcessId` + `CreationDate.ToFileTimeUtc` (POSIX: `ps pid=,ppid=,args=`), owned pids are grouped by validated parent chain (a "parent" born AFTER its child is a recycled pid — link severed, the child roots its own tree; a dead root leaves the tree rooted at the surviving ancestor), and membership is bounded to INSTANCE-AFFILIATED processes (cmdline carries the state dir's path — config or generated launcher), so a user's interactive shell, svchost, and every system ancestor never joins a tree and two instances never merge through a shared service parent; stop semantics carry TWO pinned predicates (the B4 auditor's pin 1, both directions test-pinned on the exact forensic chain): the accumulation-clearing SWEEP is port-holder-tree IMMUNE (never touches ANY pid in the serving tree; per-pid kills, root-first, deliberately no `/t` — a tree link the enumeration failed to see must never cascade into the listener) while the DELIBERATE port-freeing stops (the iteration-4 port-holder repair, the iteration-6 cutover retry, the staging teardown + preflight) act on the holder's WHOLE tree root-first with tree-kill semantics (`taskkill /t`, no respawn-capable orphan), and the teardown's live exclusion hardens from a lone pid to the live holder's whole TREE (a worker the listener spawns mid-staging-window is immune, never "residue"); a table that cannot be enumerated degrades to one-node trees (fail-open, never a guessed tree). (13b) **staleness is ROUTE-CONTRADICTION, two-armed** (the auditor's pin 2): the new `_config_model_routes` parser (coupling-pinned against the real generator, both providers) reads every (name → route) prescription INCLUDING wildcard patterns from the generated config AT COMPARISON TIME (provider-neutral — no `anthropic/*` in code); arm 1 (served→config) requires each served group's SERVING route to agree with the config — an explicitly-routed name compares against its explicit route ONLY (explicit precedence: the spawn alias absorbed by the catch-all's own consistent expansion is still a contradiction), any other name is routed iff some pattern pair matches BOTH name and route (a fresh instance's 23 wildcard-advertised expansions are ROUTED, not stale — the accelerant is gone); arm 2 (config→served completeness) requires every explicitly-routed config group present in the served state (a stale process that predates a newly added alias silently absorbs it into the catch-all — the iteration-2 dishonesty genus, caught by presence). The three incident-grade cases are pinned: fresh (with expansions) passes, healthy-long-running (more dynamic instantiations) passes, genuinely-stale (the marker, route-contradicting under the catch-all) still fails. NO deploy in this iteration — the Lead holds the deploy moratorium until the fix lands reviewed.
- **Docs.** README gateway section: the v3.41.0 capability table with the impersonation disclosure, the prisma no-DB auth-500 troubleshooting note (a DB-less LiteLLM answers ANY failed auth with 500 + `ModuleNotFoundError: prisma` instead of 401 — use the gateway.env master key), the path B (claude-shaped named alias, future toggle) / path C (upstream custom-id support) follow-up notes, and (iteration 6) the offline-incident recovery procedure — kill every instance whose cmdline matches THIS gateway's config path (never a bare image-name hunt), verify the port is free, start exactly ONE instance via the generated launcher, `status --live` — plus the never-dark deploy note (the installer stages before it swaps; a manual stop-then-start on the live port is exactly the incident shape).
- **Also repairs a v3.40.1 release miss:** `test_dispatch_banner.py::test_plugin_metadata_at_1_5_0` was left pinning 3.40.0 while the manifests shipped 3.40.1 — the version pin was one release behind, so `main` carried one red test (the shipped v3.40.1 tree was 5541 + 1F, not the recorded 5542; the v3.40.1 suite ran at Phase 5, BEFORE the Phase 8 version bump, and nothing re-ran the pin afterward). The pin now tracks 3.41.0, green with this release's manifests. Process lesson, now release discipline: after the Phase 8 version bump, re-run the version-pin tests (at minimum `test_dispatch_banner.py` + the README styling/currency checks) — a suite total recorded pre-bump does not cover the bump itself.

Tests: `tests/test_install_gateway.py` 123 → **217** (both-providers generator-shape pin, spawn-route ordering + no-duplicate-model_name, completion-probe gate/fallback/legacy-shape, state + status disclosure, restart-on-completion-failure, check-only/no-register never-probe extended to /v1/messages, status --live recorded-aliases-only; iteration 2 +16: restart-on-regeneration ×3 — running-gateway restart on a changed config, fresh/byte-identical never restart, the ANY-content-change-not-group-set auditor pin — the deployment-identity assertion pure cases ×3, the hermetic real-probe upstream cases ×4 (wildcard mismatch named / secondary-deployment pass / missing-header fail-closed / no-expectation skips `/model/info`), confirm mismatch-naming + expectation pass-through ×2, the registry-derived expectation pin ×2 (both providers — never a constant), the B6 incident replayed end-to-end through the one-restart repair, status `--live` derived expectation; iteration 3 +14: the config-group parser pinned against the real generator, `_served_state_staleness` pure cases ×5 — match / stale-marker extra group / wrong-upstream-with-EQUAL-group-sets / missing generated group / wildcard exclusion — mandatory-candidate confirm semantics ×2 (mandatory failure never falls back + never even probes the fallback; a fallback-only success rejected by name), install spawn-mandatory no-fallback-green, THE file-current-process-stale escape replayed end-to-end (byte-diff silent, served-state restart fires, green only post-restart on the spawn alias), served-state restart-failure honesty, matching-served-state never-restarts, status `--live` spawn-mandatory + the legacy-no-spawn-alias carve-out pinned; iteration 4 +10: the netstat/lsof pure parsers ×2 — LISTENING-row resolution with near-miss ports and ESTABLISHED rows never matching — the legacy no-context call shape unchanged (no port machinery), THE untracked-port-holder escape at the unit level (tracked stop no-op ⇒ pid resolved from the port, stopped, ONE start, bind verified, pid+image named on SUCCESS), the ordering pin (tracked stop freeing the port ⇒ the pid-kill never fires), refusal honesty (access-denied ⇒ named pid+image+remediation, NO start attempted), the no-image-guard pin (a non-litellm holder stopped identically), bind-failure honesty ×2 (unreachable /model/info and still-stale serve both NAMED, exactly one start, never a spawn loop), and the escape replayed end-to-end at the install call site — stale served state + held port ⇒ pid stop, bind-verified restart, green only through the spawn-mandatory completion; iteration 5 +13: the pid|cmdline / `ps` process-listing pure parsers, the Windows ownership-normalization matrix — long/upper/forward-slash/8.3 forms match, another state dir's path and our own pid never do, cannot-enumerate is fail-open None — the pre-start owned-zombie sweep with kills strictly before the ONE start, the no-blind-start pin (an owned holder KEPT, no `/run` fired), the foreign-held-port start skip, the legacy register call shape byte-identical, sweep-refusal honesty (pid+image+remediation), THE zombie-race winner caught post-start (state MATCHES yet the pre-existing owned pid holding the port is a named failure, no auto-kill, one start), the just-launched launch-window verification, the auth probe's three HTTP outcomes (401 = enforced clean / 200 = unenforced FAIL / 500 = enforced-with-named-warning per the iteration-5b arm correction, probe key never echoed), the confirm auth rung (200-fail named; the 500-warn PROCEEDS with the warning riding the confirmed-success detail; listing-only legacy shape never probes), the master-key never-rotate pin (re-run + provider switch + pure resolver), and THE SR-glm-fix-iter5 escape replayed end-to-end — sweep before start, keep-not-blind-start, port-holder stop, ownership-verified bind, auth-enforced green, exactly ONE owned instance at exit; iteration 5b +1: THE down-gateway ordering pin — a failed completion rung means the auth rung never fires, so the 5xx-warn can never read green on a down gateway; iteration 6 +14: the launcher port-guard both-ways pin (the live launcher's guard keys the live port, a staging launcher's guard keys the STAGING port and never the live one, guard before env-sourcing and exec, holder pid named) + the masked key-log pin (first 8 chars EXACTLY, never wider, the explicit MISSING arm, env-sourcing < log < exec), verify-then-swap green at the unit level (staging-spawn < staging-teardown < live stop < the one start; post-cutover ladder re-verified; exactly the launched instance left), staging failure leaves the old instance serving ×4 (bind / served-state / real-key completion / auth — no tracked stop, no live kill, no start, the rung named, the staged instance torn down every time), THE REQUIRED-PIN exception arm (a crash injected mid-ladder still stops the staged instance — try/finally posture), the foreign staging-port squatter honest failure (never killed — not ours), the owned leaked-residue preflight clear-then-stage, the staging bind-race ownership rung (a pre-existing owned zombie stealing the staging port during the launch window fails staging by name), cutover-retry exhaustion EMBEDS the recovery procedure (exactly one start + the ONE bounded retry, DARK named), the never-dark green deploy end-to-end at the install call site, and THE 2026-07-18 offline incident replayed end-to-end — a bad-env staged instance fails the real-key completion rung naming `no_db_connection`, the serving instance is never touched, no CONFIRMED, the failure hint carries the recovery procedure; plus the iteration-1–5 install-restart tests updated to the verify-then-swap contract — port-aware staging-green fakes, never-dark ordering pins (staging launch BEFORE any live stop), and the three repair-attempt gates re-keyed from `schtasks /end` evidence to staging-launch evidence with a NO-live-stop-on-staging-failure strengthening — no gate deleted; iteration 7 +14: the rich/legacy process-row parsers with the CIM ParentProcessId+CreationDate/ToFileTimeUtc column pin, `_instance_tree` on the exact deploy-6 forensic chain (root-first grouping, svchost never instance glue, an interactive shell never joins, no-table ⇒ one-node fail-open), the recycled-pid link severing + orphaned-chain rooting pure cases, THE deploy-6 sweep replay (cmd→litellm(30828)→python(54032, LISTENING): ZERO kills, relatives kept with the honest tree note, port never dark — retro-covering the iteration-5 kill of 81780/9716), the disjoint-zombie-tree sweep (owned members root-first, per-pid, no `/t`, serving tree untouched), the recycled-pid zombie NOT shielded by a false parent link at the sweep level, the cutover whole-tree root-first stop (`/t` on every member, pin 1's second direction), the teardown live-TREE immunity (a mid-staging-window worker of the listener survives; only the staged instance stopped), the `_config_model_routes` real-generator coupling pin (both auth modes, both providers, the catch-all pattern parsed), THE deploy-6 false-positive replay (23 wildcard-advertised expansions on a fresh instance NOT stale; a long-running instance's extra dynamic instantiations NOT stale), the genuinely-stale marker still failing under the catch-all (route contradiction where name matching is blind), the spawn-alias explicit-route precedence pin (the absorbed alias is a CONSISTENT catch-all expansion — only precedence catches it), the missing-explicit-group completeness arm (pin 2's escape), and the staging rung passing green end-to-end on a fresh wildcard-advertising staged instance; plus the deliberate-stop pins re-keyed to tree shape (`/t` + the tree-form kill hint) — no gate deleted), `tests/test_set_default_model.py` 44 → **48** (spawn-alias pin + registry key-set/dialect consistency pin, all-generations policy recognition, superseded-alias migration ×2, uniform-refusal ×3), `tests/test_sessionstart_run_continuity.py` 18 → **20** (spawn-state heal + whitespace-spawn fallback; serving-consistency extended to the spawn generation), `test_agents.py` `VALID_MODELS` + `claude-haiku-4-5`, + the dispatch-banner pin → 3.41.0. Suite **5542 → 5646 passing + 4 skipped** (199 test files by the `tests/test_*.py` disk-count basis, + the pytest-collected replication artifact at `tests/bug-fix-glm-secondary-route-fix/`), fully green on the release tree with the manifests at 3.41.0. Skill / agent / command counts UNCHANGED (48 / 39 / 23); **NO new skill / agent / command / hook / Layer-3 tool**; stdlib-only holds (`urllib` POST).
...
- **The `SECONDARY_PROVIDERS` registry (single-sourced in `scripts/setup/set_default_model.py`).** One dict entry per provider — `model` / `key_env` / `route_model` / `api_base` / `label`: `openai` → `gpt-5.6-sol` / `OPENAI_API_KEY` / `openai/gpt-5.6-sol`; `zai` → `glm-5.2` / `ZAI_API_KEY` / `openai/glm-5.2` with `api_base https://api.z.ai/api/paas/v4`. A future provider is ONE new entry — help text, prompts, key slots, and config routes all derive from the registry (extensibility test-pinned with a fake provider; no hardcoded name lists). Selection resolves flag > env > recorded state > grandfather > ask: `--secondary <provider>` / `CT6_SECONDARY_PROVIDER` / the choice REMEMBERED in `gateway.json` / the v3.39-era grandfather (an `openai_model`-only state counts as a recorded openai choice, so a pre-upgrade install NEVER sees a popup) / an interactive-TTY installer prompt (interrupt-at-prompt pinned to the v3.38.0 skip parity). The wrapper `commands/architect-team-setup.md` gains the **"Choose the secondary API (v3.40.0)"** AskUserQuestion section — a PRE-RUN gate that fires ONLY when the raw state records no choice (the resolved default is never mistaken for one); `--re-ask-provider` mirrors `--re-ask-keys`. `--zai-key` joins the v3.38.0 key machinery with FULL decline parity (`decline <anthropic|openai|zai>`); `--secondary-model` overrides the chosen provider's upstream id, with `--openai-model` kept as a deprecated openai-scoped synonym. Subscription mode honors the choice — the gateway serves the CHOSEN provider's model to direct callers; the split stays OFF; fable via sign-in unchanged. `gateway.env` key retention: switching providers never deletes the other provider's stored key (ADV3-2).
- **The provider-neutral alias.** The split's written model id becomes **`ct6-secondary`** (was `codex-5.6-sol`) and the policy string **`secondary-split`** (was `codex-split`); the new exported lever constants `POLICY_SECONDARY_SPLIT` / `POLICY_UNIFORM_FABLE` / `LEGACY_POLICY_CODEX_SPLIT` single-source the strings across the lever, the gateway installer, and the self-heal. READERS recognize BOTH alias generations (whitespace-trimmed); WRITERS emit only the new; the uniform `--model` lever refuses BOTH aliases; `--split secondary` is canonical with `--split codex` a deprecated synonym (`--secondary-model` likewise supersedes `--codex-model`). Ship state UNCHANGED — all 39 committed agents stay `model: fable`; `test_agents.py` `VALID_MODELS` gains `ct6-secondary` so the sanctioned split state stays valid-but-not-ship-state.
- **The adversarially-hardened migration (3 review rounds, FAIL → FAIL → PASS; 3 MAJOR field defects caught + fixed).** (ADV3-1) the SessionStart self-heal is now heal-to-RECORDED-alias — it restores the split to the alias `gateway.json` records as actually SERVED (`secondary_alias`, else the legacy `codex_alias`) and NEVER writes an alias the running gateway config doesn't route; a legacy v3.39-era state heals to `codex-5.6-sol` and keeps working, with an `install --activate` migration note (no recorded alias ⇒ no-op, never guess). (ADV3B-1) legacy migration fires on EVERY config-regenerating install regardless of `--activate`, with prior-state carry-forward — a plain re-install never silently deactivates or desyncs the recorded state from the config; uninstall stays the ONLY state downgrade. (ADV3-2) the `gateway.env` key-retention fix above. Plus: `status --live` probes the STATE-RECORDED alias (a working legacy install confirms green instead of false-failing on the new alias), provider-neutral registry-derived help/messages, carried-forward activation honesty rows, and a skipped-migration `warn` row.

Tests: `tests/test_install_gateway.py` 92 → **123** (provider selection ladder incl. grandfather + prompt + interrupt, zai config/route/api_base shape, key retention, migration-on-every-regen + carry-forward, recorded-alias live probe, `--re-ask-provider`, decline parity), `tests/test_set_default_model.py` 37 → **44** (registry shape + fake-provider extensibility, neutral-alias write/read-both/refuse-both, `--split secondary` + deprecated synonyms, constants), `tests/test_setup_install_fallbacks.py` 35 → **38** (`--secondary`/env pass-through, registry-derived help), `tests/test_sessionstart_run_continuity.py` 13 → **18** (heal-to-recorded-alias incl. legacy retention + no-alias no-op), `tests/test_commands.py` 56 → **58** (the wrapper's provider-choice section pins), `test_agents.py` `VALID_MODELS` + the dispatch-banner pin → 3.40.0. Suite **5494 → 5542 passing + 4 skipped** (199 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. Living specs 58 → **59** (new `secondary-provider-registry`; the `codex-role-split` requirement modified to the neutral alias). Skill / agent / command counts UNCHANGED (48 / 39 / 23); **NO new skill / agent / command / hook / Layer-3 tool**; stdlib-only holds.
...
- **Runtime split targeting.** New `set_default_model.installed_plugin_agents_dir()` / `runtime_agents_dir()`: resolve the installed architect-team plugin's `agents/` via Claude Code's `installed_plugins.json` (explicit path > `$CT6_PLUGIN_REGISTRY` > the real per-user registry), falling back to the repo `agents/` when no installed copy exists. `install_gateway.py`'s `_default_agents_dir()` (activate / status / uninstall) and `setup.py`'s `apply_model_policy` (`--codex`) now target the runtime dir — the split lands on the agents Claude Code actually spawns, the dev checkout stays on the committed ship state, and the split-step detail NAMES its target dir. Ship state UNCHANGED (all 39 committed agents stay `model: fable`).
- **Live split confirmation (the v3.38.1 lesson mechanized).** The v3.38.1 field bug proved a generated config can be broken while every install step reports ok. New confirm step: after a registered install, `confirm_gateway_serving` polls the LIVE gateway's `/v1/models` (bearer master key; injectable `_default_models_prober` seam; call-time-resolved retry constants) and asserts the ids the mode needs are actually SERVED — `codex-5.6-sol` always, `claude-fable-5` additionally in api-key mode. A stale process serving a pre-regeneration config gets ONE `restart_gateway` cycle (user-level: `schtasks /end`+`/run` with the detached-spawn fallback / `systemctl --user restart` / `launchctl kickstart -k` — never sudo) and a re-probe before the honest fail row + remediation. `status` gains `--live` (probe-only, no restart — status observes, install repairs); `setup_entry`'s row now says **"CONFIRMED live — CT6 runs the split"** (or the failure). Never gates; `--check-only` and `--no-register` never probe (the latter pins NO schtasks at all). This verifies what was just installed — model availability policy stays an INPUT, never probe-decided.
- **SessionStart self-heal (the split survives plugin updates).** A plugin update ships uniform-fable agent files into a fresh cache dir, silently reverting the split. `gateway.json` now records the desired `model_policy` (`codex-split` on an activated split; flipped to `uniform-fable` by uninstall so an uninstalled split is never resurrected), and `hooks/sessionstart-run-continuity.py` gains `maybe_heal_model_split()`: when the state wants the split AND the hook runs from an INSTALLED plugin copy (under `~/.claude/plugins/` — a dev checkout is NEVER rewritten) whose `agents/` drifted, it re-applies the split via the plugin's own lever and notes the heal in the session context. Fail-open on every path; no new hook wiring (the existing SessionStart entry).
- **Hermeticity (the v3.35.0 ambient-leak lesson, applied globally).** New `tests/conftest.py` autouse `_scrub_plugin_registry` points `CT6_PLUGIN_REGISTRY` at a nonexistent file for EVERY test — an unscrubbed resolution would let any model-policy test rewrite the REAL installed plugin copy mid-suite. `test_install_gateway.py` gains the autouse `_stub_live_probe` (no test ever opens a real socket — a REAL gateway may be listening on the default port — or sleeps the retry backoff).

Tests: `tests/test_set_default_model.py` 29 → **37** (resolver semantics incl. the malformed-registry + hermetic-under-suite pins), `tests/test_install_gateway.py` 79 → **92** (installed-copy targeting incl. repo-untouched, confirm ok/fail/restart-recovery/unreachable, subscription expects-codex-only, check-only/no-register never probe, status `--live`, per-OS restart never-sudo, uninstall state flip, the setup_entry one-call summary), `tests/test_setup_install_fallbacks.py` 34 → **35** (runtime targeting via `apply_model_policy`), `tests/test_sessionstart_run_continuity.py` 8 → **13** (heal / dev-checkout guard / state gates / idempotence / garbage-state fail-open), + the dispatch-banner pin → 3.39.0. Suite **5467 → 5494 passing + 4 skipped** (200 test files). Skill / agent / command counts UNCHANGED (48 / 39 / 23); NO new skill / agent / command / hook / Layer-3 tool; stdlib-only holds (`urllib`).

## [3.38.1] — 2026-07-16 — gateway explicit Anthropic routes (field-verified SR PATCH)

**PATCH — the generated LiteLLM config's `"*"` catch-all was field-observed BROKEN; every known Anthropic model id now gets an explicit route.** A real Windows deploy (2026-07-16) hit it live: with only the codex route + the `model_name: "*"` → `anthropic/*` catch-all, the gateway exposed just `codex-5.6-sol` and rejected every Anthropic id — Claude Code surfaced `API Error: 400 ... Invalid model name passed in model=claude-fable-5. Call /v1/models` even though the key itself listed `claude-fable-5` (verified directly against the Anthropic API). The operator's hand-fix — explicit per-model routes added before the catch-all, gateway restarted, fable + opus verified end-to-end through the gateway — is folded into the GENERATOR (SR-gateway-wildcard-route), because `config.yaml` is a generated file whose header says "edit by re-running the installer": without this fold-in, the next installer run would have silently reverted the hand-fix and re-broken fable.

`scripts/setup/install_gateway.py`: new `ANTHROPIC_EXPLICIT_MODELS` tuple (`claude-fable-5` first — the plugin default; `claude-opus-4-8` second — the implemented fallback; + sonnet-5 / opus-4-7 / opus-4-6 / sonnet-4-6 / haiku-4-5, the ids a real key listed via `/v1/models`); `build_gateway_config` in api-key mode now emits one explicit route per id BEFORE the `"*"` catch-all (kept as a best-effort tail for ids that ship after the list was written). Subscription mode is UNCHANGED — still zero Anthropic routes (fable stays on Claude Code's native sign-in auth). Secrets posture unchanged (env references only).

Tests: `tests/test_install_gateway.py` +2 (`explicit routes present + fable-before-catch-all ordering` in api-key mode; `zero Anthropic routes` in subscription mode) — 77 → 79. Suite **5465 → 5467 passing + 4 skipped** (199 test files; 5471 collected — canonical Windows-with-PyYAML; Linux-with-PyYAML 5469 + 2 off the same total). NO other change; skill / agent / command counts UNCHANGED (48 / 39 / 23).

## [3.38.0] — 2026-07-15 — setup asks for missing keys (ask-then-apply, never punt-to-script)

**MINOR — when setup needs an API key only the user holds, it ASKS for it in the flow and applies it itself.** The owner directive (2026-07-15, refined to an A-grade brief through two proposal-refiner iterations): everything the setup flow runs must be automatic — a missing key must be prompted for in-flow, never punted to a "run this script yourself" remediation as the only path. The observed gap was live: `/architect-team:architect-team-setup --external-llm --yes` with no `ANTHROPIC_API_KEY` landed in subscription mode with a punt; a missing `OPENAI_API_KEY` landed provisioned-but-NOT-enabled with a punt; `install_librarian.py` punted the same way. The v3.36.0/v3.37.0 printed-remediation behavior was working as designed; this change adds the missing interaction capability ON TOP of it.

- **`scripts/setup/install_gateway.py` — the installer-level prompt + the decline record.** `_prompt_for_key` is an injectable hidden-entry stdin seam (default `getpass.getpass`; degrades to visible `input()` with a one-line warning only where hidden entry is unachievable): it fires ONLY when the run is interactive, stdin is a TTY, the slot's key did not resolve (args > env > gateway.env), and the slot is not declined; blank entry skips without error; `(EOFError, KeyboardInterrupt)` → skip (the in-run parity remediation — an interrupted prompt behaves like a blank, never a crash). A captured key folds into the existing `write_env_file` path, so 0600 / last-4 masking / the `config.yaml` `os.environ/...` references are inherited unchanged. NEW per-key decline record `key-declines.json` in the state dir (`{"<slot>": {"declined_at", "via": "wrapper|prompt-skip"}}`): a wrapper decline and a blank-to-skip both record; the record AUTO-RESETS when the slot's key later resolves on any path; `--re-ask-keys` clears it; a NEW `decline <anthropic|openai> [--clear]` subcommand is the wrapper's deterministic record/clear channel (kept out of `install` flags so a decline stays an explicit, auditable action). `--interactive-prompts` plumbs interactivity; `status` reports `declined=<slots>` (the decline suppresses the PROMPT, never the truth); `uninstall --purge` removes the record with the state dir.
- **`scripts/setup/setup.py` — the interactivity pass-through.** The `--external-llm` path passes interactivity through to the installer's prompt seam ONLY when the setup run is itself interactive: NOT `--yes`/`CT6_SETUP_ASSUME_YES`, NOT `--check-only`, NOT `--no-prompt`, and stdin a real TTY (mirroring the existing Agent-Teams settings.json consent-prompt house pattern). The never-gates posture holds verbatim — a raising prompt degrades to the existing `warn` row and setup completes.
- **`scripts/setup/install_librarian.py` — full parity for its `anthropic` slot.** The same seam (hidden-getpass `_prompt_for_key`, blank-to-skip, `(EOFError, KeyboardInterrupt)` → skip), the same `key-declines.json` record in `~/.architect-team/librarian/` with auto-reset + `--re-ask-keys`, the same `decline [--clear]` subcommand, `status` decline honesty, and `uninstall --purge` symmetry. A captured key enables the daemon path exactly as `--enable` with a key does.
- **The wrapper half — `commands/architect-team-setup.md` + `commands/librarian-install.md`.** The slash-command flow runs the installers through Bash with no usable stdin, so BOTH wrappers gain an **"Ask for missing keys — never punt (v3.38.0)"** section: on an absent-key state (`ANTHROPIC_API_KEY` absent in subscription mode / `OPENAI_API_KEY` absent in provisioned-but-NOT-enabled / the librarian's no-key install) with no recorded decline, the executing agent MUST AskUserQuestion with exactly two dispositions — **capture-and-apply** (the agent itself re-runs the installer with `--anthropic-key`/`--openai-key`, or the librarian's env-scoped `--enable` run; a `--yes`/`CT6_SETUP_ASSUME_YES` on the ORIGINAL setup invocation carries over as `--activate` consent — the existing `setup_entry` convention, no new consent state invented) or **explicit decline** (recorded via the `decline` subcommand). The wrapper consults `status` `declined=` FIRST (a declined slot is not re-asked absent an explicit re-ask signal, which maps to `install --re-ask-keys`) and NEVER presents the bare run-this-script remediation as the only path. Subscription mode remains a first-class deliberate outcome on an `anthropic` decline.
- **The REQ-005 setup-surface sweep — all 11 `scripts/setup/*.py` dispositioned** (the table lives in the archived change's `design.md`): `install_gateway.py` + `install_librarian.py` **qualify** (user-holdable keys → ask-then-apply, this change); `setup.py`'s own remaining punts examined-and-excluded (plugin installs / npm EACCES / PEP-668 environment fixes / version upgrades — none user-holdable input; its Agent-Teams consent prompt ALREADY asks, and its `--external-llm` path now passes interactivity through); `install_mempalace.py` / `set_default_model.py` / the 5 non-installer helpers (`teams_mode.py`, `worktree_paths.py`, `worktree_lifecycle.py`, `agent_resume.py`, `sync_agent_boilerplate.py`) + `agent_boilerplate_blocks.py` examined-and-excluded (no user-holdable secret input; the 11th row added at Phase 5 on the test-completeness verifier's advisory so the sweep claims EVERY `scripts/setup/*.py` explicitly). Non-user-holdable remediations deliberately STAY printed.

**Postures preserved verbatim:** never-gates-setup (warn-degrade), raw keys ONLY in the 0600 env files — never echoed by the scripts (the prompt itself is hidden entry; every report line masks to last-4), consent-gated activation (the `--yes` → `--activate` carry-over reuses the existing convention), uninstall symmetry (now including the decline record under `--purge`), and subscription mode first-class. Known accepted trade-off (owner-accepted at refinement): a key captured via the WRAPPER path appears once, raw, in the session transcript; the scripts still mask everywhere and store raw only in the env file.

**Review provenance (producer≠checker throughout):** independent task-reviewer pass ×2, adversarial-reviewer pass ×2 (24 probes + `verify-no-fake-data` clean ×2), test-completeness pass (backend-integration CLEAN, 0 vacuous flows), master-review audit PASS; one in-run remediation (the gateway prompt's interrupt parity — `(EOFError, KeyboardInterrupt)` → prompt-skip, +2 tests) and one in-run advisory closure (the sweep's 11th file). No SRs opened all run.

Tests: `tests/test_install_gateway.py` 47 → **77**, `tests/test_install_librarian.py` 23 → **39**, `tests/test_setup_install_fallbacks.py` 31 → **34**, `tests/test_commands.py` 49 → **56** (wrapper-section pins), + the dispatch-banner pin → 3.38.0 — every prompt/decline test hermetic via injectable input/isatty/getpass seams (no test ever blocks on real stdin or touches the real home). Suite **5409 → 5465 passing + 5 skipped** (200 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. Living specs 57 → **58** (new `setup-key-prompting`). Skill / agent / command counts UNCHANGED (48 / 39 / 23); **NO new skill / agent / command / hook / Layer-3 tool**; stdlib-only holds (`getpass` is stdlib).
...
- **`register_gateway` — user-level on every OS, never sudo/admin.** Windows: direct `schtasks /create /tn ct6-llm-gateway /tr "<launcher>" /sc onlogon /f` as the current user + `schtasks /run` to start it NOW (deliberately NOT the XML descriptor — its boot trigger needs admin; onlogon is semantically right for a per-user gateway); **when schtasks is itself denied — OBSERVED on this run's real non-elevated Windows 11 shell — it falls back to a Startup-folder shim** (`%APPDATA%\…\Startup\ct6-llm-gateway.cmd`, a plain file write that needs no privilege at all) **and starts the launcher through a DETACHED spawner** (`_spawn_detached`: DEVNULL handles + its own minimized console — a captured-pipe `subprocess.run` inherited into the daemon and blocked the install until the gateway exited, the second live-dogfood lesson). Linux: the `bg_runtime.systemd_unit` rewritten `multi-user.target` → `default.target` and written to `~/.config/systemd/user/`, then `systemctl --user daemon-reload` + `enable --now`. macOS: a `launchd_plist` into `~/Library/LaunchAgents/` + `launchctl load -w`. `unregister_gateway` is symmetric (stop, then remove task AND shim; an absent registration is a tolerated no-op) and `is_gateway_registered` (task OR shim) feeds `status` (new `registered=` field in the report/state/JSON).
...
Tests: `tests/test_install_gateway.py` 35 → **46** (per-OS registration shapes incl. the never-sudo/user-target pins, failure detail, absent-task no-op, install auto-registers-when-enabled / `--no-register` / not-enabled-defers / failure-degrades, uninstall stops-then-deletes, status `registered=`), + the dispatch-banner pin → 3.37.0. Suite **5397 → 5408 passing + 5 skipped** (200 test files), both encodings. Counts UNCHANGED (48 / 39 / 23); NO new skill / agent / command / hook / Layer-3 tool.

## [3.36.0] — 2026-07-15 — external-LLM gateway (out-of-the-box OpenAI/Codex via LiteLLM, subscription-aware)

**MINOR — enabling external LLM usage is now one setup flag, and it never breaks the Anthropic side.** The owner directive (verbatim intent): wire the LiteLLM installer into the auto-setup so external-LLM usage works *out of the box* when enabled; and when enabling it, *allow the Claude subscription to be used if there is no Anthropic API key* — fable must be usable either via the sign-in or via an API key.

**The installer — new `scripts/setup/install_gateway.py` (stdlib-only; mirrors `install_librarian.py`).**
- **What it provisions:** the MIT-licensed LiteLLM proxy (`pip install "litellm[proxy]"` through setup.py's existing PEP-668-aware ladder — uv → `pip --user` → `--break-system-packages` — with `--only-binary litellm` riding every rung: litellm's sdist grew a Rust python-bridge whose build needs a full MSVC/cargo toolchain, observed failing on a REAL Windows install with ``linker `link.exe` not found``; forcing the wheel makes the resolver backtrack to the newest version that ships one) + a local gateway state dir at `~/.architect-team/gateway/` (`$CT6_GATEWAY_HOME` override): `config.yaml` (the model routes, secrets as `os.environ/...` references only), `gateway.env` (the ONLY raw-key location, chmod 0600 best-effort, never under the repo), a per-OS launcher (`run_gateway.bat` / `run_gateway.sh` — loads gateway.env then execs the proxy), `gateway.json` (non-secret state), and the per-OS boot descriptor via the reused `services/common/bg_runtime.py` `install_descriptor` (register hint PRINTED, never executed — the librarian posture).
- **The route that gives the v3.35.0 split a backend:** `codex-5.6-sol` (the SAME alias `set_default_model.py` writes — pinned by a cross-module test) → `openai/gpt-5.6-sol` (the id OpenAI's live `/v1/models` registry serves Codex 5.6 (sol) under, verified at authoring; `--openai-model <id>` override; the id is written at install, never probed) with `os.environ/OPENAI_API_KEY`; plus an always-on generated master key (`CT6_GATEWAY_MASTER_KEY`) so the local proxy never runs unauthenticated.
- **Two AUTH MODES, resolved from key presence (never a live probe — the `resolve_model` convention):** `api-key` (an `ANTHROPIC_API_KEY` resolves → the config adds a catch-all `"*"` → `anthropic/*` route so the gateway fronts BOTH providers; `--activate` then writes `ANTHROPIC_BASE_URL` + `ANTHROPIC_AUTH_TOKEN` into `~/.claude/settings.json` — the merge-preserving teams-mode write posture — AND applies the codex role split via the reused lever) and **`subscription` (the owner-directed no-key path: fable and every Anthropic model keep Claude Code's native sign-in auth — `ANTHROPIC_BASE_URL` is deliberately NEVER written, the codex split stays OFF (the harness cannot split-route only codex traffic through a proxy while sign-in auth serves the rest — an honest, documented boundary), and the gateway still serves OpenAI models to direct callers; the printed remediation is add-a-key + re-run `--activate`)**. Either way fable stays usable — sign-in or API key.
- **Secret hygiene:** keys resolve `--openai-key`/`--anthropic-key` > process env > the existing gateway.env; raw keys appear ONLY in gateway.env; every report masks to the last 4 chars; `config.yaml` / `gateway.json` / the launcher / the descriptor / stdout are pinned key-free by tests. No OpenAI key ⇒ an honest provisioned-but-NOT-enabled state with the remediation (the librarian no-key posture).
- **Lifecycle symmetry:** `status` (mode / masked keys / activation / the agents' model-policy state via the lever), `uninstall` (removes OUR settings.json entries ONLY when `ANTHROPIC_BASE_URL` still points at this gateway's URL — a user-customized value is never clobbered; restores uniform fable ONLY when the on-disk policy state is `codex-split` — a manual Opus state is left untouched; removes the descriptor + prints the unregister hint; `--purge` removes state), `--check-only` on BOTH install and uninstall provisions/touches nothing.

**The deploy — `setup.py` grows `--external-llm` / `--no-external-llm` + the `CT6_EXTERNAL_LLM` tri-state (v3.36.0).** The same signal convention as the codex split: the explicit disable flag beats the env var; truthy ⇒ install (activation consent-gated behind the existing `--yes` / `CT6_SETUP_ASSUME_YES`); SET-but-falsy ⇒ uninstall; ABSENT ⇒ nothing touched + an informational `check_external_llm_option()` note row. `apply_external_llm_policy` NEVER gates — any installer failure degrades to a `warn` row carrying the manual remediation (the `apply_model_policy` posture). `commands/architect-team-setup.md` gains the `## External LLM usage — the LiteLLM gateway (v3.36.0)` section + the flags in `argument-hint`.

**Hermeticity (the v3.35.0 ambient-leak lesson, applied at authoring time):** the `test_setup_install_fallbacks.py` autouse scrub extends to `CT6_EXTERNAL_LLM` (+ explicit pops in the two non-check-only e2e `main()` tests) — a leaked truthy signal would otherwise route a real `pip install litellm` + `~/.architect-team/gateway` write into the suite; the new test file scrubs `CT6_EXTERNAL_LLM` / `CT6_CODEX_56_AVAILABLE` / `CT6_GATEWAY_HOME` / both API-key vars autouse. **Folded fix (surfaced by the REAL install on this run's machine):** `tests/test_pretool_unilateral_override_guard.py::test_find_workspace_returns_none_when_no_state` latently assumed no ancestor of pytest's tmp dir carries `.architect-team/` — but per-user CT6 state at `~/.architect-team/` (sanctioned since the v3.29.0 librarian install, now also the gateway) makes the profile dir a REAL workspace ancestor on any machine with it installed; the test now asserts the walk finds nothing INSIDE its controlled subtree instead of a global None (the guard's behavior was correct all along). Two more real-Windows lessons baked in from the live dogfood: `install_gateway.py`'s `main()` gets setup.py's cp1252 stdout-reconfigure guard (the masked-key ellipsis + pip output crashed a legacy-codepage console), and the generated launchers set `PYTHONUTF8=1` (litellm's own startup banner is not cp1252-encodable and crashed app startup without it).

Tests: new `tests/test_install_gateway.py` (35 offline cases — signal tri-state incl. setup/installer agreement, base-dir + key precedence + master-key stability, auth-mode resolution, per-mode config shape (subscription = NO anthropic route), the codex-alias cross-module pin, launcher per-OS, settings.json merge/remove/never-clobber, install lifecycle incl. provisioned-but-disabled + raw-key-nowhere-but-gateway.env + check-only-writes-nothing + injected-pip-ladder + pip-failure-degrades, activation both modes incl. the subscription-mode refusal, uninstall symmetry incl. manual-Opus-untouched, status masking, `setup_entry` note/hint/never-raises + setup `main()` routing + mutual exclusion + never-gates), + the `tests/test_dispatch_banner.py` release-version pin → 3.36.0. Suite **5362 → 5397 passing + 5 skipped** (200 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. Skill / agent / command counts UNCHANGED (48 / 39 / 23); ship state unchanged (all 39 agents still commit `model: fable`); **NO new skill / agent / command / hook / Layer-3 tool**.
## [3.35.1] — 2026-07-14 — doc-currency + code/test-hygiene sweep (review-driven PATCH)

**PATCH — a 3-lens review (code cleanliness / documentation currency / test-suite health, plus the repo's deterministic self-checks) over the whole repo, with every verified finding fixed.** No feature, no behavior change; the deterministic gates were green before and after (instruction-compliance lint 0 findings / 113 files, `check_separation()` clean, closeout engine current).

**Documentation currency (the one repeating defect class: prose test-counts that badge/header refreshes missed).** README version badge 3.34.0 → current; README "Tests validate" prose `5263` → current; `docs/CODEBASE_MAP.md` System-Overview `5263` → current + its `set_default_model.py` description now names the v3.35.0 role split (it still said uniform-only); the map's Currency note advanced v3.34.0/5334 → current; `docs/INTEGRATION_MAP.md` suite-total sentence advanced; the two self-descriptions of the frozen v2.8.0 §4 baseline harmonized (13-vs-14-commands internal contradiction → the intro blockquote's figures).

**Code hygiene (scripts/hooks/services sweep — zero TODO/FIXME markers found; every flagged item fixed).**
...
- **Truthy-env parsing consolidated**: three same-tier copies (`setup.py`, `teams_mode.py`, `set_default_model.py`'s bare set) collapse onto `teams_mode.py`'s `_TRUTHY_VALUES` / `_is_truthy` — upgraded to the isinstance-guarded form so a non-string settings.json value is a safe False instead of a crash; `setup.py` + `set_default_model.py` import it (dual-form, importlib-load-safe).
- **R1a consolidation finished**: `hooks/vao_tools.py`'s private `_load_json` now delegates to `hooks/shared_util.load_json` (the fail-closed default it always had) — the one consumer the v3.10.0 helper unification left behind.
...
**Test-suite health.**
- **`tests/test_mini_run_trailer_audit.py` DELETED** — a permanently-skipping stub: it probed `pipeline-completion-audit.py` for commit-classifier functions (`is_pipeline_commit` …) that provably never existed (the hook audits run-marker JSON, not commit messages), so it green-passed as a skip while providing zero coverage. Test files 199 → **198**; the canonical skip set 5 → **4**.
- **Shared module loader**: new `tests/helpers/module_loader.py::load_module` replaces the per-file `spec_from_file_location` boilerplate — **67 loader sites across 55 test files** migrated mechanically (semantics identical: fresh module, never registered in `sys.modules`); the 16 files with genuinely special import shapes (several deliberately test import mechanics) keep their inline forms.
- **Drift pins single-sourced**: new `tests/helpers/pins.py` (`EXPECTED_EVIDENCE_FIELD_COUNT = 17`, `EXPECTED_NOTIFY_EVENT_COUNT = 10`) replaces the same magic numbers duplicated across 5 evidence-schema files + 2 notify files — still a deliberate literal tripwire, now a one-line edit. The `test_dispatch_banner.py` release-version pin stays a literal BY DESIGN (it is the did-you-bump-the-version tripwire that has caught two real misses) and advances to 3.35.1.
- **Recorded deferrals (deliberate, not omissions):** the three >900-line modules (`worktree_lifecycle.py` 1502 / `hooks/locks.py` 1106 / `setup.py` 1042) are NOT split this run — `locks.py` is concurrency-critical code freshly race-fixed (v3.32.0 RT-1…5), and pure-structure churn on run-machinery modules is regression risk with no behavioral gain; the sanctioned approach when a dedicated run takes it on is the existing facade+family pattern (`vao_tools.py` → `hooks/vao/`). Likewise the worktree test family's per-file `git init` cost (suite ≈ 42s total) is noted, not restructured.

Suite **5362 passing + 4 skipped** (198 test files; 5366 collected — canonical Windows-with-PyYAML; Linux-with-PyYAML reports 5364 + 2 off the same total; passing count unchanged from v3.35.0 — the delta is the deleted always-skip stub), green under both Windows cp1252 and `PYTHONUTF8=1`. Skill / agent / command counts UNCHANGED (48 / 39 / 23); **NO new skill / agent / command / hook / Layer-3 tool; NO behavior change**.

## [3.35.0] — 2026-07-14 — Codex 5.6 model role split (availability-gated, managed by setup)

**MINOR — with Codex 5.6 in the harness, the agent models split by role; without it, nothing changes.** The owner directive (verbatim intent): *if we have Codex 5.6*, Fable drives **all architecture and control and design agents** and Codex 5.6 (sol) drives **all development agents and code-checking and testing agents**; setup is managed by the skill and deploying it must be simple; *if this isn't available, it defaults to the current operating model (fable, and opus where needed)*.

**The engine — `scripts/setup/set_default_model.py` grows the role-split policy (the uniform lever is preserved verbatim).**
- **The classification (18 fable / 21 codex) — adversarially re-derived.** New `AGENT_ROLES` maps every one of the 39 agent stems into two buckets, and the split was re-derived by THREE independent classifiers (role-first / pipeline-position / family-consistency lenses) over the full agent bodies, with majority votes adjudicating the hard calls. `architecture-control-design` (stays on `fable`, 18): architecture — `system-architect`, `structure-analyst`, `route-mapper`, `endpoint-tracer`, `integration-explorer`, `master-synthesizer`, `codebase-map-reviewer`; control — `bug-classifier`, `prompt-refiner`, `scaffold-agent`, `doc-updater`, `closeout-agent`; design + research — `oracle-deriver`, `interaction-intuiter`, `interaction-observer`, `domain-researcher`, `diagnostic-researcher`, `mcp-design-agent`. `development-checking-testing` (takes `codex-5.6-sol`, 21): development — `backend`, `frontend`, `integration`, `reconciler` (hands-on merge-conflict resolution on built code); code checking — `adversarial-reviewer`, `task-reviewer`, `test-completeness-verifier`, `editability-reviewer`, `interaction-reviewer`, `fix-sensibility-checker`, `reference-tracer` + `structure-adversary` (mechanical code-reference closure + code-search refutation); testing — `qa-replayer`, `mini-qa`, `flow-explorer` + `flow-executor` (test-flow design + execution inside the testing pipeline), `bug-replicator`, `test-run-watcher`, `monitor-synthesizer`, `visual-capture`, `visual-analyzer`. Recorded hard calls: `diagnostic-researcher` lands fable (root-cause RESEARCH feeding the architect — the researcher family), and the doc-currency writers (`doc-updater` / `closeout-agent`) stay fable per the engine's own fail-safe (documentation, not product code). An UNCLASSIFIED stem (e.g. a newly scaffolded agent) fails safe to the fable bucket — never to codex — and `--check` surfaces it as `unclassified`.
- **The policy functions + CLI.** `apply_split` (per-role frontmatter rewrite — idempotent, line-ending-preserving, only the `model:` line, exactly the uniform lever's write discipline), `apply_policy(agents_dir, codex_is_available)` (available ⇒ `codex-split`; unavailable ⇒ `uniform-fable`, the current operating model), `policy_state` (`uniform-<m>` / `codex-split` / `mixed`), `codex_available` (truthy check) + `codex_signal_from_env` (the TRI-STATE `CT6_CODEX_56_AVAILABLE` read: truthy ⇒ available, SET-but-falsy ⇒ explicitly unavailable, ABSENT ⇒ no signal), `split_targets` / `role_for` / `unclassified_stems`. CLI: `--split codex` (apply directly), `--auto` (tri-state: truthy env ⇒ the split; set-but-falsy ⇒ uniform fable; ABSENT ⇒ **no-op** — a manually applied lever state such as the Opus fallback is never silently clobbered), `--check` (now reports the policy state + unclassified stems), `--codex-model <id>` (override for a harness that registers Codex 5.6 under a different id). The uniform `--model` lever REFUSES the codex id — codex never applies uniformly (architecture/control/design must stay on fable); and the Opus fallback for a harness that predates the `fable` alias remains the untouched `--model opus` uniform lever.
- **Availability is an INPUT, never probed** — the same injected-availability convention as `service_config.resolve_model` (and the same SETUP-ADV-1 reasoning that dropped the fable version gate as false precision: harness model availability is not knowable from a stdlib script).

**The deploy — managed by the setup skill, one flag (`commands/architect-team-setup.md` + `scripts/setup/setup.py`).**
- `setup.py` gains `--codex` / `--no-codex` (mutually exclusive) + the `CT6_CODEX_56_AVAILABLE` env var. `resolve_codex_signal` precedence: the explicit `--no-codex` beats the env var; `--codex` or a truthy env var asserts availability; a SET-but-falsy env var is an explicit unavailability assertion (⇒ restore uniform fable); **the var absent resolves to None and the model state is left UNTOUCHED** (the shipped uniform-fable default IS the operating model, and a user's manual lever state is never silently clobbered) — setup then surfaces the option as an informational `check_codex_option()` note row carrying both levers. With a signal, `apply_model_policy` drives the lever (`_load_model_lever` importlib-loads the sibling so it works however setup.py itself was loaded): `--check-only` reports the current + would-be policy and writes nothing; a missing agents dir is a `warn` row naming the path (never a success row claiming a 0-file split); a normal run applies the split (or restores uniform fable) and reports `applied`, or `present` when already compliant. **Never gates:** any lever failure degrades to a `warn` row carrying the manual `--split codex` remediation; the existing `check_model_default()` note row and every pinned setup behavior are preserved verbatim (`allow_abbrev=False` makes flag abbreviations deliberately unsupported now that `--no-codex` would shadow the historical `--no` ⇒ `--no-prompt` abbreviation).
- `commands/architect-team-setup.md` gains the `## Model policy — the Codex 5.6 role split (v3.35.0)` section (the one-flag deploy, the 18/21 split summary, the determining-availability instruction, the four lever one-liners, the `--codex-model` escape hatch, the unclassified-fails-safe note) + `--codex`/`--no-codex` in `argument-hint`.

**Adversarial review (producer≠checker) — FIX-FIRST, remediated.** Three independent classifiers re-derived the role split from the full agent bodies (majority votes flipped 5 assignments from the producer's draft: `reconciler` / `reference-tracer` / `structure-adversary` / `flow-explorer` → codex, `diagnostic-researcher` → fable), and three adversarial reviewers + a skeptic verification pass attacked the diff. One MAJOR was CONFIRMED by concrete reproduction and fixed: with `CT6_CODEX_56_AVAILABLE=1` ambiently exported (the documented deploy configuration), the two PRE-EXISTING end-to-end `setup.main()` tests routed the truthy signal into a REAL `apply_model_policy` write and rewrote 18 git-tracked `agents/*.md` mid-suite, cascading ~20 downstream failures — fixed with a module-wide autouse `CT6_CODEX_56_AVAILABLE` scrub + explicit pops in both tests (the suite is hermetic w.r.t. the deploy var again). Four MINOR fixed: the `--auto` absent-env branch clobbered a manually applied Opus fallback back to fable (now the tri-state no-op above); `apply_model_policy` claimed success on a missing agents dir (now a `warn` naming the path); `tests/test_agents.py` `VALID_MODELS` rejected the sanctioned post-split state as INVALID frontmatter rather than merely non-ship-state (codex id added; the uniform-fable ship pin unchanged); argparse prefix abbreviations silently broke (`--no` became ambiguous — now deliberately unsupported via `allow_abbrev=False`). Plus doc-count/stamp corrections from the consistency reviewer.

**Ship state unchanged.** All 39 committed `agents/*.md` still carry `model: fable` — the split is an install-time policy applied on the user's machine (exactly like the Opus fallback), so `tests/test_agents.py`'s uniform-fable pin and all ~38 per-agent model pins hold verbatim. The instruction-compliance lint stays at zero findings (113 files).

Tests: `tests/test_set_default_model.py` +18 (role-map completeness pinned against the real `agents/` dir + bucket disjointness, split apply / spot checks / idempotence / only-the-model-line, `apply_policy` both branches, `policy_state` transitions, `codex_available` + tri-state `codex_signal_from_env` parsing, unknown-stem fail-safe, `--check` unclassified surfacing, CLI `--split` / `--auto` truthy+falsy / `--auto` absent-env no-op / policy-state reporting / `--codex-model` override, uniform-codex refusal), `tests/test_setup_install_fallbacks.py` +10 (`resolve_codex_signal` precedence incl. set-but-falsy, the option note, check-only-writes-nothing, apply/restore, the never-gates warn degrade, missing-agents-dir warn, main() routing for flag / env / no-signal, + the autouse hermeticity scrub), + the `tests/test_dispatch_banner.py` release-version pin → 3.35.0. Suite **5334 → 5362 passing + 5 skipped** (199 test files; 5367 collected — canonical Windows-with-PyYAML; Linux-with-PyYAML reports 5364 + 3 off the same total), green under both Windows cp1252 and `PYTHONUTF8=1`. Skill / agent / command counts UNCHANGED (48 / 39 / 23); **NO new skill / agent / command / hook / Layer-3 tool**.

## [3.34.0] — 2026-07-13 — informative run notifications (emails that carry the plan, the progress, and the waits)

**MINOR — the project email notifier goes from bare status pings to informative run coverage, on every architect-team task.** The owner directive (verbatim intent): emails must *always* be sent as the agent engages in ANY architect-team task; they must carry *meaningful* updates — what is about to start, where the run is in the process, and what it completed; the kickoff email must include *the architecture and solution plan as part of one email*; and the run must alert when it is *waiting on agents* and again when *it completes*. Before this release the notifier had six events with one-line bodies ("has started Phase 2"), no run-level bookends, no dispatch-wait visibility, no plan delivery — and the ux-test pipeline had ZERO notification wiring.

**The engine — `scripts/notify/notify.py` `EVENT_TYPES` 6 → 10 (REQ-001).**
- **`run_start` — the kickoff email that carries the plan itself.** Fires ONCE per run at the moment the architecture + solution plan first exists. A new repeatable `--plan-file <path>` flag embeds each plan artifact (proposal.md / design.md / tasks.md — or the bug-fix proposal / the ux-test distilled flow catalog) under its own filename header IN the email body — the plan arrives in ONE email, not as a pointer. Bounded: `PLAN_FILE_MAX_CHARS = 20_000` per file with an explicit `[truncated for email]` marker, `MAX_PLAN_FILES = 8` with an omission note; a missing/unreadable file degrades to a one-line could-not-be-read note — `_read_plan_file` NEVER raises (best-effort preserved).
- **`waiting_on_agents` / `agents_complete` — the dispatch-wait pair.** New `--agents` flag carries the roster: `waiting_on_agents` names each dispatched agent + its mission the moment the Lead enters a wait; `agents_complete` reports per-agent outcomes when the dispatch fully returns.
- **`run_complete` — the run's final email.** Run id + `--elapsed` + `--commit <final SHA>` (both optional lines omitted when absent, no placeholder noise) + what shipped via `--details`.
- **Universal informative blocks on EVERY event (REQ-002):** `--details` (what is about to happen / what was accomplished), `--progress` (where the run stands), `--next-step` (what follows) render as their own body blocks (`Details:` / `Where the run stands:` / `Up next:`) on all ten events, omitted when absent — heartbeat emails become informative too. Every existing subject/body substring, the opt-in / best-effort / always-exit-0 contract, stdlib-only, and secrets-via-env-indirection are preserved verbatim.

**The canonical content contract — CPC `## Notifications wiring convention` (REQ-003).** The section is rewritten around the ten-event vocabulary (a per-event table with fire-points + key flags) plus the new **"Informative, not just status"** rule: every `phase_start` passes `--details` with what the phase is about to do *for this run* + `--progress` (+ `--next-step`); every `phase_complete` passes what the phase actually accomplished; the FIRST `phase_start` of a run carries the requirement summary — the **engagement email** — so recipients learn a run began *and what it is for* the moment the pipeline engages; a bare status-only invocation is non-compliant wiring (heartbeat excepted). Run-level bookends (`run_start` at Phase 1 / B3 / M3 / U4; `run_complete` at Phase 8 / B8 / M7-green / U9) and the dispatch-wait rule (at EVERY dispatch-and-wait point, named or not) live here canonically.

**The wiring — all FOUR pipelines (REQ-004…007).**
- **`skills/architect-team-pipeline/SKILL.md`:** ten-event `## Notifications` section + informative templates; `run_start` inline at the end of Phase 1 (the OpenSpec bundle validates → ONE email embedding proposal + design + tasks); `waiting_on_agents` bash at the Phase 2 team spawn + `agents_complete` bash at the Phase 3 gate-all-passed moment (with `--agents` rosters); the pair also named at Phase −1B / 3b / 6 dispatch anchors; `run_complete` as a new Phase 8 subsection right after `--mark-complete`, before the compact prompt.
...
- Tests: new `tests/test_claude_design_import.py` (47 offline tests — detection on both trigger forms, `?file=` / `Implement:` parsing, path-safety rejection of absolute + `..` paths, materialize + import via `FakeClaudeDesignSource`, the instruct-then-fallback planner). Suite **5263 passing + 5 skipped** (199 test files) on the canonical Windows-with-PyYAML environment (5268 collected; the without-PyYAML environment, e.g. macOS, reports **5264 passing + 4 skipped** off the same 5268 total). Skill count **47 → 48** (new `claude-design-import`); **agents 39 / commands 23 UNCHANGED**; **NO new command / agent / hook / Layer-3 tool**.

## [3.32.0] — 2026-07-06 — Fable 5 default + first-install setup hardening + two SR-driven infrastructure fixes

**MINOR — make Fable 5 the default model everywhere with an implemented Opus 4.8 fallback, harden the first-install path against five real-VM failures, and fold in two infrastructure fixes surfaced mid-session.** A real first install of CT6 on a fresh Linux VM (2026-07-05) hit five distinct setup failures, each costing live debugging; and the plugin's model defaults predated Fable 5 (31 agents on `opus` + 8 on `sonnet`; `service_config.DEFAULT_MODEL = "claude-opus-4-8"`). This release moves the default to **Fable 5** (`claude-fable-5`) across the agent frontmatter and the service tier — with an IMPLEMENTED **Opus 4.8** (`claude-opus-4-8`) fallback where the `fable` alias is unavailable — hardens `setup.py` against the five friction points, and folds in the two session-review-surfaced infrastructure fixes (a skill-gate teammate false-block and a lock-concurrency race).

**Fable 5 as the default model (REQ-006, REQ-007).**
- **All 39 `agents/*.md` → `model: fable`, uniformly.** The prior opus/sonnet split was a cost heuristic; the owner directive ("we need fable running wherever it is available") overrides it. The uniform state is itself pinned by a new `tests/test_agents.py` assertion so drift is caught.
- **The deterministic model lever — new `scripts/setup/set_default_model.py`** (stdlib-only, mirroring `sync_agent_boilerplate.py`'s line-ending-preserving, write-only-if-changed shape): `--model fable|opus|sonnet|haiku` rewrites ONLY each agent's frontmatter `model:` line (idempotent; an unknown model is refused, touching nothing); `--check` reports the current distribution + whether it is uniform. This IS the implemented Opus fallback for a harness that predates the `fable` alias — `python3 scripts/setup/set_default_model.py --model opus` restores the known-good state in one command.
- **Service-tier default with an INJECTED fallback — `services/common/service_config.py`:** `DEFAULT_MODEL = "claude-fable-5"`, new `FALLBACK_MODEL = "claude-opus-4-8"`, and `resolve_model(preferred, fallback, availability_checker=None)` — a pure function: with no checker it returns `preferred` (the Anthropic API itself errors informatively if a model is genuinely absent); a checker reporting `preferred` unavailable — OR one that RAISES — returns `fallback` (a probe failure must never crash model selection; it degrades to the known-good fallback). The new `build_llm_client` routes the model through `resolve_model` and rebuilds the config against the resolved id. The live-availability probe stays an ADAPTER boundary (REPO-4: no network/SDK import leaks into the stdlib core), so `check_separation()` stays green.
- **Honest divergence (SETUP-ADV-1):** the design planned a *version-gated* fable-availability heuristic in setup (print the fallback remediation only when the Claude Code version looks too old). Adversarial review rejected the version gate as false precision — the Fable-5-aware Claude Code version is not knowable from setup, and the TEAMS 2.1.32 minimum is not the fable-alias version. So the shipped `check_model_default()` is UNCONDITIONAL: it ALWAYS surfaces ONE informational `note` stating the fable default and carrying the deterministic Opus fallback lever, never version-gated, never auto-applied, and never gating the run.

**First-install setup hardening — `scripts/setup/setup.py` (REQ-001…004).** Every install function that could fail on the VM now gets an injectable `runner`/`uv_path`/`pip_available` seam so the ladders are exercised in tests without touching npm/uv/pip for real.
- **Cartographer marketplace provenance (REQ-001).** `cartographer` ships from the THIRD-PARTY marketplace `kingbootoshi/cartographer`, which no CT6 doc had ever named — the installer had to GitHub-search for it. When cartographer is missing, `plugin_remediation_lines()` now emits the `/plugin marketplace add kingbootoshi/cartographer` step FIRST, then `/plugin install cartographer@cartographer-marketplace` (built-in-marketplace plugins like superpowers / ralph-loop keep their single install line). README + `commands/architect-team-setup.md` document the source.
- **npm EACCES fallback (REQ-002).** A global `npm install -g @fission-ai/openspec@latest` that fails with a permission error (`EACCES` / `EPERM` / "not permitted" / "permission denied") retries ONCE, non-persistently, with `--prefix ~/.local`, and — on success — surfaces the persistent remediation (`npm config set prefix ~/.local` + the `~/.local/bin` PATH note). Setup NEVER mutates the user's npm config; a failed retry returns an actionable, traceback-free message.
- **PEP-668 install ladder + tiktoken (REQ-003).** The Python-deps step is now a ladder: `uv pip install [--system]` (unchanged when uv is present) → `python -m pip install --user` → on the PEP-668 `externally-managed-environment` error, retry with `--break-system-packages`; when neither uv nor an importable pip exists, a failed row carries a `python3-pip` remediation (`sudo apt install python3-pip` on Debian-family) instead of a traceback. `tiktoken` (a cartographer runtime dependency observed missing on the VM) joins `PYTHON_TEST_PACKAGES` (the list name is kept for backward compatibility).
...
- **`docs/archive/INDEX.md` created — the never-delete archive registry.** Documentation dispositioned OUT of the living set is `git mv`'d into `docs/archive/`, never hard-deleted, and indexed here. **Zero flat docs required archival this run** (the archived-flat-docs table is honestly empty — no flat doc was stale-and-not-worth-updating), so the only material archived was the three OpenSpec changes, recorded in the index at their `openspec/changes/archive/` destination for reachability.
...
- **Map stamps re-verified.** `docs/CODEBASE_MAP.md` (`last_mapped`) and `docs/INTEGRATION_MAP.md` (`last_synthesized`) were re-verified against the tree and stamped `2026-07-04`; nothing structural shipped, so no map content row changed — only the note-ledger gained this refresh entry.
- **Widened doc-currency audit — PASS.** The Phase-8 documentation-currency audit was run over all 86 files (not just the release-touched subset) and returns PASS.
...
## [3.31.0] — 2026-07-03 — instruction-compliance standard: a rubric + a deterministic lint + an enforced suite gate

**MINOR — make the plugin's own AI-facing instruction surface machine-checkably compliant.** CLAUDE TEAM SIX IS a body of AI-facing instructions — 47 `skills/*/SKILL.md`, 39 `agents/*.md`, 23 `commands/*.md`, plus `CLAUDE.md` and the two `docs/*_MAP.md` maps (112 files). An agent's compliance is only as reliable as those instructions are uniform, unambiguous, and internally consistent, yet there was NO written standard for what "compliant" instruction text looks like, and the deterministic test pins checked only frontmatter *presence* (name/description) — not frontmatter *shape*, section structure, cross-reference validity, terminology consistency, or literal-imperative wording. Drift (a term meaning two things in two files; a directive phrased as a suggestion; a skill/agent/command/path reference that no longer resolves; a description silently truncated by the loader) shipped undetected. This release establishes the standard, mechanizes the machine-checkable part as a suite-enforced lint, remediates every in-scope file to a pass, and adds enforcement ONLY where wording alone cannot hold compliance.

**The written standard — `docs/INSTRUCTION_COMPLIANCE_RUBRIC.md` (the contract, REQ-001):** grades every in-scope file on **three equally-weighted dimensions** — a file passes only when all three pass (equal weighting operationalized as "each dimension is independently necessary"):
- **(a) Structural / format uniformity [DETERMINISTIC]** — frontmatter fence + `yaml.safe_load` parse; the house no-`': '`/no-`' #'` rule (an unquoted plain-scalar value with `': '` errors `yaml.safe_load`; one with `' #'` is silently truncated as an inline comment); required fields per class; a uniform **1024-char raw-description cap** across skills/agents/commands, measured on the RAW authored value BEFORE any `' #'`/`': '` truncation can mask an over-length value; class-appropriate section structure; and cross-reference validity for the recognized path forms.
- **(b) Terminology + contradiction hygiene [LLM-judgment]** — one term of art means one thing everywhere; no two files give conflicting directives; a load-bearing rule is stated once in its canonical home (`common-pipeline-conventions` / `team-spawning-and-review-gates` / `hooks/review_evidence_schema.py`) and cross-referenced, not duplicated with drift; inventory counts (47 / 39 / 23 / 20 Layer-3 tools / hook scripts) are consistent (the historical v3.13.0 drift class).
- **(c) Literal-imperative wording [LLM-judgment]** — every load-bearing (gating) rule is a followable imperative (MUST / MUST NOT / never / always), not a soft modal ("should consider", "ideally"); hedges are allowed only on genuinely advisory guidance; no self-contradicting imperative.

**The deterministic engine — `scripts/compliance/instruction_compliance.py` (the machine, REQ-002):** stdlib-only (`yaml.safe_load` used only when importable, exactly as `tests/helpers/frontmatter.py` does), no import-time side effects, mirroring the established `scripts/claude_md/claude_md_efficiency.py` engine shape (an `assess_*` function returning a findings list + a `__main__` CLI). `assess_instruction_files(root)` walks the in-scope set and grades dimension (a) + cross-reference validity, emitting a flat findings list (each names the citing file, the `check` kind, a human issue, the specific evidence). The cross-reference grammar is deliberately NARROW — it resolves only unambiguous this-repo-inventory forms (`skills/<name>`, `agents/<name>.md`, `commands/<name>.md`, `hooks|scripts|services/<path>.py|json`, and the two canonical `docs/` maps). The **invoke-form** (`/architect-team:<cmd>`) and **bare back-ticked skill-name** citations are the rubric's documented LLM-judgment carve-out — deliberately NOT machine-checked, because they collide with legitimate prose the engine cannot disambiguate deterministically (the cancel vocabulary, future-placeholder command names, `readme-theme=<name>` config keys), so a false positive there is a wording fix, not an engine bug, and the sweep judges those forms. The 1024-char cap (a.4) IS engine-checked for all three frontmatter classes (a `frontmatter-description-too-long` finding at > 1024) — not a sweep-manual carve-out, so a green lint cannot coexist with an over-cap description.

**The test pins — `tests/test_instruction_compliance.py` (199 cases, REQ-003):** the suite-level pin runs the engine across the full in-scope set and FAILS on any finding (the enforced zero-findings gate), plus a `yaml.safe_load` real-parse pin over every in-scope file, the 62 per-file agents/commands raw-description cap pins, and the engine unit cases (a broken cross-reference is flagged; a `': '`/`' #'`/unparseable frontmatter is flagged; stdlib-only + no import-time side effects asserted). `tests/test_skills.py` / `test_agents.py` / `test_commands.py` each gain **one** section-structure pin that EXTENDS (does not duplicate) their existing frontmatter-presence coverage, reusing `EXPECTED_SKILLS` / `EXPECTED_AGENTS` / `EXPECTED_COMMANDS` and `tests/helpers/frontmatter.py`.

**Remediation to green — 17 instruction files brought to a rubric pass + a clean lint IN PLACE (REQ-005):** 6 skills (`common-pipeline-conventions`, `interaction-intuition`, `phenotypes`, `playwright-user-flows`, `team-spawning-and-review-gates`, `verified-agent-output` — incl. a broken cross-reference and stale v6→v7 evidence-schema-version labels), 9 agents (`doc-updater`, `fix-sensibility-checker`, `flow-explorer`, `frontend`, `integration`, `mini-qa`, `prompt-refiner`, `qa-replayer`, `system-architect` — incl. the over-cap description rewrites, v6→v7 schema labels, and the `qa-replayer` Write-posture fix), and 2 commands (`mini`, `refine-prompt`). The **7 over-cap descriptions** — `agents/doc-updater.md` (1381 raw chars), `fix-sensibility-checker` (1237), `qa-replayer` (1213), `mini-qa` (1162), `prompt-refiner` (1070), `flow-explorer` (1046), and `commands/mini.md` (1048) — were rewritten trigger-first to ≤ 1024; before remediation each one's trailing trigger guidance was silently truncated by the loader (the exact defect the raw-length cap exists to catch), so the model never saw it. The engine reproduced the over-cap set to the character at adoption and reports 0 over-cap corpus-wide after remediation.

**Enforcement only where text cannot hold it (REQ-004) — NO hook added, a deliberate recorded decision.** The 3-round adversarial review sweep expanded the flagged set from 95 to the full **112** in-scope files (and corrected the sweep) and NAMED no dimension-(b)/(c) gap that instruction wording alone cannot hold — no rule agents keep violating despite clear, correct text. Enforcement is never added speculatively, so no `hooks/*.py` change was made; the discipline is held by the deterministic lint + the suite pins. (Producer ≠ checker throughout: the review sweep, the engine, and the enforced suite gate are separate surfaces.)

- Tests: new `tests/test_instruction_compliance.py` (199 cases) + 1 new section-structure pin each in `tests/test_skills.py` / `tests/test_agents.py` / `tests/test_commands.py`. Suite 4851 → **5159 passing + 5 skipped** (195 → 196 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. Skill / agent / command counts unchanged (47 / 39 / 23); **NO new skill / agent / command / hook / Layer-3 tool** (REQ-004's zero-hook outcome).

## [3.30.0] — 2026-07-03 — run continuity: autonomous continuation + sticky resume-via-Skill

**MINOR — close the two run-abandonment gaps observed in real runs:** (1) *the pipeline arbitrarily stops mid-run* — the orchestrator ends its turn with "we've done a lot — say continue if you want me to keep going" even though the mandate is the entire stack end to end; (2) *a resumed session doesn't resume through the skill* — after a restart or a compact, "continue" is not a pipeline command, so the agent hand-solves the run's work without the playbook. Once the architect team is engaged for a run, the run is now driven through the pipeline to completion — across turn ends, compactions, and session restarts — unless the USER explicitly stands it down.

**Root causes (from the enforcement code, not conjecture):**
- `pipeline-completion-audit.py` audited only **worklist debt** (open SRs / failing verdicts). A run BETWEEN phases with a momentarily clean worklist produced zero violations, so the Stop was allowed — exactly when the model volunteers a checkpoint. And even a blocked stop gave up after ONE round: `stop_hook_active` ⇒ unconditional allow.
- `pretool_skill_gate.py` (v3.15.0) anchors to the **single most-recent user prompt** — deliberate then ("a natural escape"), but it means a resume prompt ("continue") stands the gate down entirely. After a compact, the transcript still shows the old Skill call as satisfied while the playbook TEXT is gone from context — nothing forced re-invocation. ("want me to continue?" was already cataloged as a forbidden deferral in `override_markers.py` — but only in the after-the-fact Layer-6 detector.)

**The new substrate — `hooks/run_continuity.py`** (stdlib-only, fail-open throughout, dual-form importable):
- **The `active-run.json` lifecycle marker** (`<workspace>/.architect-team/active-run.json`, schema 1): `status: active | complete | stood-down` + `skill` / `session_id` / `started_at` / `updated_at` / `run_id` / `slug` / `phase`. **Engaged deterministically** — the gate script (also wired at `PostToolUse(Skill)`) writes it the moment a run-driving Skill (`architect-team-pipeline` / `bug-fix-pipeline` / `ux-test-builder` / `mini-architect-team-pipeline`) COMPLETES; no LLM cooperation required, and a call the user denied or that errored never engages (`proposal-refiner` deliberately never engages — it runs standalone). Re-engaging an active marker preserves run identity (started_at / slug / run_id / phase). **Completed explicitly** — `--mark-complete` is now the documented LAST state action of Phase 8 / B8 / M8 / U9, after commit + push + auto-merge. **Stood down only by the user** — `--stand-down "<the user's words>"` writes the auditable `pipeline-stand-down.md` artifact (the same user-authorization bar as the v3.0.0 unilateral-override discipline).
- **The progress fingerprint**: `.architect-team/**` stat walk (excluding the guard's own state + the liveness marker) + git HEAD/ref/index stats + a `git status --porcelain` hash — "did anything observable change since the last blocked stop?"
- **Transcript analysis** (reusing the gate's battle-tested record helpers): pipeline-skill engagement (whole-ledger or **since the last compact boundary** — both known boundary shapes: `subtype: "compact_boundary"` and `isCompactSummary`), genuine-prompt detection (sidechain/meta-safe), and teammate recognition.
- **CLI**: `--status` / `--engage <skill>` / `--set k=v ...` / `--mark-complete` / `--stand-down <reason>` (+ `--root`); ASCII JSON output.

**The three enforcement surfaces:**
1. **Stop-hook CONTINUATION GUARD** (`pipeline-completion-audit.py`): an ACTIVE marker blocks the Stop even with a clean worklist (a run between phases is not done). ENGAGED sessions (transcript shows a pipeline skill) lose the one-block-then-give-up semantics: the guard keeps blocking across stop-chains while the run makes PROGRESS (fingerprint changes — unbounded, per the v3.8.0 Unbounded solving discipline), and after `CT6_MAX_NO_PROGRESS_STOPS` (default 3) CONSECUTIVE no-progress blocks it **auto-escalates** — writes `escalation-pending.md` and allows — so a wedged run surfaces loudly instead of looping (a fresh genuine user prompt resets the budget). The block message names the forbidden deferral explicitly and, post-compact, directs the Skill re-invocation first. NON-engaged sessions keep the legacy semantics verbatim (one block per stop-chain, `stop_hook_active` ⇒ allow) plus a **resume nudge** naming `Skill(<skill>)` when a run is active — funneling a resumed session back into the pipeline without wedging unrelated sessions. The sanctioned pauses are UNCHANGED and checked first: `escalation-pending.md`, fresh `in-progress.md`. `--check` mode (the Phase 8 pre-commit gate) deliberately skips the lifecycle check — Phase 8 runs it while the run is still active.
2. **PreToolUse STICKY RUN ARM** (`pretool_skill_gate.py` arm 2): while a run is active, a user-facing session that has NOT invoked a pipeline skill since its last compact boundary is blocked from build/dispatch tools (`Edit` / `Write` / `NotebookEdit` / `Agent` / `Task*` — read-only tools and the wrapper's `Bash` never gate, preserving v3.15.1) until it re-invokes the Skill; the block message gives the three resolutions (resume via Skill / `--mark-complete` if genuinely done / user-authorized `--stand-down`). Arm 1 (the v3.15.0 most-recent-prompt mandate) is unchanged and takes precedence. **Deterministic engagement recording** lives in `main()` (never in the pure `check_payload`), rides the `PostToolUse(Skill)` wiring (a DENIED or errored Skill call never writes a phantom marker), requires an explicit payload `cwd` + transcript evidence of a genuine non-teammate session, and never affects the verdict.
3. **The first `SessionStart` hook** — `hooks/sessionstart-run-continuity.py` (8th wired event): on `startup` / `resume` / `clear` / `compact`, an active marker injects the resume directive — invoke `Skill("<skill>")` FIRST, then continue the run — sharpened when the source is `compact` (the playbook was just dropped from context). Prints nothing otherwise; never blocks.

**Teammate safety (the brick-risk the design centers on):** teammates never invoke Skills, so both guards MUST stand down for them. `team-spawning-and-review-gates` now mandates the literal first line `[CT6-TEAMMATE <name> RUN <run-id-or-slug>]` in every spawn brief / dispatch prompt; recognition falls back (fail-OPEN — a missed teammate would brick the run's own workers; a missed user session merely defers to the Layer-6 audit) to the brief-shaped heuristic (no `<command-name>` records, long first prompt referencing `.architect-team` paths). Sidechain/subagent transcripts (no genuine user prompt) stand down likewise.

**Skill-text (the guidance half):** new canonical `common-pipeline-conventions` `## Run continuity discipline (v3.30.0)` (marker lifecycle, the three surfaces, the no-arbitrary-checkpoint rule, the teammate token, escapes + kill-switch); `architect-team-pipeline` gains the `### Mark the run complete` Phase 8 step + a run-continuity operating rule (inherited by bug-fix / ux-test); explicit mark-complete steps in `bug-fix-pipeline` B8, `mini-architect-team-pipeline` M7 (with the M6-red full-pipeline hand-off deliberately NOT marking complete — the full pipeline's Skill refreshes the same marker), and `ux-test-builder` U9 (U8-routed bug-fix runs are their own runs/markers).

**Escapes and bounds:** `CT6_RUN_CONTINUITY_DISABLED=1` kills all three surfaces (legacy worklist audit untouched); `CT6_MAX_NO_PROGRESS_STOPS` tunes the budget; `complete` / `stood-down` markers, `escalation-pending.md`, and fresh `in-progress.md` stand everything down; every reader fails open (missing/malformed state ⇒ pre-v3.30.0 behaviour). The marker consultation requires an explicit payload `cwd` — ambient process state is never used to gate unrelated payloads.

**Adversarial review (producer ≠ checker) returned FIX-FIRST; remediated 3 MAJOR + 5 MINOR before ship:** (1) MAJOR — a long-lived teams-mode teammate could be BRICKED when its spawn brief (the `CT6-TEAMMATE` token lives on the brief's first line, at the transcript HEAD) scrolled past the gate's 2 MB tail-read → identity/engagement questions now consult a HEAD slice (`read_transcript_head` / `load_transcript_slices`) in addition to the tail; a truncated transcript with no readable head is treated AS a teammate, and an engagement answer that cannot be proven on a truncated transcript is AMBIGUOUS and never blocks (the Stop guard additionally recognizes the orchestrator by the marker's recorded `session_id`, which survives any truncation). (2) MAJOR — engagement was recorded at PreToolUse, so a Skill call the user DENIED still wrote a phantom `active` marker → recording moved to a new `PostToolUse(Skill)` wiring (the same script, trigger-split like review-gate-task.py); the marker now exists only for a Skill that actually ran. (3) MAJOR — the marker had no staleness bound and the sticky arm ignored the sanctioned pauses → the sticky arm now stands down on `escalation-pending.md` (the human may hand-edit to resolve the very blocker) and on a STALE marker (`CT6_RUN_MARKER_STALE_HOURS`, default 72h — an abandoned run stops taxing the workspace); the continuation guard heartbeats `updated_at` on every engaged block (fingerprint-excluded — never fake progress), so a LIVE run never goes stale, and the SessionStart directive softens to informational on a stale marker. (4-8 MINOR) the Stop guard's engaged detection survives tail-eviction via the session_id match + head scan; the compact-boundary record shapes were EMPIRICALLY validated against real local transcripts (both `subtype: compact_boundary` system records and `isCompactSummary` — the detection was correct as written); the new model-facing stderr strings are pure ASCII for cp1252 consoles; `--mark-complete` is now GUARDED (refuses on open worklist debt via the same `--check` audit Phase 8 uses, `--force` logged) and every lifecycle exit appends to `run-completion.log`; the no-progress attempt display no longer prints a confusing `attempt 0`.

**Adversarial review (producer ≠ checker) returned FIX-FIRST; remediated 3 MAJOR + 5 MINOR before ship:** (1) MAJOR — a long-lived teams-mode teammate could be BRICKED when its spawn brief (the `CT6-TEAMMATE` token lives on the brief's first line, at the transcript HEAD) scrolled past the gate's 2 MB tail-read → identity/engagement questions now consult a HEAD slice (`read_transcript_head` / `load_transcript_slices`) in addition to the tail; a truncated transcript with no readable head is treated AS a teammate, and an engagement answer that cannot be proven on a truncated transcript is AMBIGUOUS and never blocks (the Stop guard additionally recognizes the orchestrator by the marker's recorded `session_id`, which survives any truncation). (2) MAJOR — engagement was recorded at PreToolUse, so a Skill call the user DENIED still wrote a phantom `active` marker → recording moved to a new `PostToolUse(Skill)` wiring (the same script, trigger-split like review-gate-task.py); the marker now exists only for a Skill that actually ran. (3) MAJOR — the marker had no staleness bound and the sticky arm ignored the sanctioned pauses → the sticky arm now stands down on `escalation-pending.md` (the human may hand-edit to resolve the very blocker) and on a STALE marker (`CT6_RUN_MARKER_STALE_HOURS`, default 72h — an abandoned run stops taxing the workspace); the continuation guard heartbeats `updated_at` on every engaged block (fingerprint-excluded — never fake progress), so a LIVE run never goes stale, and the SessionStart directive softens to informational on a stale marker. (4-8 MINOR) the Stop guard's engaged detection survives tail-eviction via the session_id match + head scan; the compact-boundary record shapes were EMPIRICALLY validated against real local transcripts (both `subtype: compact_boundary` system records and `isCompactSummary` — the detection was correct as written); the new model-facing stderr strings are pure ASCII for cp1252 consoles; `--mark-complete` is now GUARDED (refuses on open worklist debt via the same `--check` audit Phase 8 uses, `--force` logged) and every lifecycle exit appends to `run-completion.log`; the no-progress attempt display no longer prints a confusing `attempt 0`.

- Tests: new `tests/test_run_continuity.py` (28 — marker lifecycle incl. re-engage identity preservation + stand-down artifact, fingerprint stability/exclusions, no-progress counting + resets, engagement incl. both compact-boundary shapes, teammate token + heuristic + user-session negatives, CLI incl. `--set` lifecycle-verb refusal, plus the remediation pins: staleness, the fingerprint-excluded heartbeat touch, head/tail transcript slices incl. a real >2 MB token-past-the-tail-cap case, truncated-without-head fail-open, the worklist-guarded `--mark-complete` + `--force`, the completion log), `tests/test_pipeline_completion_audit_continuation.py` (17 — the clean-worklist lifecycle block, `--check` exclusion, kill-switch, malformed-marker fail-open, sanctioned pauses, the non-engaged nudge + legacy never-loop, engaged persistence past `stop_hook_active`, no-progress auto-escalation at the budget + the escalation artifact, progress-resets-budget, worklist listing, post-compact reload directive, plus stale-marker stand-down, session_id-match engagement, the marker heartbeat, and the ASCII-stderr pin), `tests/test_sessionstart_run_continuity.py` (8), 18 sticky-arm/engagement cases appended to `tests/test_pretool_skill_gate.py` (incl. escalation-pause + stale-marker stand-downs, the PreToolUse-never-engages / PostToolUse-engages pins, and the errored-Skill-run-never-engages pin from the re-verify), and the SessionStart + PostToolUse(Skill) wiring pins in `tests/test_hooks_structure.py`. **Every legacy Stop-hook and gate pin passes verbatim** (non-engaged semantics preserved by construction). Also FOLDED IN a v3.29.0 doc defect the full-suite gate surfaced: `commands/librarian-install.md`'s frontmatter description carried a `": "` ("Subcommands: install / ..."), which breaks the yaml test parser (`test_command_frontmatter_valid`) — rewritten with the em-dash form per the house frontmatter rule. Suite 4780 → **4851 passing + 5 skipped** (195 test files; the 2 extra skips vs the v3.29.0 note are the Unix-only symlink tests, environment-dependent), green under both Windows cp1252 and `PYTHONUTF8=1`. Skill / agent / command counts unchanged; **+1 enforcement script (6 → 7), +1 wired event (SessionStart, 7 → 8), +1 wired matcher (PostToolUse(Skill))**; NO new Layer-3 tool.

## [3.29.0] — 2026-06-18 — the Librarian becomes installable (CT6-6 server tier)

**MINOR — make the CT6-6 Librarian a first-class INSTALLABLE, mirroring the `mempalace-install` pattern.** The v3.24.0 Librarian shipped as a runnable stdlib-only core; this release adds the install lifecycle around it — REUSE-FIRST over the existing `services/common/bg_runtime.py` (the scheduler + the per-OS boot descriptor), `services/common/service_config.py` (the same-Anthropic-key model + the `LLMClient` adapter), `services/librarian/librarian.py` (the fetch→extract→index→metadata orchestration), and `services/librarian/library_index.py` (the sqlite reference index). No new service LOGIC — the new code is the installer, the CLI, and the daemon entry point.

**HONEST BOUNDARY (read first):** the plugin core stays **stdlib-only**, `check_separation()` stays clean, and nothing here is "deployed" or "running in production." The accurate framing is: the Librarian is now **installable**, and **once installed and enabled with an API key it runs as a background daemon on the local machine**. The real network `Source` (the live HTTP fetch) and the real Anthropic LLM remain adapter boundaries with stdlib fallbacks; the installer GENERATES the per-OS boot descriptor and PRINTS the register hint — it never auto-loads it.

- **`commands/librarian-install.md`** — the 23rd slash command `/architect-team:librarian-install` (slash-command count 22 → 23). Full-lifecycle: `install` / `status` / `add-topic` / `list-topics` / `remove-topic` / `run-once` / `uninstall`, with the flags `--enable` / `--check-only` / `--json` / `--purge`. A thin wrapper over the installer CLI.
- **`scripts/setup/install_librarian.py`** — a stdlib-only installer + CLI. Provisions state under `~/.architect-team/librarian/` (`config.json`, `topics.json`, `index.sqlite`, `bodies/`, `metadata/`, `librarian.log.jsonl`). LLM wiring: it uses the real Anthropic adapter when `ANTHROPIC_API_KEY` resolves, and otherwise falls back to `FakeLLMClient` in an honest **degraded** mode — no key ⇒ **provisioned-but-disabled**, with the `--enable` remediation surfaced so the operator can turn it on once a key is available. It generates the per-OS boot descriptor via `bg_runtime.install_descriptor` (launchd / systemd / schtasks) and PRINTS the register hint (never auto-loads).
- **`services/librarian/daemon.py`** — the daemon **entry point** (a path-runnable script that builds the `Librarian` + the `Scheduler` and calls `run_forever`) + `UrlSource(Source)`, a stdlib `urllib` fetcher over the topic→URL registry that skips a failing fetch gracefully (the live HTTP fetch is the real network adapter; its stdlib-only fallback keeps the core testable).
- **Honest-boundary discipline preserved** — the in-repo plugin core is still stdlib-only; the separability invariant `check_separation()` remains clean; nothing is described as "deployed/running in production."
- Tests: new `tests/test_install_librarian.py` (23 offline tests — config/topics provisioning + the state-dir layout, the with-key vs no-key (provisioned-but-disabled + `--enable`) paths, the lifecycle subcommands, the boot-descriptor generation + print-not-load, and the `UrlSource` skip-on-failure) + the `tests/test_commands.py` / `tests/test_skill_invocation_audit_canonical.py` command-count pins updated for the 23rd command. Suite 4757 → **4780 passing + 3 skipped** (192 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. Skill / agent counts unchanged; **+1 command (22 → 23)**; NO new Layer-3 tool.

## [3.28.0] — 2026-06-17 — the service-tier separation manifest (CT6-6 server tier: REPO-1 … REPO-4)

**MINOR — the FINAL CT6-6 component, under `services/separation.py` + `services/SEPARATION_MANIFEST.md`.** The two-repo separation plan (REPO-1/2/3) + a machine-checkable separability invariant (REPO-4). **This completes the entire CT6-6 program** — the in-repo tier (v3.17.0–v3.22.0) + the service tier (v3.23.0–v3.28.0).

**HONEST BOUNDARY (read first):** this DESIGNS + ENCODES + VALIDATES the separation boundary. The actual repo SPLIT — creating the separate paid/closed repo and moving the closed pieces into it — is a future operation, NOT performed here.

- **`services/separation.py` — `SEPARATION_MANIFEST`** (REPO-1/2/3): the open-core-vs-paid-repo plan; every service marked separable; the adapter SEAMS each closed/paid piece plugs into (the SEC-4 project-unique attestation algorithm → `attestation_verifier`; the SMP-4 phenotype entitlement/billing → `entitlements_for`; plus the non-paid `LLMClient` / `Source` / `IssueSink` / `transport` / `bg_runtime` seams). `validate_manifest` checks the shape (every service separable; every paid piece maps to a real seam).
- **`services/separation.py` — `check_separation()`** (REPO-4, the machine proof): parses every `services/**/*.py` and asserts each is IMPORT-CLEAN at module load — only stdlib + in-repo modules; any external/third-party module-load import is a violation (it must be injected via a seam, never hard-imported). The walker recurses through every load-time compound statement (`try`/`except`, `if`, `with`, `for`, class bodies) but NOT function bodies, so a genuinely-lazy in-function import (the deferred `import anthropic`) is allowed while a `try: import chromadb` at module level is caught. That import-cleanliness is what makes each service liftable into its own repo.
- **`services/SEPARATION_MANIFEST.md`** — the human companion: the two-repo plan, the seam table, the paid/closed pieces, and the "how to separate out" procedure.
- **Adversarial review (producer ≠ checker) returned FIX-FIRST; remediated 3:** (1) a CRITICAL scanner soundness hole — the original body-only scan inspected only direct children of the module body, so a module-load import nested in a `try/except` (the exact optional-dep idiom used 3× in the tree for `bg_runtime`), an `if` block, or a class body was INVISIBLE; a hard `try: import chromadb` would have passed as clean, making the REPO-4 guarantee hollow → the walker now recurses through all load-time statements (but not function bodies); (2) `validate_manifest` crashed (AttributeError) on a malformed non-dict entry instead of reporting invalid → guarded; (3) on Python < 3.10 (no `sys.stdlib_module_names`) the check would have over-flagged all stdlib → an explicit 3.10+ guard. The review also confirmed the manifest is honest (no "already split" overclaim) and the scanner catches `import a.b.c` / `from ext import x` / aliased / multi-name imports while correctly treating relative + in-function imports as allowed. Documented residual: the in-repo allow-list is by bare module name, so a same-named external package is masked — acceptable, since the in-repo module shadows it on `sys.path` at runtime anyway.
- Tests: new `tests/test_services_separation.py` (7 cases — the manifest validate + the documented paid pieces/seams, the malformed-manifest guard incl. non-dict entries, the headline REPO-4 import-clean assertion over the real tree, and the scanner edges: external top-level vs lazy in-function, in-repo + reuse-name allow, and the nested-`try`/`if`/class-body catch). Suite 4746 → **4753 passing + 5 skipped** (191 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. Skill / agent / command counts unchanged (the service tier is not a skill/agent/command); NO new Layer-3 tool.

## [3.27.0] — 2026-06-17 — the Seeded MemPalace service (CT6-6 server tier: SMP-1 … SMP-5)

**MINOR — the fourth concrete service on the v3.23.0 substrate, under `services/seeded_mempalace/`.** An optional load-up from a separate service that downloads a seeded MemPalace during setup (SMP-1), with a valid auth key, from the project's server (SMP-2). The seeded MemPalace ships a clear schema + curated content while leaving room for the user's own projects (SMP-3), is where phenotypes are stored with a future purchase model (SMP-4), and carries the latest research synthesis (SMP-5).

**HONEST BOUNDARY (read first):** design + a runnable, stdlib-only deterministic core + tests, NOT a live-deployed service. ChromaDB (the real MemPalace vector store), the live HTTP server, the network transport, and the entitlement/billing system are adapters / operator-provided (the separated tier's own deps, REPO-4), not stood up here. SMP-1's "MemTime" is interpreted (and disclosed) as the MemPalace load/init at session start — the seeded download precedes it.

- **`services/seeded_mempalace/bundle.py`** — the defined seeded-MemPalace bundle (SMP-3/5): `schema` + `curated` + `phenotype_catalog` + a `research_synthesis` freshness section (SMP-5 — `last_updated` + entries). `merge_into_local` REFRESHES the seeded sections from a fresh download while PRESERVING every user top-level key untouched — so a re-download never clobbers the user's own projects (any namespace / collection / metadata, not just the reserved one).
- **`services/seeded_mempalace/catalog.py`** — the phenotype catalog (SMP-4), REUSING the existing phenotype store (`scripts/phenotypes/phenotypes.py::discover_phenotypes`) rather than a new schema. `build_catalog` is the master catalog (browse metadata + full record per entry); `gate_catalog` serves full records ONLY for entitled phenotypes (a non-owner browses metadata, purchases to download — the future purchase model), deep-copying so a served record can't corrupt the master.
- **`services/seeded_mempalace/client.py`** — SMP-1/2: `build_download_request` signs a request with the local Ed25519 key (REUSING `services/common/handshake.py` — SEC-2/3/5, the same lower-risk handshake the Librarian uses); `install_seeded_mempalace` downloads via an injected transport, validates the bundle, and merges it locally (preserving the user's projects). Runs during setup, before MemTime.
- **`services/seeded_mempalace/server.py`** — SMP-2/4: `handle_bundle_request` verifies the signed envelope (no per-user code; replay/tamper rejected) and serves the bundle with its catalog gated to the requester's entitlements — resolved by the VERIFIED PUBLIC KEY (the real identity), not the self-asserted requester string. `make_handler` wires it onto a stdlib `http.server` (binding is the operator's; replay protection defaults on).
- **Adversarial review (producer ≠ checker) returned FIX-FIRST; remediated 5 before ship:** (1) a genuine IMPERSONATION hole — entitlements were keyed on the self-asserted `requester` string, so anyone could sign with their OWN key, claim another user's name, and receive that user's paid phenotypes → entitlements now resolved by the verified public key (`entitlements_for(requester, public)`); (2) `gate_catalog` aliased the master's nested records, so a mutation to a served bundle corrupted the master + every later requester → the served records are deep-copied; (3) `merge_into_local` dropped every user top-level key except the reserved namespace (SMP-3 data loss) → it now preserves all user keys, refreshing only the seeded ones; (4) the served record leaked the operator's absolute filesystem paths (the `_dir`/`_label_dir` fields `discover_phenotypes` injects) → stripped; (5) the shipped server defaulted to NO replay protection → `make_handler` now defaults a `seen_nonces` set.
- Tests: new `tests/test_services_seeded_mempalace.py` (15 cases — the bundle schema/validate/merge incl. preserve-all-user-keys, the catalog build/gate/entitled-labels + the real-phenotype-store reuse, the signed client + install (authorized / unauthorized / invalid-bundle), the server auth + catalog gating + tamper rejection, and an end-to-end client↔server install — plus the impersonation-defeated, no-aliasing, and no-path-leak remediation edges). Suite 4731 → **4746 passing + 5 skipped** (190 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. Skill / agent / command counts unchanged (the service tier is not a skill/agent/command); NO new Layer-3 tool.

## [3.26.0] — 2026-06-17 — the Session Review service (CT6-6 server tier: SR-1 … SR-3)

**MINOR — the third concrete service on the v3.23.0 substrate, under `services/session_review/`.** A session-review agent of similar design to the Librarian (SR-1): if activated, it sits on the server or laptop and reviews agentic output AT THE SESSION level. As it works through a session it performs a simple outbound PUSH summarizing the session's output (SR-2), and it identifies the issues the agents ran into — specifically the ones they were NOT competent enough to solve on the first attempt (SR-3).

**HONEST BOUNDARY (read first):** design + a runnable, stdlib-only deterministic core + tests, NOT a live-deployed service. The real LLM (the session reviewer), the live outbound-push target (SR-2 — the triage server; a real pusher signs the payload via `services/common/handshake.py` and POSTs it), and persistence are adapters / operator-provided (the separated tier's own deps, REPO-4), not stood up here.

- **`services/session_review/session_review.py`** — the whole service (it is small + reuse-heavy). `build_session_review_prompt` + `parse_session_review` (string-aware `raw_decode`, brace-in-string safe, capped) drive the SR-1 session-level review via the injected `LLMClient`. `review_session` keeps ONLY the issues the agents did NOT solve on the first attempt (SR-3) — using a robust boolean coercion (`_solved_on_first_attempt`) so a stringified `"false"` is NOT mistaken for solved — and normalizes each as a REUSED triage `issue` record (so they follow the same triage process as EVAL + HD). `SessionReview.review_and_push` does the SR-2 outbound summary push + files the unsolved issues through the triage `sink`; `build_review_task` + `install_descriptor` put it on the shared `bg_runtime`, like the Librarian.
- **EVAL-17 default-off (privacy):** under `privacy_level == "off"` (the default) the service transmits NOTHING off-machine — neither the (free-text, possibly identifiable) summary push NOR the issue tickets; the review is produced LOCALLY only and the operator opts in to `summary`/`full`. (Consistent with the triage server's `off` behavior.) Issue evidence is redacted per level by the reused `make_issue` (`logit` engine).
- **Adversarial review (producer ≠ checker) returned FIX-FIRST; remediated before ship:** (1) the SR-3 filter used Python truthiness on the LLM's `solved_on_first_attempt`, so a stringified `"false"` (truthy) SILENTLY DROPPED the exact unsolved issue SR-3 exists to capture → an explicit affirmative coercion (default-keep; only a real `True` / `"true"`/`"yes"`/`1` excludes); (2) the SR-2 summary push transmitted the raw free-text summary at EVERY level including `off`, contradicting the EVAL-17 posture → under `off` nothing is pushed or filed; (3) `parse_session_review` was O(N²) on pathological brace-heavy input → capped candidate starts; (4) a raising `sink.create_ticket` crashed the review (asymmetric with the best-effort push) → per-ticket best-effort; (5) EVAL-8 (version) enforced only lazily → guarded at `review_session` / `SessionReview.__init__`.
- **Folded-in fix (pre-existing, surfaced by the full-suite gate — not session-review):** `hooks/inflight_inbox.py` `_inbox_lock` crashed under full-suite CPU contention with a `PermissionError` on the `.lock` file. On Windows, when a concurrent holder is mid-`unlink` of the lock, `os.open(O_CREAT|O_EXCL)` raises `PermissionError` (ERROR_ACCESS_DENIED) instead of `FileExistsError`; the acquire loop only caught the latter, so a high-contention append-vs-rewrite race surfaced as an uncaught `PermissionError` out of `mark_processed`. The loop now treats it as the same transient contention (wait + retry, bounded by the deadline). The existing `test_concurrent_append_survives_mark_processed` stress test now passes reliably, plus a new deterministic `test_inbox_lock_rides_out_windows_permission_error`. (Recording the incidental fix here, as the v3.13.0 release did for its `discipline_registry` fix.)
- Tests: new `tests/test_services_session_review.py` (11 cases — the session-level prompt + string-aware parse, the SR-3 unsolved-only filter incl. the stringified-boolean edge, the SR-2 push + the EVAL-17 off-transmits-nothing posture, the no-pusher / raising-pusher best-effort paths, the BG task + install descriptor, and the version guard) + 1 deterministic regression test for the inflight-lock fix. Suite 4719 → **4731 passing + 5 skipped** (189 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. Skill / agent / command counts unchanged (the service tier is not a skill/agent/command); NO new Layer-3 tool.

## [3.25.0] — 2026-06-17 — the Triage / Evaluator service (CT6-6 server tier: EVAL-1 … 17 + SEC)

**MINOR — the second concrete service on the v3.23.0 substrate, under `services/triage/`.** Make the models self-correcting by capturing, de-duplicating, and triaging issues across versions (EVAL). The automatic counterpart to the manual v3.21.0 helpdesk path — both produce the same normalized issue record and follow the same triage process.

**HONEST BOUNDARY (read first):** design + a runnable, stdlib-only deterministic core + tests, NOT a live-deployed service. The live triage HTTP server socket, the GitHub issues API (+ the pull-back), Postgres persistence of the tally/backlog, the Anthropic LLM (the evaluator), and the real SEC-4 attestation are adapters / operator-provided — the separated tier's own deps (REPO-4), not stood up here.

- **`services/triage/issue.py`** — the normalized triage-issue record (EVAL-8/9/14): what / what-happened, version + metadata, source (auto / manual-helpdesk), and a stable `fingerprint` (the EVAL-4 dedup key, joined with a NUL sentinel so distinct category/what pairs can't alias). Evidence is privacy-redacted by REUSING the helpdesk engine `scripts/helpdesk/logit.py` (EVAL-15…17); the default is `off` (EVAL-17 — automatic logging is OFF by default; a caller opts in to summary/full).
- **`services/triage/evaluator.py`** — EVAL-1: `build_evaluation_prompt` reviews the logs "as a senior agentic architect", categorizing + ROOT-CAUSING each issue; `parse_evaluation` is a string-aware JSON parse (a bracket/brace inside a string value can't truncate the array); `evaluate_logs` turns the result into issue records. EVAL-3: `build_optimization_task` is the ~hourly BG optimization task on the shared `bg_runtime` that feeds issues to a sink.
- **`services/triage/tally_queue.py`** — EVAL-4/10: batch duplicate issues by fingerprint (count + representative + versions) so the open-ask list can't grow uncontrollably; promote entries crossing a threshold to the longer-lasting backlog.
- **`services/triage/triage.py`** — the server-side triage. The MOST IMPORTANT piece is the **quarantine rule** (EVAL-11/12): an issue first seen on an OLD version may already have been fixed by an intermediate release the reporter simply hasn't upgraded to — `classify_issue` holds it in quarantine and names the version to verify from (deterministic version-tuple logic; the "agents judge it already fixed" signal is an honest input). Plus EVAL-6 `ResolutionLog` ("may already be fixed"), EVAL-7/13 `RecurrenceTracker` (does it recur from the fixed version onward?), and EVAL-5 `two_stage_review` (a collection → common core issues).
- **`services/triage/sink.py`** — EVAL-2: the issue-sink adapter. `github_issue_payload` builds the GitHub-issue REST payload (title/body/labels); `GitHubIssueSink` posts via an injected `poster` (the real token + HTTP POST is the operator's, mirroring `scripts/notify/notify.py`); `InMemorySink` for tests. No poster → recorded not-transmitted (honest).
- **`services/triage/server.py`** — EVAL-2 + SEC: `handle_submission` verifies a signed **Ed25519 submission envelope** (reusing `services/common/handshake.py` — signature + freshness + nonce-replay; no per-user secret, SEC-1/2/3), then re-applies privacy server-side (defence in depth, EVAL-16) before recording each issue + creating a ticket. The SEC-4 genuine-logger attestation is a pluggable verifier (the separable closed/paid piece). `make_handler` wires it onto a stdlib `http.server`; binding + `serve_forever` is the operator's.
- **Adversarial review (producer ≠ checker) returned FIX-FIRST; remediated before ship:** (1) the automatic-path privacy default was `summary`, contradicting EVAL-17 ("logging OFF by default") + the reused `logit.py`'s own stated posture, and the EVAL-3 task couldn't set it → defaulted the automatic path to `off` and threaded `privacy_level` through `build_optimization_task`; (2) a SEC-1 docstring OVERCLAIM (asymmetric verification alone "cannot be spammed") → softened to the truth (anti-spam / genuine-logger needs the SEC-4 attestation, off by default; replay needs a caller-supplied `seen_nonces`); (3) the `fingerprint` field boundary collapsed under whitespace-normalization so distinct (category, what) pairs aliased → joined with a NUL sentinel; (4) the server's summary defence-in-depth re-redacted only `evidence`, not unknown top-level keys → it now rebuilds each issue from a structural allow-list; (5) `KeyError` on a fingerprint-less issue in public methods → a clear `ValueError` / skip. The review independently verified the EVAL-11/12 version window is correct (numeric ordering, earliest-intermediate-fix selection, the strict `seen < fix <= current` boundaries) and the SEC core (forged / tampered / stale / replayed rejected; the attestation is bound into the signature).
- Tests: new `tests/test_services_triage.py` (30 cases — the issue record + privacy levels, the evaluator prompt/parse incl. brace-in-string + the EVAL-17 off default, the tally/backlog, the quarantine rule incl. the intermediate-fix + first-occurrence + boundary cases, resolution / recurrence / two-stage review, the sink transmission, and the signed-submission server incl. tamper / replay / off / summary-re-redaction / attestation + the 8 remediation edges). Suite 4689 → **4719 passing + 5 skipped** (188 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. Skill / agent / command counts unchanged (the service tier is not a skill/agent/command); NO new Layer-3 tool.

## [3.24.0] — 2026-06-17 — the Librarian service (CT6-6 server tier: LIB-1 … LIB-13)

**MINOR — the first concrete service on the v3.23.0 substrate, under `services/librarian/`.** A background curation/research service: it pulls data for user-defined topics, reads + extracts each download via the shared LLM, indexes the keepers, and writes the per-topic metadata files agents look for. It runs on the shared BG runtime (scheduled, restartable) and uses the shared configurable Anthropic key (LLM == sign-up).

**HONEST BOUNDARY (read first):** design + a runnable, stdlib-only deterministic core + tests, NOT a live-deployed service. The data SOURCE (web scrape / an attached API endpoint), the MemPalace VECTOR store (LIB-9, preferred), and the Anthropic LLM are **adapters** behind interfaces with dependency-free fallbacks (`StaticSource` + `FakeLLMClient` for tests) — the separated service tier's own deps (REPO-4), not wired here. NOT built (design-stage): LIB-4's centralized MemPalace curation ENDPOINT (topics are a local registry here, not a server-side curation point) and LIB-7's global-MemPalace-install research / standardization.

- **`services/librarian/library_index.py`** — a stdlib `sqlite3` keyword / summary / concept-cloud **reference index** (LIB-11/12/13) + **conceptual search** (LIB-10): `add_document` / `get` / `summary` / `search_by_keyword` / `search_by_concept` / `conceptual_search` / `all_concepts` / `count`. `conceptual_search` ranks documents by the weighted overlap of the query terms with each doc's concept cloud (×3), keywords (×2), and title + summary tokens (×1), over **unicode-folded** tokens (NFKD + combining-mark strip + casefold, so `café` ~ `cafe`). Honestly hedged: this is token OVERLAP, NOT semantic / synonym / stem expansion — true relatedness is the LIB-9 vector-store adapter; this is the deterministic stdlib stand-in.
- **`services/librarian/extract.py`** — the extraction contract (LIB-11/12): `build_extraction_prompt` asks the LLM to confirm relevance and return a single JSON object (title + summary + strong keywords + concept cloud); `parse_extraction` parses it **string-aware** via the stdlib `JSONDecoder.raw_decode` looping over each `{` until one decodes to a dict, so a `{`/`}` inside a string VALUE can't miscount and truncate the object — an unparseable reply yields a not-relevant record (skipped), never raises; `extract_record` gives each doc a stable sha256 id (idempotent re-ingest).
- **`services/librarian/librarian.py`** — the orchestration (LIB-1…9): `Source` / `StaticSource` content adapters, then `research_topic` runs fetch → extract → index the relevant keepers (a `relevant AND (title OR summary)` gate) → write the per-topic metadata file (LIB-6). `build_scheduler_tasks` registers a per-topic task on the shared `bg_runtime.Scheduler` (LIB-5, with correct per-topic closure binding); `install_descriptor` emits the per-OS boot/restart daemon descriptor (LIB-2/3). The LIB-8 **file-folder body store** (`body_dir` + `_store_body` / `get_body`) persists full document bodies on disk with a **path-safe** filename (`[^A-Za-z0-9._-]` → `_`) so an operator-supplied `doc_id` can't escape the folder.
- **Adversarial review (producer ≠ checker) returned FIX-FIRST; remediated 4 before ship:** (1) the JSON parser was string-UNAWARE (a brace inside a summary string truncated the object) → switched to the stdlib `JSONDecoder.raw_decode`; (2) LIB-8's file-folder body store was specified but missing → added `body_dir` + `_store_body` / `get_body` with the path-safe filename; (3) LIB-10 was over-claimed as conceptual / semantic → docstrings hedged to "token overlap, not synonym / stem expansion" with the LIB-9 vector adapter named as the real semantic path; (4) the tokenizer was ASCII-only → NFKD unicode-folding so diacritics index + match. The honesty ledger also records LIB-4 / LIB-7 as design-stage, not built.
- Tests: new `tests/test_services_librarian.py` (14 cases — the index round-trip / idempotent re-index / keyword + concept search / conceptual ranking / concept cloud; the prompt + string-aware parse; the orchestration relevant-keep-vs-skip + metadata write + scheduler tasks + install descriptor; and the remediation edges — brace-in-string, empty-query + unicode fold, relevant-but-no-title gate, file-folder body storage + path safety, per-topic closure binding). Suite 4675 → **4689 passing + 5 skipped** (187 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. Skill / agent / command counts unchanged (the service tier is not a skill/agent/command); NO new Layer-3 tool.

## [3.23.0] — 2026-06-17 — Service-tier foundation (the CT6-6 server tier begins)

**MINOR — the first piece of the CT6-6 SERVICE/SERVER tier, under a new top-level `services/`.** With the in-repo tier complete (v3.17.0–v3.22.0), this begins the server/service tier. The user chose to build ALL of it, **in-repo but separable** (REPO-3), with **one shared Anthropic key** (the LLM key == the sign-up key), and an **Ed25519 local-signature** triage handshake (SEC-3). This release ships the shared substrate the Librarian / Session-Review / Evaluator / seeded-MemPalace services will sit on.
...
- **`services/common/ed25519.py`** — a pure-Python, **stdlib-only Ed25519** (RFC 8032; `hashlib` only, no third-party dependency) so the SEC handshake (SEC-3) is testable in a stdlib-only environment: `generate_keypair` / `publickey` / `sign` / `verify`, with the S-range malleability check and a `verify` that is total on adversarial input (returns False, never raises). Extended-coordinate arithmetic for speed (~tens of ms/op).
- **`services/common/handshake.py`** — the submission handshake (SEC-1/2/3/5): `make_envelope` signs a payload with the logger's Ed25519 key; `verify_envelope` checks the signature + freshness (a ±window on the timestamp) + nonce-replay, binding EVERY field (payload/nonce/ts/alg/attestation) into the signature so none can be swapped. The project-unique "genuine logger" proof (SEC-4 — the separable closed/paid piece) is a pluggable `attestation` hook with a stdlib HMAC stub; the open core honestly discloses that a random self-generated keypair is NOT proof of a genuine logger without it.
- **`services/common/bg_runtime.py`** — the BG-1…4 always-on runtime: a cron-like `Scheduler` (deterministic `due`/`run_due`/`health`), a self-check (BG-3 — a task is stale if it hasn't SUCCEEDED within 2× its interval; a failing task never crashes the loop), per-OS boot-start + restart install descriptors (systemd `Restart=always`+`WantedBy=multi-user.target` / launchd `KeepAlive`+`RunAtLoad` / Windows Task-Scheduler `BootTrigger`+`RestartOnFailure`), and a `LogShipper` interface (BG-4) with a stdlib local-JSONL fallback. The thin daemon loop is honest that it does not itself daemonize — it GENERATES what the operator installs/runs.
- **`services/common/service_config.py`** — the decided same-Anthropic-key model (`llm_key == signup_key == anthropic_key`, resolved from config or `ANTHROPIC_API_KEY`, masked by `redacted()`), plus the `LLMClient` adapter interface (the real Anthropic call is a lazy, documented boundary; `FakeLLMClient` for tests).
- **Adversarial SECURITY review (producer ≠ checker): SHIP.** It independently validated the hand-rolled Ed25519 byte-for-byte against libsodium (`cryptography`) across 50 random trials AND the authoritative RFC 8032 §7.1 vectors — no forgery path, `verify` total on adversarial input, malleability closed. Its one genuinely-concerning item — the BG descriptor generators interpolated `name`/`command` without escaping (a *latent* injection; no untrusted caller today) — was remediated: XML-escaping for the launchd/Task-Scheduler descriptors + a control-char guard for the systemd/schtasks fields, with tests.
- Tests: new `tests/test_services_common.py` (26 cases — the Ed25519 KAT/round-trip/determinism/tamper/malleability/non-canonical-point, the handshake replay/stale/attestation/field-binding, the scheduler/self-check/descriptor/log-shipper, and the config/key model). Suite 4649 → **4675 passing + 5 skipped** (186 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. Skill/agent/command counts unchanged (the service tier is not a skill/agent/command); NO new Layer-3 tool.

## [3.22.0] — 2026-06-16 — Token compression (CT6-6 component 6: TC-1…TC-3)

**MINOR — a "caveman" internal-communication token-compression discipline, the SIXTH and FINAL in-repo component of the Claude Team 6 (CT6-6) program.** Agents spend tokens on two kinds of text: what they say to the OUTSIDE (users, other systems, deliverables) and what they say to THEMSELVES and EACH OTHER. The outside must stay full-quality; the internal can be terse. *"Optimize agents' internal communication without harming their external communication; incorporate a token-compression approach ('talk like a caveman') that reduces internal verbosity."*

- **New skill `skills/token-compression/SKILL.md`** over a **new stdlib-only engine `scripts/token_compression/caveman.py`**. `compress(text)` reduces verbosity meaning-preservingly (TC-2): it drops pure filler (articles `a`/`an`/`the`, politeness `please`/`kindly`, intensifiers/hedges `just`/`really`/`very`/…) and wordy phrases (`in order to` → `to`), while PRESERVING content words, identifiers, numbers, line structure, and — critically — fenced (` ``` `) and inline (`` ` ``) code verbatim, AND keeping prepositions/conjunctions/copulas so the compressed text stays understandable to the agent that re-reads it. `compression_stats(text)` reports the original-vs-compressed token estimate + the saved percentage (~30% on realistic internal text).
- **The hard internal-only boundary (TC-1)** — the skill states it as a NON-NEGOTIABLE rule: NEVER compress external output (the final answer to the user, API payloads, commits, file contents, PRs, emails, test output). It applies ONLY to inter-agent messages + an agent's own scratch / internal notes / re-read context.
- **TC-3** — the stdlib caveman compressor is the FLOOR (always available, zero dependencies). A heavier ML token-compression package (LLMLingua-style prompt compression) is documented as a third-party, app-layer option evaluated against `compression_stats` as the baseline to beat — NOT bundled into this stdlib-only plugin.
- **Honest boundary** — a lossy-of-FILLER heuristic, NOT a semantic ML compressor; token counts are estimates (~4 chars/token); a single-letter content token outside backticks (a variable `a`, the grade `A`) can be dropped as an article — wrap it in `` ` `` so the code-preserve path keeps it.
- **Adversarial review (producer ≠ checker): SHIP** (no blockers). It verified code-preservation, idempotency, null-safety, stdlib-only, and a measured ~30% saving without unreadability; the two minor findings were remediated — the prose↔code boundary-space glue (a single boundary space is now preserved when rejoining) and a broadened content-homograph disclaimer; the recommended boundary-space / unbalanced-backticks / compress-to-nothing tests were added.
- Tests: new `tests/test_token_compression.py` (15 cases — filler drop + content/preposition retention, code + line-structure preservation, phrase subs, the boundary-space + unbalanced-backtick + compress-to-nothing edges, and the CLI). Suite 4632 → **4649 passing + 5 skipped** (185 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. NO new command / agent / Layer-3 tool.

**This release completes the in-repo CT6-6 tier — components 1–6 (the Data Dictionary v3.17.0, Closeout v3.18.0, Claude.md efficiency v3.19.0, MCP design agents v3.20.0, Logit/Helpdesk v3.21.0, and Token compression v3.22.0).** The remaining CT6-6 program tiers (the seeded-MemPalace service, the Librarian / Session-Review / Evaluator background services, the triage-server security handshake, and the two-repo split) require external infrastructure and are designed/scaffolded separately, not shipped here.

## [3.21.0] — 2026-06-16 — Logit / Helpdesk (CT6-6 component 5: HD-1…HD-3)

**MINOR — a manual triage-submission skill, the fifth in-repo component of the Claude Team 6 (CT6-6) program.** The MANUAL counterpart (HD-1) to the (server-tier) automatic issue logging: *"the user runs it after a session that went badly; it asks for consent and what privacy level to apply, and the submission follows the same triage process as the automatic path."* Built reuse-first; the actual transmission mirrors the existing `scripts/notify/notify.py` best-effort-emit pattern (by reference, not duplicated).
...
- **Three privacy levels (HD-2 / EVAL-15…17)** — `full` shares code/data snippets (EVAL-15); `summary` keeps ONLY a safe ALLOW-LIST of structured fields (`summary` / `category` / `what_happened` / `agent_could_not_solve`) and drops everything else — code/data snippets, nested objects, unknown keys, non-object items — so nothing identifiable is sent (EVAL-16, default-deny); `off` produces no submission at all (EVAL-17). Both evidence AND issues go through the redaction so neither channel can leak.
- **Honest boundary** — the engine PRODUCES the privacy-applied submission locally; the actual SEND to the triage server (the **SEC** handshake + the **EVAL** server) is the **server-tier**, designed separately and NOT part of this in-repo plugin. The skill + command both state, explicitly, never to claim a report was "sent to triage" when only the local payload was produced (`transmitted: false` in the payload).
- **Adversarial review (producer ≠ checker) returned FIX-FIRST; remediated a real privacy leak + 2 robustness bugs:** (1) `summary` redaction was a fixed 6-key DENY-LIST that demonstrably shipped `secret` / `token` / `email` / `url` / nested-dict code (and the validator checked the same list, greenlighting the leak) → inverted to an ALLOW-LIST (default-deny, recurses past nested/unknown keys, drops non-dict items) and the validator now rejects ANY non-allow-listed key as a true backstop; (2) a non-dict evidence item crashed the builder (`AttributeError`) → guarded; (3) `build_submission` accepted `version=None` (an EVAL-8 violation) → now rejected at build. The skill also gained an explicit warning not to paste identifiable content into the retained free-text fields under `summary`.
- Tests: new `tests/test_helpdesk.py` (18 cases — the privacy levels, the allow-list leak regressions incl. unlisted-key / nested-dict / non-dict, consent + version guards, and the CLI build→validate round-trip). Suite 4610 → **4632 passing + 5 skipped** (184 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. NO new agent / Layer-3 tool.

## [3.20.0] — 2026-06-16 — MCP design agents (CT6-6 component 4: MCP-1…MCP-3)

**MINOR — best-in-class output standardization for LLM agents embedded in an application, the fourth in-repo component of the Claude Team 6 (CT6-6) program.** When an application embeds an LLM agent and asks it to PRODUCE something, free-text or best-effort JSON breaks the consuming code intermittently. *"For every case where an agent is asked to produce something, apply a best-in-class deployment pattern — including clear, specific output contracts / return formats — so that its outputs are guaranteed consistent and standardized."* Built reuse-first; the outward-facing counterpart to the CT6-internal `verified-agent-output` framework.

- **New skill `skills/mcp-output-contract-design/SKILL.md` + new agent `agents/mcp-design-agent.md`** over a **new stdlib-only engine `scripts/mcp_design/output_contract.py`**. `build_output_contract(name, fields, ...)` assembles a CLOSED JSON Schema (typed fields, a required set, enums, `additionalProperties: false`) + the structured-output tool the model is FORCED to call; `validate_against_contract(value, contract)` is the runtime guarantee (object-ness, required, closed-object extras, per-field type + enum + array-item type — with `bool` correctly NOT satisfying `integer`/`number` and `int` correctly satisfying `number`); `assess_contract(contract)` emits best-in-class completeness signals (`no-fields`, `open-object`, `nothing-required`, `fields-missing-description`, `no-structured-output-mechanism`).
- **The pattern (MCP-1/MCP-3)** — for every producer point: a closed output contract + a forced structured-output mechanism + validation of every produced value + retry-on-mismatch. **MCP-2** — it fires whenever an agent is embedded INSIDE an application (not one-off interactive chat). The `mcp-design-agent` enumerates an app's producer points and designs a contract for each (bounded Write to `.architect-team/mcp-design/`; it never writes the app's code).
- **Honest boundary** — the engine guarantees REJECTION of a non-conforming value, NOT PRODUCTION of a conforming one (that is the forced-tool mechanism + the retry loop, which is the app's runtime and provider-dependent — "guaranteed consistent" means the app never CONSUMES a value that fails the contract). It is a minimal stdlib validator (no `format` / `pattern` / range / nested-object schemas); the contract's `json_schema` is standard JSON Schema for a fuller validator in the app.
- **Reuse boundary** — `verified-agent-output` (VAO) verifies CT6's OWN agent output inside the pipeline; `mcp-output-contract-design` is the outward-facing discipline for output contracts in the USER's embedded-agent application.
- **Adversarial review (producer ≠ checker): SHIP** (no blockers). It verified the engine's JSON-type subtleties (int-as-number, bool-is-not-int, no array-items crash, sane empty-fields) and the clean VAO boundary; the two minor items it surfaced were remediated — a build-time enum-type guard (an enum whose values don't match the field type is now rejected as unsatisfiable) and a softened "guaranteed" claim — and the recommended `number`-accepts-int / object-no-recurse / empty-fields / enum-mismatch tests were added.
- Tests: new `tests/test_mcp_design.py` (18 cases — build/validate/assess units, the type guards, and the CLI build→validate→assess round-trip). `mcp-design-agent` joins the boilerplate standard-agent set (verified byte-identical by the sync tool). Suite 4588 → **4610 passing + 5 skipped** (183 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. NO new command / Layer-3 tool.

## [3.19.0] — 2026-06-16 — Claude.md efficiency (CT6-6 component 3: CMD-1…CMD-4)

**MINOR — a pointer-style `CLAUDE.md` discipline, the third in-repo component of the Claude Team 6 (CT6-6) program.** A large `CLAUDE.md` that an agent must internalize in full burns context on every turn. When the project has a memory store to point INTO, `CLAUDE.md` should stop being a container and become a pointer: *"on reading Claude.md the agent must not internalize its entire contents; instead it tells the agent where to find things — e.g. 'first read your wake-up script, located at XYZ' — so context is loaded on demand."* Built reuse-first on `mempalace-integration`.

- **New skill `skills/claude-md-efficiency/SKILL.md`** over a **new stdlib-only engine `scripts/claude_md/claude_md_efficiency.py`** (the machine/contract split). `assess_claude_md(text)` scores an existing `CLAUDE.md` for pointer-shape (a `POINTER_MARKERS` heuristic) + size (a byte budget, `CLAUDE_MD_POINTER_BUDGET_BYTES`) and emits advisory signals — `over-budget`, `no-pointers`, `missing-standards-pointer`, `missing-customizations`; `generate_pointer_claude_md(...)` emits a minimal, correctly-shaped pointer doc (a wake-up first step + a standards section pointing to the reference MemPalace + toggleable customizations).
- **CMD-1 conditionality honored** — the discipline applies WHEN (and only when) MemPalace is installed: with a reachable MemPalace there is somewhere to point INTO, so `CLAUDE.md` becomes a pointer; with none, a self-contained `CLAUDE.md` is correct and the discipline does NOT apply. The engine is precondition-agnostic (it reports shape/size); the skill carries the stop-if-absent gate and delegates detection to `mempalace-integration` rather than reinventing it.
- **CMD-2/3/4** — pointer-not-container + load-on-demand (CMD-2); a byte budget keeps it small (CMD-3); the two parts — standards→reference MemPalace + toggleable customizations (CMD-4) — are emitted by the generator and detected by the assessor.
...
- **Adversarial review (producer ≠ checker): SHIP** (no blockers). It verified the engine at every boundary (empty/None, exactly-at-budget vs +1, multi-byte UTF-8 byte counting, long-name overflow flagged loudly) and confirmed the CMD-1 conditionality is handled correctly; the recommended boundary tests (empty input, at-budget off-by-one, byte-counting) were added.
- Tests: new `tests/test_claude_md_efficiency.py` (12 cases — the assessor on container vs pointer docs, the generate→assess round-trip, the CLI, and the boundary pins). Suite 4574 → **4588 passing + 5 skipped** (182 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. NO new command / agent / Layer-3 tool (skill + engine only).

## [3.18.0] — 2026-06-16 — the `closeout` capability (CT6-6 component 2: CO-1…CO-3)

**MINOR — a session-end documentation-currency double-check, the second in-repo component of the Claude Team 6 (CT6-6) program.** Work is not done when the code is done — it is done when the code AND the documentation reflect what changed. The closeout closes that gap: *"a closeout agent that fires before any suggestion to compact context, reviews the documentation and changes against the requirements, and — if the documentation looks lax — performs the update itself."* Built reuse-first on the existing `documentation-currency` discipline + the `doc-updater` whole-file-rewrite pattern; the novel piece is the fire-before-compact trigger.

- **New `PreCompact` hook `hooks/precompact-closeout.py` (CO-1)** — the first `PreCompact` wiring in `hooks/hooks.json`. Fires before context is compacted: runs the deterministic engine against the working tree and, when docs look stale, injects a closeout reminder (delivered on both `systemMessage` and `hookSpecificOutput.additionalContext`). It is a DOUBLE-CHECK, not a gate — it ALWAYS exits 0 (never blocks/delays compaction) and fails open on any error (unreadable payload, git failure, import failure). A hook is deterministic code and cannot author doc prose; performing the update is the agent's job, prompted by the reminder.
- **New deterministic engine `hooks/closeout_check.py` (stdlib-only)** — classifies a session's working-tree changes (via `git status --porcelain`, handling renames / staged-adds / copies / quoted+spaced paths) into currency-docs / code / version-source / new-surfaces, and emits advisory staleness `signals`: `code-changed-no-doc`, `version-bumped-no-changelog`, `source-changed-no-changelog`, and `new-surface-undocumented`. Its currency-doc inventory MIRRORS `documentation-currency` (pinned by a test against drift — the v3.13.2 gap class). The `new-surface-undocumented` signal keys off the SPECIFIC inventory-count docs (`README.md` / `CLAUDE.md` / `docs/CODEBASE_MAP.md`) so a lone CHANGELOG touch cannot mask a stale inventory grid.
- **New skill `skills/closeout/SKILL.md` (CO-2/CO-3)** — the contract: when fired (by the PreCompact reminder, at any end-of-work boundary, or via the command), review the changes against the REQUIREMENT (not just structural doc-staleness), confirm every affected doc in the currency inventory, and — if lax — perform the update itself via the `documentation-currency` + `doc-updater` pattern. Honest boundary: the engine signals are heuristics; a clean result does not prove semantic currency, and a signal is never silenced by editing a doc that did not need changing.
- **New agent `agents/closeout-agent.md`** — the spawnable worker for an isolated/heavy closeout pass. Write-only (no `Edit` — whole-file rewrites like `doc-updater`), bounded to the documentation-currency inventory. It operates from the working-tree diff alone, so it functions OUTSIDE a full pipeline run — the genuine justification for not reusing `doc-updater`, which depends on the coverage-map + run-ledger inputs the closeout context lacks.
- **New command `/architect-team:closeout` (21st)** — the manual trigger (`--check` for read-only signals; default performs the review + update). The same mechanism the PreCompact hook fires automatically.
- **Adversarial review (producer ≠ checker) returned FIX-FIRST; remediated 4 findings:** (1) the deterministic CO-2 check had a false-negative — any CHANGELOG touch silenced `new-surface-undocumented`, so a new skill/agent with a stale README read as "current" → the signal now keys off the specific inventory-count docs; (2) the porcelain parser missed copies (`C`) and renames (`R`) in its added-file detection → added; (3) `_resolve_repo_root` fell back to the plugin's own install dir when the payload omitted `cwd` → now prefers the process cwd; (4) the tests blessed the gaps → backfilled (the CHANGELOG-touch case, rename/staged-add/spaced-path parsing, a multi-file mixed change, the `additionalContext` content).
- Ripple: `EXPECTED_SKILLS` 42→43, `EXPECTED_AGENTS` 37→38, `EXPECTED_COMMANDS` 20→21; the frozen `CANONICAL_COMMANDS` fallback + `test_canonical_commands_has_20_entries` (now 21); `agent_boilerplate_blocks.py` (`closeout-agent` added to the standard-agent set — boilerplate verified byte-identical by the sync tool). The README HOOKS box was also brought current (it had silently omitted the v3.15.0 skill-gate row).
- Tests: new `tests/test_closeout.py` (29 cases — engine units, the staleness signals incl. the M1 regression pin, the working-tree collector against a real temp git repo, the PreCompact hook as a subprocess incl. fail-open, and the surface/wiring checks). Suite 4539 → **4574 passing + 5 skipped** (181 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. NO new Layer 3 tool.

## [3.17.0] — 2026-06-16 — the `data-dictionary` skill (CT6-6 component 1: DD-1…DD-18)

**MINOR — a new self-contextualizing data-dictionary builder, the first in-repo component of the Claude Team 6 (CT6-6) program.** Given code and/or documentation that reference data tables/databases, it derives the data model, defines every field, and records provenance + corroboration — answering *"what does this field mean, where does it come from, and which code touches it."* Built reuse-first against the existing `*-mapping` skill family, with the deterministic parts as a stdlib-only engine + the LLM-judgment workflow as the skill contract (the same machine/contract split as `lineage_graph.py` ↔ `data-lineage-mapping`).

- **New skill `skills/data-dictionary/SKILL.md`** (the contract) over a **new stdlib-only engine `scripts/data_dictionary/data_dictionary.py`** (the machine). The skill branches by input type — code / documentation / direct user input (DD-2); recursively follows objects that mask DB connections to find table references (DD-3/4); maps doc definitions onto the field list (DD-5); sequences AFTER codebase + integration mapping (DD-6) with the standard-name freshness check (DD-16).
- **The engine (DD-7 … DD-14):** SQLite introspection (schema/columns/FKs) + ~100-row sampling (DD-9/10); grain inference from declared PK + sampled uniqueness (DD-11); field inference from name + declared type + sampled values (DD-12); the FIXED provenance vocabulary `direct-user-input` / `direct-code-comment` / `inference` / `live-data` (DD-13); **value-level corroboration of EVERY provided definition against the real data — key claims AND type claims, not only keys** (DD-14): the classic "user says it keys on `customer_id`, the data keys on a hash" conflict is flagged AND a claimed type the data contradicts (e.g. "boolean flag" on a free-text column) is flagged, both surfaced (⚠) with confidence downgraded; the by-field/by-table reference map + the relational/blend map incl. non-DB code joins, e.g. census merged onto customers on zip (DD-7); the `DATA_DICTIONARY_MAP.md` (+ `data-dictionary.json` sidecar, `schema: data-dictionary/v1`) serializer (DD-7/16).
- **Honest live-DB boundary.** Live inspection (DD-9/10) needs a reachable DB with valid credentials — not available in this environment, so the live path is dogfooded against a local throwaway SQLite stand-in, not claimed against a production DB. The no-DB `build_from_inputs(...)` path builds from code + docs + provided context only: NO field is ever marked `live-data`, and the artifact's `live_inspection` block records `ran: false` with the reason (the serializer renders a "Live inspection: NOT run" line). An inferred key seen in only a handful of sampled rows (`< MIN_KEY_SAMPLE`) is hedged in the grain string and never asserted above `medium` — N distinct values in N rows is not proof of a key. MemPalace persistence via `mempalace-integration` if available (DD-15); the maintenance discipline (DD-17/18) mirrors `documentation-currency`.
- **Adversarial review (producer ≠ checker) returned FIX-FIRST and drove 5 pre-ship fixes:** (1) DD-14 was implemented for key-claims ONLY — every other provided definition was asserted `high` confidence unverified → now `corroborate_definition(...)` corroborates all provided defs; (2) a SQL-identifier crash — table names were interpolated into PRAGMA/SELECT without doubling embedded quotes (a legally-named table `a"b` crashed) → a `_q(...)` escape on all three sites + a corrected "trust ≠ escaping" comment; (3) the SKILL documented a `live_inspection` artifact field + a no-DB build path that did not exist → both implemented; (4) un-hedged small-sample inferred keys → the `MIN_KEY_SAMPLE` hedge; (5) the tests blessed exactly these gaps → backfilled.
- Tests: new `tests/test_data_dictionary.py` (21 cases — deterministic units, the end-to-end local-SQLite dogfood reproducing the requirements doc's customer_id-vs-hash + census-on-zip example, plus the 9 remediation pins: non-key corroboration, empty table, all-null column, small-sample hedge, populated reference-map render, the no-DB path, the live_inspection block, and the quoted-identifier crash). Verified by a real CLI dogfood (emit → rendered `DATA_DICTIONARY_MAP.md` with live_inspection + grain + field inference). Suite 4516 → **4539 passing + 5 skipped** (180 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. NO new Layer 3 tool.

## [3.16.0] — 2026-06-16 — responsive + parallel `/architect-team:inject` (concurrent in-run lanes)

**MINOR — make `/architect-team:inject` responsive and able to spawn concurrent work.** Verbatim driver: *"even running as teams, inject just sits there passively and I need it to spawn more teams so we can have multiple problems worked on in parallel … our listener is just caught up with other stuff."* Two root causes: (1) the orchestrator only READ the inbox at phase boundaries and blocked synchronously on teammate dispatches, so an inject sat until a boundary; (2) the v2.5.0 discipline DELIBERATELY forbade spawning a parallel team (`spawn-sibling-invocation` was a banned anti-pattern), so an inject could only be FOLDED into the single sequential flow — never opening a second lane.
...
- **Responsiveness (poll on every wake):** the inbox-check protocol (canonical `## In-flight clarification injection mechanism (v2.19.0)` + all 3 pipeline bodies) now drains the inbox at every phase boundary AND after every background-dispatch return / wake — with background dispatch (`run_in_background: true`) freeing the Lead's turn — not only at phase boundaries.
- **Sanctioned lanes vs forbidden siblings:** the `spawn-sibling-invocation` anti-pattern is AMENDED — a fragmenting second `/architect-team` RUN (two coverage-maps / openspec-changes / commit-ranges) stays FORBIDDEN; a concurrent in-run LANE (one run's state, disjoint lock, converges via Phase 4) is the NEW sanctioned mechanism. Canonical home: `## In-flight clarification discipline (v2.5.0)` `### Parallel lanes (v3.16.0)`.
- **Honest about the limits** (adversarial-review-driven): the responsiveness is POLLING on every wake, not async push/preemption (a message injected mid-tool-call is serviced when the call returns); lane isolation is `globs_intersect` FILE-GLOB-level + ADVISORY — `cdlg_overlap` is NOT wired into `acquire_lock`, so a transitively-shared hot callee or an out-of-glob edit is NOT caught (keep lane scopes coarse + independent; Phase 4 is the backstop); and background lanes + every-wake polling are TEAMS-mode primitives that degrade to sequential in the subagents-mode fallback. A failed lane spawn downgrades the message to `clarification`/`scope-amendment` rather than wedging Phase 8.
- Tests: new `tests/test_parallel_lane_inject.py` (13 cases — classification + lane_id contract, the end-to-end dogfood append→read→lock-isolated-lane→processed→verified, overlapping-scope-blocked, and doctrine/honesty pins). Suite 4503 → **4516 passing + 5 skipped** (179 test files), green under cp1252 + `PYTHONUTF8=1`. Verifier (`verify_inflight_clarifications_processed`) unchanged in contract; no new Layer 3 tool.

## [3.15.1] — 2026-06-16 — skill-gate narrowing fix (stop over-firing on the command wrapper)

**PATCH — fixes the v3.15.0 `pretool_skill_gate.py` over-firing on the slash command's own pre-Skill setup steps.** Verbatim driver (observed on a 3.15.0 server): `/architect-team` was invoked, the model ran the command's documented FIRST step — the dispatch banner (a Bash call) — and the gate BLOCKED it (`tool about to fire: Bash <- blocked`), along with `ToolSearch`. Root cause: the gate used a `*` matcher that blocked **every** non-`Skill` tool, but the `/architect-team` command is a WRAPPER that by spec runs the dispatch banner + merged-worktree cleanup + worktree creation (all Bash) BEFORE it invokes the pipeline Skill (`commands/architect-team.md` "runs first" sections). The gate conflated "the slash command" with "the skill" — two layers, with legitimate setup tools between them.

- **Fix:** the gate now blocks ONLY build/dispatch tools before the Skill — `_BLOCKED_TOOLS = {Edit, Write, NotebookEdit, Agent, Task, TaskCreate, TaskUpdate, TaskStop}` — and returns allow immediately for any other tool. Read-only investigation (`Read`/`Grep`/`Glob`/`ToolSearch`/…) and the command wrapper's own `Bash` setup are never blocked; a normal run invokes the Skill before any build/dispatch tool, so the gate never fires on a well-behaved run. It fires ONLY on a genuine attempt to BUILD (`Edit`/`Write`) or DISPATCH (`Agent`/`Task`) by hand before loading the pipeline — which still fully covers the original bypass.
- **Verified on real data:** re-simulated across 9 real transcripts / 3939 tool calls — blocks now occur ONLY on build/dispatch tools (Edit 140, Agent 26, Write 24, TaskUpdate 9, TaskCreate 5 = 204 genuine catches); ZERO non-build/dispatch tools blocked (1036 Bash, 536 Read, ToolSearch, etc. all allowed). Adversarial review: SHIP (no blockers); it flagged + I corrected a docstring overclaim and documented the one inherent residual (a model could still build via raw `Bash` heredocs — Bash can't be blocked without breaking the wrapper; that lane is backstopped after-the-fact by the Layer-6 `skill_invocation_audit` + `verify-no-pipeline-bypass`).
- Tests: `tests/test_pretool_skill_gate.py` updated for the narrowed behavior + new regressions (`test_wrapper_banner_bash_before_skill_is_allowed`, `test_known_limitation_bash_and_sendmessage_not_blocked`, build-vs-setup tool split). Suite 4501 → **4503 passing + 5 skipped** (178 test files), green under cp1252 + `PYTHONUTF8=1`. No behavior change to the prior brick/precedence/anti-substring fixes.

## [3.15.0] — 2026-06-16 — skill-invocation hard-gate (real-time pipeline-bypass prevention)

**MINOR — a new PreToolUse hook that converts skill-invocation enforcement from after-the-fact DETECTION into real-time PREVENTION.** Verbatim driver: *"sometimes when I call for the architect team, the AI doesnt load the skill"* / *"you invoked the skill and I've been driving it by hand instead of running it."* When a user invokes a pipeline-driving command (`/architect-team:architect-team`, `/architect-team:bug-fix`, `/architect-team:ux-test`, `/architect-team:mini`, `/architect-team:refine-prompt`), the command body *instructs* the model to invoke the underlying Skill — but nothing FORCED it, and the only guard (`hooks/skill_invocation_audit.py`, Layer 6) is an after-the-fact auditor that flags a miss only at end-of-turn (and isn't even wired into `hooks.json`). Soft prompts (CLAUDE.md directive, command body, using-superpowers) are routinely rationalized past; deterministic code cannot be.

- **New hook `hooks/pretool_skill_gate.py`** (stdlib-only), wired as `PreToolUse[*]`: when the session transcript's most-recent GENUINE user prompt is an unsatisfied pipeline-command request, it BLOCKS (exit 2) the first non-`Skill` tool call (Read/Bash/Edit/Write/Agent/…) until a matching `Skill` call appears. The instant a pipeline skill is engaged, the gate opens.
...
- **False-block-safe by construction** — the catastrophic failure mode for a `*`-matcher hook is a spurious block. The anchor excludes the harness's injected/meta records (`isMeta` command/skill body-echoes, `promptSource:"system"` task-notifications, `isSidechain` subagent transcripts); satisfaction = engaging ANY pipeline skill after the request (so `/architect-team`'s documented first step — invoke `proposal-refiner`, THEN the pipeline — never false-blocks) with timestamp ordering for user-precedence; the `Skill` tool is always allowed; fail-open on missing/unreadable transcript or ANY error. **Adversarial review against two real transcripts caught a session-bricking `isMeta` body-echo bug and a refiner-first false-block** before ship; the fix was re-verified across 9 real transcripts / 3822 simulated tool calls: **0 spurious blocks, 402 genuine historical bypasses correctly caught.**
- **Scoped** to the 5 pipeline-driving commands only — read-only plugin commands (`/status`, `/memory`, …) and built-in REPL commands (`/effort`, `/model`, `/login`) never gate.
- Tests: new `tests/test_pretool_skill_gate.py` (38 cases — gate open/close, Skill-always-allowed, user-precedence, the real nested transcript shape, the injected-meta/sidechain/substring regressions, fail-open, universality, wiring, end-to-end subprocess incl. cp1252). The existing PreToolUse wiring test was updated for the two-hook (matcher-routed) reality. Suite **4463 → 4501 passing + 5 skipped** (178 test files), green under both Windows cp1252 and `PYTHONUTF8=1`. No new Layer-3 tool; the Layer-6 `skill_invocation_audit.py` detection is reused unchanged.

## [3.14.0] — 2026-06-11 — appearance-change policy (strict / propose / innovate)

**MINOR — a new cross-pipeline discipline closing the unsolicited frontend-appearance-change gap.** Verbatim driver: *"sometimes when asking for updates, the agent will arbitrarily change our front end, adding things we didnt explicitly ask for as part of an ask to improve … by default we are strict on appearance changes with a no unless explicity asked or given direction to do so."* Three modes now govern what a user SEES: **`strict`** (the DEFAULT — no appearance-affecting change beyond the explicit mandate; backend changes unrestricted, *"do what you need to on the backend"*; improvement ideas RECORDED as proposals, never implemented), **`propose`** (proposals batched at ONE multi-select `AskUserQuestion` approval gate — a domain gate per v0.9.21; only approved ones implemented), **`innovate`** (authorized — and every delta logged + `DESIGN_MAP.md`-reconciled in the same change). This is the inverse axis of the v3.10.0 scope-fidelity family: the family catches under-delivery (narrowing / deferring); this catches visual over-delivery.

- **Canonical home** — `common-pipeline-conventions` `## Appearance-change policy discipline (v3.14.0)`: the three modes; the appearance-affecting classification (visual styling / UI-surface additions-removals-relocations / displayed copy the requirement does not name / asset swaps — wiring with no visible delta, backend-only changes, pure a11y attributes, and test files excluded); the three sanctioned mandate sources under `strict` (**requirement text** — only the NAMED surfaces; **spec restoration** — `DESIGN_MAP.md` / design source / the intended rendering a bug broke, ALWAYS in scope in every mode; **mandated-capability minimum** — the smallest surface a required capability needs, matching the existing design system, zero decorative extras); mode selection (`--appearance <strict|propose|innovate>` + parse-time natural-language equivalents + requirement-prose detection with an intake `AskUserQuestion` domain gate on genuine look-may-change ambiguity); the proposals artifact `<workspace>/.architect-team/appearance-proposals/<run-id>.json` (statuses `recorded` / `approved` / `rejected` / `implemented-approved` / `implemented-innovate`); the v2.10.0 interplay (unimplemented proposals are NOT in-scope items in the v2.10.0 sense — `recorded` / `rejected` is their sanctioned terminal disposition; the strict report listing is imperative, never interrogative, so it cannot trip `_FOLLOWUP_QUESTION_MARKERS`); the completeness-SR interplay (an SR whose remediation would ADD new visible UI surface is `appearance_gated: true` and user-gated under strict/propose — detection unchanged, only implementation timing gated; pure-wiring SRs route unchanged in every mode — generalizing the v2.18.0 "SR-route-only, UX decision, not mechanical" rule); 5 forbidden anti-patterns (while-I'm-here restyling, unsolicited UI surface, redesign-while-wiring, implement-then-confess, improvement-framing-as-cover) + 8 canonical reviewer markers (*"while I was at it"*, *"took the liberty"*, …).
- **Schema v7 gains a THIRD OPTIONAL field** — `hooks/review_evidence_schema.py` adds `appearance_scope_review` to `OPTIONAL_VAO_FIELDS` (+ `VALID_APPEARANCE_SCOPE_VALUES = {"pass", "n/a", "fail"}` + the guarded validator — exactly the v2.1.0 `interactions_honored_review` / v2.2.0 `live_verification_review` extension shape). Required when the slice's diff touches frontend presentation surface (styling files, components, templates, routes, assets); string OR `{verdict, verdict_path}` dict; the hook BLOCKS `fail` with a targeted revert-or-propose message; `n/a` requires a non-empty `appearance_scope_review_note`; `REQUIRED_EVIDENCE_FIELDS` stays at **17** and pre-v3.14.0 evidence files remain valid (the same optional-ness backwards-compat guarantee, third application).
- **Pipeline wiring** — `architect-team-pipeline` gains `### Phase −2 appearance-mode binding (v3.14.0)` (bound alongside the deploy-mandate detection; persisted to `intake-state.json`; carried in every teammate spawn brief like `deploy_mandate` / `baseline_sha`) + a Phase 8 final-report proposals rule (strict: read-only listing citing the artifact path, imperative phrasing; propose: per-proposal gate decisions with citations; innovate: every `implemented-innovate` delta enumerated). `bug-fix-pipeline` + `mini-architect-team-pipeline` gain `## Appearance-change policy (v3.14.0)` sections — a bug-fix run is `strict` by nature (the mandate is the named symptom; spec restoration in scope; restyling beyond it not).
- **Commands** — `--appearance <strict|propose|innovate>` documented on `/architect-team`, `/architect-team:bug-fix`, `/architect-team:mini` (each with natural-language equivalents); the no-flags default lines gain `APPEARANCE_MODE = strict`.
- **Agents** — `agents/frontend.md` (record-never-implement; no redesign-while-wiring; no implement-then-confess — the v3.0.0 unilateral-override pattern on the appearance surface; honest `appearance_scope_review`; innovate = freedom + logging + DESIGN_MAP reconciliation; + a hard rule), `agents/task-reviewer.md` (per-delta mandate trace cited like `file:line` criteria; an untraceable delta is a `spec_review` gap; a `pass` self-review over an untraceable delta is a lying self-review → verdict `fail`; + a hard rule), `agents/system-architect.md` (Master Review Audit run-level diff walk + proposals-artifact integrity — no `approved` without `user_citation`, no `implemented-innovate` outside innovate mode + `appearance_gated` SR-routing check + report-phrasing check; + a hard rule).
- **What v3.14.0 does NOT ship** — NO new Layer 3 tool (the v3.5.0 phenotype-convergence-rules precedent: gate + reviewer + schema enforcement first); a deterministic `verify-no-unsolicited-appearance-change` (pure-style-file diff scan + declared-delta / mandate-ref cross-check against the proposals artifact) is the named v3.14.x follow-up. The 20 Layer-3 CLI subcommands are unchanged.
- **Docs** — `team-spawning-and-review-gates` teaches the 3-optional-field contract (ground-truth sentence, the present-only-when-applicable paragraph, a new validity bullet); README gains `### ▸ Constrain appearance changes (v3.14.0)` in USAGE + the 3-optional-fields line + a v3.14.0 timeline entry; CLAUDE.md + CODEBASE_MAP + INTEGRATION_MAP schema regions updated in lockstep.
- **Tests** — NEW `tests/test_appearance_change_policy.py` (**48 tests**): canonical-section pins (heading-line uniqueness, verbatim driver prose, the 3 modes + default, flag spelling, mandate sources, proposal statuses, anti-pattern markers, the NOT-ship note, the v2.10.0 + SR interplays), 3-pipeline + 3-command + 3-agent + team-spawning + README cross-wiring pins, and the full optional-field semantics (absent / pass / n/a-with-note / n/a-without-note blocks / fail blocks / dict-shape / dict-missing-verdict_path blocks / 17-required-unchanged / prior optional fields intact). Suite 4415 → **4463 passing** + 5 skipped (177 test files; both encodings).
...
- **Source:** `https://github.com/AsyncFuncAI/deepwiki-open.git` (MIT license), cloned READ-ONLY into `.architect-team/reference/deepwiki-open/` for P1–P2 analysis; its pattern, not its files, is the absorption product (`provenance.absorbed_from`).
- **Kind = SINGLETON** (the P2 verdict): deepwiki ships as a Next.js frontend + a FastAPI backend, but the backend exists only for (1) LLM-backed chat/RAG and (2) a thin filesystem JSON store; strip the LLM and the two surviving non-LLM reads (wiki-cache GET, projects-list GET) fold into Next.js server-component filesystem reads. **No separate content server is needed** — the generalized phenotype is one Next.js app, one port, one content volume.
- **Strip inventory (the stripped-LLM delta):** the 9 provider clients (OpenAI/OpenRouter/LiteLLM/Bedrock/Azure/DashScope/Google/Ollama + embedders), the embedding/RAG pipeline (faiss/adalflow/tiktoken), the WebSocket chat + Ask + Deep Research, the slides/workshop generation, the model/provider-picker UI, every API key, and the entire `api/` Python tree — all die. Wiki **viewing** (tree, markdown, Mermaid, syntax highlight, theming, the multi-codebase index) survives and is the phenotype's body.
- **Wiki-cache JSON contract (kept verbatim):** `WikiStructure { id, title, description, pages[], sections[], rootSections[] }` + `WikiPage { id, title, content (markdown w/ inline ` ```mermaid ` blocks), filePaths[], importance, relatedPages[] }` — deepwiki's `WikiCacheData` shape, so the viewer is unchanged; only the SOURCE changes (filesystem maps, not an LLM).
- **Maps-ingestion contract:** `lib/maps-loader` reads `codebases.json` (a registry of `{name, maps_dir}` entries under `WIKI_CONTENT_DIR`), enumerates each codebase's `docs/*_MAP.md`, and builds the wiki content shape — one section per codebase, one page per map, content = the map markdown verbatim, importance by map kind. Mermaid rides inside the markdown.

### Scaffold shape (30 templates)

A lean generalized Next.js starter (App Router, `output: 'standalone'`), **plain CSS** (the 10-variable CSS-custom-property palette generalized from deepwiki's `globals.css`; the Tailwind-v4 toolchain dropped to keep the dependency set minimal): `next + react + react-dom + react-markdown + remark-gfm + mermaid`. App shell (theme-init layout + globals.css), a codebase selector (card grid + client search), a **URL-backed** wiki viewer (`/wiki/[codebase]/[pageId]` — the deep-link improvement over deepwiki's `useState`-only nav), the recursive `WikiTreeView` (importance dots + flat fallback), the `Markdown` pipeline (code-block copy button + Mermaid fence dispatch), the client `Mermaid` component (render-to-SVG, palette-matched `themeCSS`, English strings, resolved-theme-keyed dark mode), `lib/maps-loader` ingestion, a `/health` route, `Dockerfile` + `docker-compose.yml` (single container, content volume, healthcheck), and `iac/aws` + `iac/gcp` **service-layer plug-ins**.

### Cross-seed

`components.deploy.via = "config-management phenotype"` (the **ai-management precedent** — no schema extension): cloud hosting composes with the `config-management` phenotype for the platform/load-balancer/registry layers; the code-wiki scaffold ships only the service-layer module call (ECS Fargate on AWS / Cloud Run on GCP), shaped to config-management's `module_inputs` contract (`project`/`service`/`env`/`image`/`enable_*`; naming `${project}-${service}-${env}`), reading the platform via `terraform_remote_state`.

### Demo evidence (executed, not described)

- **Engine gates:** `python scripts/phenotypes/phenotypes.py validate code-wiki` → `code-wiki: OK`; `match "launch a code wiki for my codebases hosted locally"` → code-wiki score 5; `emit code-wiki <tmp> --dry-run` → lists 30 scaffold files (all exit 0).
- **Local demo (REQ-006):** the scaffold was emitted to a temp dir, `npm install` (237 packages, 18 s), `npm run build` (clean, TypeScript type-check passed), `npm run start`; `codebases.json` pointed at this repo's `docs/`. HTTP transcript: `/health` 200, `/` 200 (lists the codebase), `/wiki/<codebase>` 307→first page, the CODEBASE_MAP page 200 with CODEBASE_MAP-derived content ("architect-team-plugin", "/architect-team:bug-fix"), an unknown codebase 404. A Playwright (chromium) screenshot of the CODEBASE_MAP wiki page captured **2 Mermaid diagrams rendered to SVG + the navigation tree** (svgCount=2, treePages=2, zero console errors). Artifacts under `.architect-team/code-wiki-demo/`.
- **Cloud static validation (REQ-007):** real `tofu validate` (OpenTofu v1.11.5) on both rendered IaC sets — `iac/aws` and `iac/gcp` each "Success! The configuration is valid."
...
Absorbing deepwiki-open clones its frontend repo READ-ONLY into `.architect-team/reference/deepwiki-open/`; `hooks/discipline_registry.py`'s frontend-marker scan globbed `**/*.tsx` and `**/package.json` skipping only `_SKIP_DIR_PARTS = ('node_modules', '.venv', 'venv', '__pycache__', '.git')` — which did NOT exclude `.architect-team/`. So the vendored read-only clone (23 `.tsx` files + a next/react `package.json`) was mistaken for the CT6 plugin repo's own UI surface, flipping `multi-persona-path-coverage` to applicable and breaking `tests/test_discipline_applicability.py`'s two THIS-repo cases. Surfaced by the code-wiki run's Phase 5 RCA (proven clone-only: moving the clone outside `REPO_ROOT` made both tests pass), routed as an SR (`origin.kind: rca-product-bug`) through `diagnostic-research-team`, and fixed by adding `.architect-team` to `_SKIP_DIR_PARTS` (the class fix lands at all 3 scan sites — the `.tsx`/frontend-source glob, the `**/package.json` dependency scan, and the test-file walker) + **4 regression tests** asserting a deepwiki-style reference clone under `.architect-team/reference/` no longer false-flags a no-UI repo. The committed CI was always green (the gitignored clone is absent on a fresh checkout); the fix makes the suite green even with the clone present.

## [3.12.0] — 2026-06-11 — structure-optimization performance + review remediation

**MINOR — behavior-preserving correctness fixes + performance optimizations + new permanent guardrails section, all on the v3.11.0 `structure-optimization` set.** A 3-lens multi-agent review panel (consistency architect + adversarial refuter + performance analyst) audited the just-shipped v3.11.0 pipeline per the user's directive — *"review the code mapping skill we just created and then optimize to maximize performance, then deploy"* — and verified 16 findings: 2 blocking correctness bugs in the deterministic partition-check snippet, 1 dead notifier wiring, 8 material ambiguities an implementing orchestrator would hit, and 3 under-pinned invariants — plus a ranked optimization set with a stage-by-stage cost model showing S5 (unbounded ×3-opus rounds) and S3 (unbounded round-robin) dominate run cost. Every fix and every accepted optimization is an IN-PLACE extension of an existing skill/agent/command/test file — zero new files, zero new dependencies. The minor bump reflects the new user-visible `## Optimization guardrails` section + the new published `partition-check.json` per-round artifact + warm-start brief fields; every accuracy invariant (the deterministic partition check, the full reference closure with mandatory `search_log`, the two-consecutive-clean adversarial exit, the architect all-five-blocks gate, openspec strict validation) is preserved verbatim.

### Correctness fixes

- **C-001** — `skills/structure-optimization/SKILL.md` Stage S8 notify event `pipeline_complete` → `phase_complete` (the canonical terminal event in `scripts/notify/notify.py`; `pipeline_complete` was never a `notify.py` event type, so the notify call was a silent no-op).
- **C-003** — the kind-semantics line now states `delete-dead` carries `"to": []` (the empty target list is the tombstone marker).
- **A-001** — the S3 partition-check snippet parses `git ls-files` with `.splitlines()` instead of a bare `.split()` (a bare `.split()` whitespace-splits and corrupts space-bearing filenames into multiple phantom paths).
- **A-002** — the snippet normalizes paths via `os.path.normcase` on BOTH the tracked set and the movement/stays paths (case-insensitive-filesystem safety: a case-only difference could previously hide an orphan or duplicate on Windows/macOS).
- **A-003** — S3 documents that a duplicate (a path in two movements, or in a movement AND the stays list) is RECOVERABLE: it routes the proposal back to the analysts for revision through S3, never aborts the run.
- **A-005** — S4 assembly rule: the orchestrator merges the shard files into `movements.json` and VALIDATES that every `movement_id` from the converged proposal appears in exactly one shard (a missing/duplicated id fails assembly and re-dispatches the affected shard).
- **A-006** — S6 gains a per-failure-kind re-execution boundary table (objective/structural → S3 re-convergence + S4 re-trace + full S5; closure → S4 re-trace of named movements + full S5; migration-order → batch-plan repair + full S5); every row ends in the full S5 two-consecutive-clean loop.
- **A-007** — `commands/optimize-structure.md` argument precedence: an explicit codebase path takes precedence; a path combined with `--all` is a surfaced error (ask which was meant), never a silent pick.
- **A-012** — S0 documents the orchestrator creates the run directories via `mkdir -p` (Bash) — the agents have bounded Write into their own paths but do not create the tree.
- **A-013** — the partition check is documented as invoked once per codebase in scope (each codebase's movements/stays evaluated against its OWN `git ls-files`; the partition is per-codebase, never a cross-codebase merge).

### Performance optimizations — each {mechanism, target axis, preserved invariant}

- **P-001 — adversary-round warm-start.** Mechanism: after round N the orchestrator computes the round delta (movement_ids whose closure/partition state changed) + carries forward each adversary's `modalities_run` union + clean per-movement evidence; round N+1 re-runs every modality on the delta movements, runs any modality NOT yet in the carried union across all movements, and re-confirms (not re-derives) carried clean evidence for unchanged movements. Axis: all three (wall-clock, tokens, convergence). Invariant: the two-consecutive-clean exit is UNTOUCHED — any revision resets the streak, both exit rounds are all-clean across all three adversaries with non-empty `modalities_run`, the carried union only grows; warm-start changes work INSIDE a round, never the exit rule.
- **P-002 — deterministic-check front-loading.** Mechanism: the orchestrator runs the partition check on EACH draft as it lands (S2) and on EVERY revision (S3, delta attached to the next brief), not only at the gate. Axis: convergence. Invariant: the convergence-gate run remains and still gates the promise.
- **P-003 — per-round partition-recompute dedup.** Mechanism: the orchestrator runs the canonical from-scratch deterministic recompute once per round and publishes `adversarial/round-<R>/partition-check.json`; the three adversaries consume it for the orphan/duplicate dimension instead of three redundant LLM re-derivations. Axis: tokens. Invariant: the from-scratch-every-round property is preserved — by deterministic orchestrator code rather than three opus re-runs of the same arithmetic.
- **P-004 — payload-trimmed briefs.** Mechanism: the per-shard tracer brief carries the shard's movement slice + relevant map sections (not other drafts/full rationale); the per-round adversary brief carries closures + `search_log`s + batches + stays + a fan-in-ordered manifest (not analyst rationale prose). Axis: tokens. Invariant: the tracer still closes its shard completely and the adversary still attacks every movement; trimming removes prose, never a movement or a file.
- **P-005 — executable shard policy.** Mechanism: shards balanced by ESTIMATED reference surface — the orchestrator pre-estimates fan-in from the maps + a basename grep count, isolates each top-fan-in file in its own shard, and batches low-fan-in leaf movements (heuristic N ≈ ceil(total_movements/8), min 3/shard except singleton high-fan-in shards). Axis: wall-clock. Invariant: every movement is closed by exactly one tracer, completely, with file:line evidence — sharding changes who closes what, never the closure obligation.
- **P-006 — structured agree/dispute convergence.** Mechanism: each analyst emits its agreed-set + disputed movements (one-line decisive argument each) + one proposed resolution per dispute; the orchestrator freezes agreed rows between passes and re-dispatches only the dispute set; completion fires when all three sign the identical FULL table AND the orchestrator partition check passes on it. Axis: convergence. Invariant: frozen rows remain part of the final gated table; the gate is the orchestrator's own partition run over the full table.
- **P-007 — S1 per-codebase freshness pipelining.** Mechanism: the orchestrator freshness-checks all codebases first and releases each codebase's S2 analyst inputs the moment THAT codebase's maps are confirmed fresh, instead of blocking the workspace on the slowest cold-map codebase. Axis: wall-clock. Invariant: every in-scope codebase still has a freshness-verified map before its analysts run.
- **P-008 — S7 mechanical transcription.** Mechanism: each movement → a spec REQ keyed by `movement_id`; each `references_in` entry → an acceptance criterion; each batch → a tasks.md group in batch order; the analyst drafts' `approaches_considered` lifted verbatim into design.md. Axis: convergence/tokens. Invariant: every movement, reference, and batch is transcribed deterministically — nothing in the verified plan is lost or re-argued at authoring time.
- **P-009 — S6 thinnest-coverage spot-check sampling.** Mechanism: the system-architect Restructure Plan Audit weights its ≥10-movement spot-check toward the movements with the THINNEST adversary-modality coverage (computed from the union of the final two clean rounds' `modalities_run`). Axis: tokens (higher marginal coverage at the same sample size). Invariant: the ≥10-movement sample size + the fresh-modality requirement are unchanged.
- **P-010 — orchestrator-precomputed file universe.** Mechanism: the orchestrator runs `git ls-files` + a per-directory file-count histogram ONCE per codebase and hands that canonical universe to all three analysts, who partition against it instead of each re-deriving it. Axis: wall-clock, tokens. Invariant: every analyst partitions against the same `git ls-files` universe — precomputing it removes three redundant derivations, never changes the universe each analyst sees.
- **P-012 — 3-adversary floor note.** Mechanism: S5 states the 3-adversary width is a FLOOR on every round including the confirming clean rounds; warm-start trims per-round work, never the adversary count. Axis: documentation (prevents a future tuning round from narrowing the panel). Invariant: two clean rounds at width-3 is the evidence the plan survives hostile judgment.

### Rejected dispositions (recorded, not silently dropped)

- **A-009 / A-010 / A-011** (test-pinned string catalogs constrain refactors) — REJECTED-with-rationale: the pins ARE the intentional structural contract; the constraint is documented in design.md, not a defect.
- **P-011** (per-shard early adversary pass) — REJECTED-with-rationale: small-to-medium gain against a medium risk of mis-reading the clean-round invariant; revisit only with run-metrics evidence from real runs.

### Permanent `## Optimization guardrails` (four anti-candidates, fenced forever)

```
(a) Trusting the analysts' partition self-checks at the gate  — REJECTED (v0.9.13 producer-cannot-be-its-own-checker; the recompute is sub-second).
(b) Exiting S5 after one clean round                          — REJECTED (one clean round is luck; two consecutive is evidence — warm-start makes the second round cheap).
(c) Downgrading the structure-adversary to sonnet            — REJECTED (refutation is open-ended hostile judgment, not pattern-matching; the mechanical slice is offloaded to deterministic code instead).
(d) Dropping the mandatory search_log / modalities_run       — REJECTED (they are how the adversary knows what was NOT run and how a clean verdict differs from not-having-looked; warm-start depends on them).
```
...
Inventory unchanged at **41 skills / 37 agents / 20 commands** (this release edits the v3.11.0 set in place). Suite 4356 → **4385 passing** + 5 skipped (across 175 test files; green under both Windows cp1252 AND `PYTHONUTF8=1`); all new tests additive.

## [3.11.0] — 2026-06-10 — Structure Optimization Pipeline

**MINOR — new capability.** A new planning pipeline that produces an adversarially-verified codebase-restructure plan: it analyzes the code + the codebase maps (producing them via the existing `cartographer-team` machinery when missing or stale), converges three independent restructure drafts, closes every proposed movement's reference-impact set, refutes the whole through dedicated adversary rounds, architect-audits it, and ships the result as `RESTRUCTURE_PLAN.md` + `movements.json` + a strict-validated OpenSpec change. Plan-only by design — execution belongs to `/architect-team` driving the produced change (`--execute` hands off immediately). Backbone per the user mandate: ralph-loop (canonical `--completion-promise` form, no caps), openspec (`openspec-propose` authoring + the uniform `openspec validate --all --strict --json` gate), superpowers (brainstorming at draft time, `writing-plans` conventions for the plan document, `verification-before-completion` before any done-claim) — all per `common-pipeline-conventions` `## Uniform plugin usage (v3.9.0)`.

- **NEW skill `skills/structure-optimization/SKILL.md`** — stages S0–S8. S0 init + MemPalace wake-up + superpowers pre-flight; S1 maps current (delegates to `cartographer-team` with `freshness_check: true`; `INTEGRATION_MAP.md` freshness for multi-codebase workspaces); S2 ×3 `structure-analyst` independent drafts, each carrying a FULL file partition; S3 round-robin convergence wrapped in `/ralph-loop … --completion-promise "STRUCTURE PROPOSAL CONVERGED"`, gated by the orchestrator-run **deterministic partition check** (`git ls-files` must equal movement-table ∪ stays-list exactly — zero orphans, zero duplicates; the detect-once polyglot snippet lives in the skill body); S4 reference closure — non-overlapping `reference-tracer` shards close `references_in` (import / require / include / config / build / ci / docs / string-path / test), `references_out_relative`, and forced `refactors` with `file:line` evidence + a mandatory per-shard `search_log`, assembled into **`movements.json` (schema v1.0)** with `batches` (ordered migration steps, per-batch verification commands) and `delete-dead` admissible only with confirmed zero inbound references; S5 adversarial verification wrapped in `/ralph-loop … --completion-promise "RESTRUCTURE PLAN VERIFIED"` — ×3 `structure-adversary` per round, exit ONLY after **two consecutive all-clean rounds** (a clean verdict with an empty `modalities_run` log is rejected); S6 `system-architect` Restructure Plan Audit + `RESTRUCTURE_PLAN.md` assembly per `superpowers:writing-plans`; S7 OpenSpec authoring via `openspec-propose` + strict validate wrapped in `/ralph-loop … --completion-promise "OPENSPEC AUTHORING COMPLETE"`; S8 verification-before-completion + MemPalace mining + best-effort notify + handoff.
- **NEW command `commands/optimize-structure.md`** (`/architect-team:optimize-structure [codebase-path | --all] [--objective "<prose>"] [--execute] [--no-commit] [--no-push] [--no-compact]`) — dispatch-mode banner first; plan-producer rule (the command never moves a single source file); default-branch guard (`architect-team/optimize-structure-<slug>`); explicit-artifact staging (never `git add -A`); `--execute` default `false`.
- **3 NEW agents** — `structure-analyst` (opus, blue, ×3; independence + partition self-check + ≥2 approaches considered per `superpowers:brainstorming`), `reference-tracer` (sonnet, orange, ×N; mechanical closure, never judges the structure), `structure-adversary` (opus, red, ×3; refutation via modalities the tracers did not log — basename grep, extensionless module-path grep, string-literal scan, config/glob expansion, `git log --follow` rename history; re-runs the partition check from scratch; attacks migration order, tooling, runtime-only references, and `delete-dead` claims). All three carry the canonical boilerplate blocks (synced via `scripts/setup/sync_agent_boilerplate.py`; baked lists extended).
- **1 EXTENDED agent** — `agents/system-architect.md` gains the ninth mode, **`## Restructure Plan Audit (structure-optimization Stage S6)`** (+ index row): objective fidelity per the scope discipline, partition check re-confirmed from scratch, reference-closure spot-check with fresh modalities (≥10 movements, weighted to high fan-in), migration-order soundness walk, reuse-first fit; verdict JSON at `.architect-team/structure-optimization/<slug>/architect-verdict.json`.
- **Producer/checker at pipeline scale (v0.9.13)** — analysts design, tracers close, adversaries refute, the architect audits; the orchestrator runs the deterministic checks itself and never trusts a recorded green block.
- **Registries + glue** — `EXPECTED_SKILLS` / `EXPECTED_AGENTS` / `EXPECTED_COMMANDS` extended; `hooks/skill_invocation_audit.py` frozen fallback now lists 20 commands (the derived form picks the new command up automatically); `scripts/setup/agent_boilerplate_blocks.py` baked standard-agent lists extended.
- **Deferred (named, not shipped)** — a deterministic `verify-restructure-partition` Layer 3 VAO tool; the skill-side orchestrator-run check + the adversarial re-runs carry the guarantee this release.

NEW tests: `tests/test_structure_optimization_skill.py` + `tests/test_structure_optimization_agents.py` + `tests/test_optimize_structure_command.py`; `tests/test_skill_invocation_audit_canonical.py` count assertion 19 → 20. Inventory **41 skills / 37 agents / 20 commands**. Suite 4268 → **4356 passing** + 5 skipped (across 175 test files; green under both Windows cp1252 AND `PYTHONUTF8=1`).

## [3.10.0] — 2026-06-10 — Second-tier review improvements (R1–R7)

**MINOR — new capabilities + behavior-preserving refactors + two bug fixes.** Implements the six design-level streams the v3.9.3 review-remediation run deferred, plus the discipline-detector-applicability fix (R7). R6 adds NEW user-visible capability (the `security-hunter` adversarial shape, the interaction-completeness accessibility axis, the `heartbeat` notify event), which forces a minor; R1/R2/R3/R4 are behavior-preserving refactors and R5/R7 are bug fixes. Every existing Layer 3 CLI subcommand, verdict shape, severity name, and fixture round-trip is unchanged; review-evidence schema v7 is unchanged; the 19 slash-command surfaces are unchanged.

- **R1 — Scope-discipline + marker/helper consolidation.** `_utc_now_iso` / `load_json(path, *, missing_ok)` / the JSONL reader unified into NEW `hooks/shared_util.py` (dual-form imports; each call-site's fail-open/fail-closed preserved); the two in-`vao_tools` localhost lists unified into one union re-exported as `_LOCAL_ENV_HOST_PATTERNS`; a canonical `## Scope-fidelity discipline family (v3.10.0)` CPC section names the five disciplines (v0.9.36 anti-deferral / v1.4.0 scope / v2.8.0 no-standing-red / v2.10.0 no-end-of-run-deferral / v2.14.0 no-implementation-scope-cut) as one family with the shared 3-disposition model + a firing-moment table, folding the five scattered neighbor-comparison tables; two named false positives fixed (`detect_virtue_framed_override` proximity requirement; standalone-admission-phrase pruning).
- **R2 — `hooks/vao_tools.py` split into the `hooks/vao/` package.** The 5,209-line / 20-tool monolith split into 11 per-discipline-family modules (each ≤900 lines); `vao_tools.py` remains a ≤400-line facade re-exporting the full 125-name surface (20 functions + 42 constants + 62 helpers + 1 alias, each `is` the package object) and keeping the CLI byte-identical. ZERO behavior change (every fixture + the real-subprocess glue-execution test stay green).
- **R3 — Narrative diet.** `CLAUDE.md` restructured operative-first (overview facts + `## Recent releases` + a CHANGELOG pointer); `common-pipeline-conventions` narrative compressed (verbatim transcripts → ≤2-line summaries + CHANGELOG pointers, the five neighbor tables folded into R1c's family table) with zero rule loss — every marker table / severity name / protocol / path / schema example kept; the four pipeline bodies collapse their duplicated `## Plugin prerequisites` + dispatch re-spelling + v2.18/v2.19/v2.20/v3.0 bash blocks to one-line CPC references + a minimal inline abort stub, with the invocations parameterized in a single CPC table.
- **R4 — Agent definition hygiene.** Retired `LS` (30 files) + `NotebookRead` (8 files) removed from every agent's frontmatter (NotebookEdit kept); `Task` retired from the tool vocabulary; bounded `Write` granted to `qa-replayer` + `test-completeness-verifier` (their bodies command writing verdict/SR JSON) with a task-reviewer-pattern scope note; the git-discipline + checkpoint boilerplate re-synced across all 34 agents via `scripts/setup/sync_agent_boilerplate.py` (the 3 VAO agents — adversarial-reviewer / interaction-observer / oracle-deriver — now carry the canonical git + checkpoint blocks, restoring oracle-deriver's dropped `$BASELINE_SHA`; the canonical checkpoint block gains the analysis-only exemption sentence); invalid colors mapped to the documented palette (domain-researcher amber→yellow, integration magenta→pink, test-run-watcher + monitor-synthesizer teal→cyan; a `VALID_COLORS` assertion added); `scaffold-agent` kept, its stale `LS`/`NotebookRead`/`Task` vocabulary fixed, and documented as an on-demand utility.
- **R5 — `hooks/locks.py` concurrency fixes.** `acquire_lock` creates the lock with `os.open(O_CREAT|O_EXCL|O_WRONLY)` (an existing file fails acquisition surfacing the holder; stale reclaim via atomic `os.replace`); an intersecting-scope re-scan releases own + names the winner; `globs_intersect` gains the prefix/suffix candidate class so `("src/**", "**/auth.py")` returns True both orders. `cdlg_overlap` unchanged.
- **R6 — New capabilities.** (a) `security-hunter` — the 6th `adversarial-reviewer` shape (missing/weakened authz, injection-prone construction, secrets in the diff, unsafe deserialization, unjustified dependency additions); spawn-brief trigger rules (backend-dep → both fake-data + security; auth/security-path or dependency-add → mandatory); NEW SR origin kind `security-finding`; the closed SR-origin enum reconciled to an OPEN canonical catalog (the `integration-failure`→`integration-test-failure` + `visual-fidelity-cascade`→`visual-fidelity-drift` spelling forks fixed across the routing lists). (b) `## Accessibility axis (v3.10.0)` for interaction-completeness + the matching `## Accessibility audit (v3.10.0)` for `interaction-reviewer` (keyboard reachability / accessible names / axe-core-via-Playwright; `a11y-gap` sub-kinds keyboard-unreachable / missing-accessible-name / axe-violation; NEW SR origin kind `a11y-gap`; a no-UI n/a rule). (c) Unbounded-run `heartbeat` — `scripts/notify/notify.py` gains a 6th event type `heartbeat`; `hooks/run_metrics.py` gains `heartbeat_snapshot(workspace, run_id)`; a `### Heartbeat discipline (v3.10.0)` CPC subsection (refresh `.architect-team/in-progress.md` + emit during any >30-min phase and at every post-first-hour phase boundary; never gates, never caps).
- **R7 — `discipline_registry.py` applicability guards.** The prod-safe-test-classification detector counts only Playwright/QA-shaped files; the multi-persona detector records `applicable=false` with no frontend markers; the registry schema gains `{applied, not_applicable, reason}`; this no-UI repo now records both disciplines `not_applicable` instead of false-flagging its pytest suite.

NEW canonical fixture `tests/fixtures/vao/security-finding-routed.json`. NEW skills package layout `hooks/vao/` behind the `hooks/vao_tools.py` facade. Suite 4097 → **4268 passing** + 5 skipped (across 172 test files; green under both Windows cp1252 AND PYTHONUTF8=1); all new tests additive.
