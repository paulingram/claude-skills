...
Tests: **1 new test file** (`test_usage_review.py`, 26 tests) covering the R4 measured/omitted split, the R5 pinned round-trip + corroborate-on-apply + default-summary redaction, and both hardening reds. Suite **6926 → 6952 passing + 6 skipped, 0 failed** (235 test files, Windows-measured under BOTH the default cp1252 environment and `PYTHONUTF8=1`; +26 tests, zero new failures); `check_separation` clean (checked 26, unaffected). Skills / agents / commands / hooks / Layer-3 tools UNCHANGED (53 / 39 / 25 / 7 / 21); `tests/test_dispatch_banner.py`'s version pin moved in lockstep with the bump.
