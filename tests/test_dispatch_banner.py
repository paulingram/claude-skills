...
def test_plugin_metadata_at_1_5_0(plugin_root: Path) -> None:
    ...
    version', which the current release (v3.55.2 — data-eng-realtime-gate-fix)
    makes 3.55.2. NOTE: the v3.40.1 release bumped the manifests without
    ...
    assert plugin_json["version"] == "3.55.2"
    # marketplace.json has plugins[0].version
    assert marketplace_json["plugins"][0]["version"] == "3.55.2"
