...
def test_plugin_metadata_at_1_5_0(plugin_root: Path) -> None:
    ...
    version', which the current release (v3.25.0 — the Triage / Evaluator service)
    makes 3.25.0."""
    ...
    assert plugin_json["version"] == "3.25.0"
    # marketplace.json has plugins[0].version
    assert marketplace_json["plugins"][0]["version"] == "3.25.0"
