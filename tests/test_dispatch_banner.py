...
def test_plugin_metadata_at_1_5_0(plugin_root: Path) -> None:
    ...
    version', which the current release (v3.16.0 — responsive parallel-lane inject)
    makes 3.16.0."""
    ...
    assert plugin_json["version"] == "3.16.0"
    # marketplace.json has plugins[0].version
    assert marketplace_json["plugins"][0]["version"] == "3.16.0"
