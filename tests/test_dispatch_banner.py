...
def test_plugin_metadata_at_1_5_0(plugin_root: Path) -> None:
    ...
    version', which the current release (v3.42.0 — credit-failover-to-login)
    makes 3.42.0. NOTE: the v3.40.1 release bumped the manifests without
    ...
    assert plugin_json["version"] == "3.42.0"
    # marketplace.json has plugins[0].version
    assert marketplace_json["plugins"][0]["version"] == "3.42.0"
