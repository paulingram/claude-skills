...
- [ ] 1.3 Red-first + implement `classify_turn_output(text)` — pure function, no I/O. SHIPPED (after six revisions): trips on a structural marker at `>= 2` lines, on the absolute line ceiling, or on enough unbroken prose. Never trips on a ONE-LINE turn whatever markers it carries, and the `>= 3 report-length lines` arm was RETIRED because it fired on three ordinary narration sentences. Pin BOTH directions; a rule that never fires is decoration and a rule that fires on a terse status update gets disabled.
