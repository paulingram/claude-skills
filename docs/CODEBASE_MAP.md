---
last_mapped: 2026-05-22T22:30:00Z
codebase: architect-team-plugin
note: Doc refresh 2026-05-22 for v0.9.21 (interaction-intuition-discovery) — the new skills/interaction-intuition/ skill and agents/interaction-intuiter.md agent catalogued in §3/§4; new Phase −1D sub-section in skills/architect-team-pipeline/SKILL.md and skills/intake-and-mapping/SKILL.md; new per-codebase artifact INTERACTION_INTUITION_MAP.md produced at Phase −1D alongside ROUTE_MAP.md / DESIGN_MAP.md; v0.9.20 (gates opt-in / domain-gate carve-out) and v0.9.21 (bulk-verify gate as a domain gate) integrated into the pipeline skill's `## Default mode of operation` section; test-file count now 33 (test_interaction_intuition_skill.py + test_interaction_intuiter_agent.py + test_phase_minus_1d_bulk_verify_wiring.py + test_interaction_intuition_map_schema.py added). Current reality - 21 skills, 18 agents, 6 commands, 3 enforcement hooks + 1 shared schema module (review-gate evidence schema v6), 3 setup/support scripts (setup.py + install_mempalace.py + notify/notify.py), 730 pytest self-tests across 33 test files. Covers v0.9.1 (auto-compact) through v0.9.21 (interaction-intuition discovery + Phase −1D bulk-verify gate).
---

# Codebase Map

> The `architect-team` Claude Code plugin. Last refreshed 2026-05-22 for v0.9.21.

## 1. System Overview

The `architect-team` Claude Code plugin (v0.9.21) is a spec-to-production multi-agent coding pipeline. It accepts EITHER a requirements folder (OpenSpec, Superpowers, or plain markdown) OR a plain-language requirement typed directly as prose (v0.9.17), and drives it end-to-end through eight-and-a-bit phases: intake & mapping (Phase −1, with a new sub-section D in v0.9.21 producing per-frontend `INTERACTION_INTUITION_MAP.md` artifacts and firing a bulk-verify gate before Phase 0), detection & normalization (Phase 0, now consuming the confirmed intuition map as a binding input), a 100%-coverage planning-validation gate (Phase 1, now requiring confirmed element-action-endpoint triples as acceptance criteria for in-scope frontend / both-layer requirements), parallel team decomposition & spawn (Phase 2), hook-enforced per-team review gates (Phase 3), continuous solution-requirement intake (Phase 3b), reconciliation (Phase 4), cross-layer integration (Phase 5) with its visual-fidelity and editability and interaction-completeness and live-app-verification sub-gates, the outer task-group loop (Phase 6), master review (Phase 7), and the final report + auto-commit (Phase 8). The v0.9.20 `## Default mode of operation` rule (gates are opt-in for *process* gates) and the v0.9.21 *domain-gate* carve-out (the Phase −1D bulk-verify, the `editability-completeness` and `interaction-completeness` `ambiguous` escalations) coexist in the same section.

The plugin ships **21 skills, 18 named agent definitions, 6 slash commands, 3 enforcement hooks** (plus a shared schema module), **3 setup/support scripts** (the `setup.py` + `install_mempalace.py` installers and the `scripts/notify/notify.py` email notifier, v0.9.18), and **730 pytest self-tests** that validate every structural artifact and guard against cross-component drift. Its enforcement is layered: Python hooks gate teammate task-completion, teammate idle, and the orchestrator's terminal state; independent verifier agents and teams re-check test-completeness, editability, interactive-element-and-page genuineness, and visual fidelity against reality; and the discipline skills are pressure-written to resist rationalization. A run optionally emits per-project email notifications (v0.9.18) when the target project supplies a `.architect-team-notify.json`.
