...
note: "Doc refresh 2026-05-28 for v1.0.0 — Agent Teams as the default dispatch mode. Adds the new `scripts/setup/teams_mode.py` mode-detection helper, the new `hooks/locks.py` cross-session lock layer (4 functions: acquire_lock / release_lock / detect_stale / globs_intersect), the `TaskCompleted` + `TeammateIdle` trigger registrations in `hooks.json` (alongside the existing PostToolUse(TaskUpdate) / SubagentStop), and the small uniform long-lived-teammate framing rewrite across all 27 agent bodies. Every previously-nested team pattern flattens to Lead-owned dispatch per the Agent Teams docs' no-nested-teams constraint. Current reality — 26 skills, 27 agents, 11 commands, 3 enforcement hooks + 1 shared schema module (review-gate evidence schema v6) + 1 new lock-layer module (`hooks/locks.py`), 4 setup/support scripts (the new `scripts/setup/teams_mode.py` detector + the existing `setup.py` / `install_mempalace.py` / `scripts/notify/notify.py`), ~1629 pytest self-tests across 72 test files. Carries forward the v0.10.0 mini-architect-team-pipeline (single architect, parallel devs cross-reviewing, single mini-qa, auto-merge to main on green) and the v0.9.36 bug-fix testing-enforcement / anti-deferral discipline."
---
...
├── tests/                   # ~1629 pytest self-tests (72 test files + conftest + helpers/)
