---
last_mapped: 2026-05-18T00:00:00Z
total_files: 50
total_tokens: 94540
note: total_tokens reflects the original cartographer scan; actual repo content has grown since. Targeted refresh on 2026-05-18 bumped version (0.4.0), added the new design-fidelity-mapping skill, extended route-mapper to conditionally produce DESIGN_MAP.md, extended codebase-map-reviewer to review it, added visual-fidelity test layer to playwright-user-flows, and bumped test counts (77).
---

# Codebase Map

> Auto-generated by Cartographer. Last mapped: 2026-05-16T09:32:23Z (targeted refresh 2026-05-16T17:55Z; refresh 2026-05-17T00:00:00Z for v0.3.0 root-cause-test-failures; refresh 2026-05-18T00:00:00Z for v0.4.0 design-fidelity-mapping).

## 1. System Overview

The `architect-team` Claude Code plugin (v0.4.0) is a spec-to-production multi-agent coding pipeline. It accepts a requirements folder (in OpenSpec, Superpowers, or plain-markdown format) and drives it end-to-end through intake mapping (now including conditional design-fidelity mapping when screenshots / Figma / tokens / Storybook / assets are present), 100%-coverage planning validation, parallel team implementation with mandatory review gates, reconciliation, cross-layer integration testing with mandatory per-test expectations and a 3-pass root-cause loop on every failure, and a final coverage-verified report. The plugin ships 10 skills, 10 named agent definitions, 2 slash commands, 2 enforcement hooks, a cross-platform setup script, and 77 pytest self-tests that validate every structural artifact.
...
│   ├── design-fidelity-mapping/         # NEW v0.4.0 — conditional DESIGN_MAP.md (design tokens + asset registry + per-screen visual specs)
...
│   ├── playwright-user-flows/           # White-box Playwright methodology (Step 0 user-intent escalation + PROCEED test + state-guard naming + visual-fidelity tests when DESIGN_MAP exists)
...
├── tests/                   # 77 pytest self-tests (was 76 pre-v0.4.0, 75 pre-v0.3.0, 69 pre-v0.2.4, 54 pre-v0.2.3, 52 pre-v0.2.2)
...
| `tests/test_skills.py` | 11 | 1 presence + 10 parametrized frontmatter checks (v0.3.0 added root-cause-test-failures; v0.4.0 added design-fidelity-mapping) |
