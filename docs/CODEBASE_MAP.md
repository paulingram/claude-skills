---
last_mapped: 2026-05-28T03:00:00Z
codebase: architect-team-plugin
note: "Doc refresh 2026-05-28 for v1.2.0 — Auto-Worktree Lifecycle. The natural follow-up to v1.1.0. v1.1.0 made cross-session state coordination worktree-aware (the lock dir + MemPalace now route through `shared_state_dir()` so two worktree-based sessions properly share locks + memory), but left worktree CREATION as a manual user action. The user's follow-up ask was direct: *always on when using architect team.* v1.2.0 ships exactly that — `/architect-team`, `/architect-team:bug-fix`, and `/architect-team:mini` auto-create a fresh worktree at `<parent-of-repo>/<repo-name>-<slug>/` on branch `architect-team/<slug>` before invoking the pipeline skill (default; opt out with `--no-worktree`). The new helper is **`scripts/setup/worktree_lifecycle.py`** — sibling to v1.1.0's `worktree_paths.py`, with 4 stdlib-only functions: `create_run_worktree(slug, base_branch=\"main\", parent_dir=None) -> Path` (creates `<parent>/<repo-name>-<slug>/` on branch `architect-team/<slug>`; collision handling appends `-2`, `-3`, ...; raises `RuntimeError` with an actionable message on parent-dir-not-writable / base-branch-missing / `git worktree add` failure); `cleanup_run_worktree(worktree_path, remove_branch=False) -> None` (`git worktree remove` + optional `git branch -d`; idempotent on already-gone worktree; falls back to `--force` once before raising); `current_worktree_is_run() -> bool` (re-entry detection — `git rev-parse --abbrev-ref HEAD` starts with `architect-team/`); `current_run_slug() -> str | None` (slug extraction from the branch name). The 3 pipeline-driving slash commands each gain a new `## Auto-worktree creation (v1.2.0)` section that fires AFTER argument parsing + refinement and BEFORE skill invocation; skipped on `--no-worktree` (natural-language opt-outs: *no worktree* / *don't create a worktree* / *single tree* / *in place* / *in current tree*) OR re-entry from inside an existing `architect-team/*` worktree. The 7 utility commands (visual-qa / editability-audit / refine-prompt / memory / mempalace-install / setup / mini-review-sweep) do NOT get the step. **Cleanup is NOT automatic** — pipeline emits a recommendation at Phase 8 / B8 / M7 success. `common-pipeline-conventions/SKILL.md` gains a `## Auto-worktree lifecycle` section AFTER `## Running in parallel sessions` documenting the trigger / detection / opt-out / path + branch convention / collision handling / cleanup semantics with a default-run + re-entry + opt-out shell-example trio. Backwards-compatible — v1.1.0 single-tree users pass `--no-worktree` and get verbatim v1.1.0 behavior. Carries forward all v1.1.0 reality (26 skills, 27 agents, 11 commands, 3 enforcement hooks + 1 shared schema module + the `hooks/locks.py` lock layer), NOW **6 setup/support scripts** (the new `scripts/setup/worktree_lifecycle.py` joins the existing `setup.py` / `install_mempalace.py` / `teams_mode.py` / `worktree_paths.py` / `notify.py`), **1702 pytest self-tests + 1 skipped across 74 test files** (was 1694 + 1 / 73 in v1.1.0; +8 in the new `test_worktree_lifecycle.py`)."
---

# Codebase Map

> The `architect-team` Claude Code plugin. Last refreshed 2026-05-28 for v1.2.0.
...
├── tests/                   # 1702 pytest self-tests + 1 skipped (74 test files + conftest + helpers/)
...
- **`scripts/setup/worktree_lifecycle.py`** — **v1.2.0 worktree-lifecycle helper.** Four stdlib-only functions: `create_run_worktree(slug, base_branch="main", parent_dir=None) -> Path` creates `<parent>/<repo-name>-<slug>/` on a fresh branch `architect-team/<slug>` via `git worktree add -b`, with collision handling that appends `-2`, `-3`, ... when either path or branch is taken (bounded at 999 attempts), and raises `RuntimeError` with an actionable message on parent-dir-not-writable / base-branch-missing / `git worktree add` failure; `cleanup_run_worktree(worktree_path, remove_branch=False) -> None` runs `git worktree remove <path>` (idempotent on already-gone worktrees — catches the `is not a working tree` / `is not a registered` markers) with optional `git branch -d architect-team/<slug>`, falling back to `--force` removal once before raising on unexpected failures; `current_worktree_is_run() -> bool` is True iff `git rev-parse --abbrev-ref HEAD` starts with `architect-team/` — used by the slash commands' re-entry detection to skip nested worktree creation; `current_run_slug() -> str | None` extracts the slug from `architect-team/<slug>` or returns None on non-run branches. Consumed by the 3 pipeline-driving slash commands (`commands/architect-team.md`, `commands/bug-fix.md`, `commands/mini.md`) via the polyglot `python3 -c '...' || python -c '...'` invocation pattern in each command's `## Auto-worktree creation (v1.2.0)` section. The split from v1.1.0's `worktree_paths.py` is intentional — paths is read-only resolution; lifecycle is side-effecting subprocess work. The 3 slash commands fire the auto-worktree step by default; `--no-worktree` (or natural-language equivalents) opts out. Cleanup is NOT automatic — pipeline recommends `git worktree remove <path> && git branch -d architect-team/<slug>` at Phase 8 / B8 / M7 success and the user decides when to clean up. Full rules in `skills/common-pipeline-conventions/SKILL.md` `## Auto-worktree lifecycle`.
