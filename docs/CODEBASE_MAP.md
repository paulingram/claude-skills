---
last_mapped: 2026-05-17T00:00:00Z
total_files: 49
total_tokens: 94540
note: total_tokens reflects the original cartographer scan; actual repo content has grown since. Targeted refresh on 2026-05-17 bumped version (0.3.0), added the new root-cause-test-failures skill, expanded playwright-user-flows (Step 0 escalation + PROCEED test + state-guard naming), wired RCA across architect-team-pipeline / dev-api-integration-testing / integration / backend / frontend agents, and bumped test counts (76).
---

# Codebase Map

> Auto-generated by Cartographer. Last mapped: 2026-05-16T09:32:23Z (targeted refresh 2026-05-16T17:55Z; second refresh 2026-05-17T00:00:00Z for v0.3.0 root-cause-test-failures skill).

## 1. System Overview

The `architect-team` Claude Code plugin (v0.3.0) is a spec-to-production multi-agent coding pipeline. It accepts a requirements folder (in OpenSpec, Superpowers, or plain-markdown format) and drives it end-to-end through intake mapping, 100%-coverage planning validation, parallel team implementation with mandatory review gates, reconciliation, cross-layer integration testing with mandatory per-test expectations and a 3-pass root-cause loop on every failure, and a final coverage-verified report. The plugin ships 9 skills, 10 named agent definitions, 2 slash commands, 2 enforcement hooks, a cross-platform setup script, and 76 pytest self-tests that validate every structural artifact.
...
├── skills/                  # 9 skill directories, each containing SKILL.md
...
│   ├── playwright-user-flows/           # White-box Playwright methodology (Step 0 user-intent escalation + PROCEED test + state-guard naming)
│   ├── reuse-first-design/              # Extend > compose > reuse > build-new ladder
│   ├── root-cause-test-failures/        # NEW v0.3.0 — per-test expectations + 3-pass RCA loop on every failure
...
├── tests/                   # 76 pytest self-tests (was 75 pre-v0.3.0, 69 pre-v0.2.4, 54 pre-v0.2.3, 52 pre-v0.2.2)
...
| `tests/test_skills.py` | 10 | 1 presence + 9 parametrized frontmatter checks (v0.3.0 added root-cause-test-failures) |
