---
last_mapped: 2026-05-28T02:00:00Z
codebase: architect-team-plugin
note: "Doc refresh 2026-05-28 for v1.1.0 — Worktree-Aware State Resolution. Surgical follow-up to v1.0.0 that closes the cross-session coordination gap for git-worktree-based parallel runs. v1.0.0's `hooks/locks.py` and the MemPalace integration both resolved via `git rev-parse --show-toplevel`, which in a worktree returns the worktree's own path — so each worktree got its own locks dir + its own MemPalace, defeating the v1.0.0 cross-session coordination intent. v1.1.0 adds `scripts/setup/worktree_paths.py` (3 stdlib-only functions: `shared_state_dir()` → main worktree's `.architect-team/`; `run_state_dir()` → current worktree's `.architect-team/`; `is_worktree()` → True iff cwd is inside a `git worktree add`-created worktree); routes `hooks/locks.py`'s default `locks_dir` through `shared_state_dir() / 'locks'`; updates the MemPalace integration skill to note the palace resolves through `shared_state_dir() / '.mempalace' / 'palace'`; documents the new 3-layer model (filesystem isolation = worktrees / architectural coordination = locks / context sharing = MemPalace) + the shared-vs-run state split in `common-pipeline-conventions/SKILL.md` `## Running in parallel sessions`. Backwards-compatible — single-session users (no worktrees) see ZERO behavior change. Carries forward all v1.0.0 reality: 26 skills, 27 agents, 11 commands, 3 enforcement hooks + 1 shared schema module + the `hooks/locks.py` lock layer, NOW 5 setup/support scripts (the new `scripts/setup/worktree_paths.py` helper joins the existing `setup.py` / `install_mempalace.py` / `teams_mode.py` / `notify.py`), **1694 pytest self-tests + 1 skipped across 73 test files** (was 1688 + 1 / 72 in v1.0.0; +6 in the new `test_worktree_state_resolution.py`)."
---

# Codebase Map

> The `architect-team` Claude Code plugin. Last refreshed 2026-05-28 for v1.1.0.
...
├── scripts/
│   ├── setup/               # setup.py + install_mempalace.py + teams_mode.py (v1.0.0) + worktree_paths.py (v1.1.0 worktree-aware state resolver)
...
├── tests/                   # 1694 pytest self-tests + 1 skipped (73 test files + conftest + helpers/)
...
### Tests (1694 PASS + 1 SKIPPED)

73 test files under `tests/` (discovered via `test_*.py`), plus `conftest.py` (session fixtures) and helper modules under `tests/helpers/` (`frontmatter.py`, v0.10.0's `qa_guidance.py` and `mini_run_trailer.py`, plus **v1.0.0's `teams_mode.py`** — the optional monkeypatching helper for env / settings.json / `claude --version` fixtures used by `test_teams_mode.py` and `test_setup_teams_checks.py`). Coverage spans v0.9.0 → v1.0.0: plugin/marketplace JSON; all 26 skill + 27 agent + 11 command frontmatters; hooks.json wiring for all 5 trigger events (PostToolUse + SubagentStop + Stop + the v1.0.0 TaskCompleted + TeammateIdle); the three hooks' script logic; setup + MemPalace install scripts; the notifier module (`test_notify.py` + `test_notify_wiring.py`); the ui-interaction-fidelity discipline; **cross-component consistency** (`test_cross_consistency.py`); the v0.9.22 bug-fix pipeline; the v0.9.29 ux-test-builder; the v0.9.31 code-path witness; the v0.9.33 proposal-refiner; the v0.9.34 email-testing skill; the v0.10.0 mini-architect-team-pipeline; and **v1.0.0's seven new test files** — `test_teams_mode.py` (19 cases: mode-detection scenarios from REQ-1 — env-var truthy/falsy, `~/.claude/settings.json` fallback, `claude --version` parsing, `--no-teams` override, malformed settings.json graceful degradation), `test_locks.py` (17 cases: lock-layer scenarios from REQ-3 — acquire writes a lock file, overlapping scope is blocked, disjoint acquires cleanly, stale lock is detected and auto-released, release frees the scope, malformed lock is treated as stale, `globs_intersect` semantics), `test_setup_teams_checks.py` (REQ-7 setup ergonomics — `--check-only` reports status without modifying files, interactive consent writes settings.json, `--no-prompt` skips the write, idempotent on re-run), `test_hooks_trigger_split.py` (REQ-4 — exercises both `PostToolUse(TaskUpdate)` and `TaskCompleted` payload shapes against `review-gate-task.py`; both `SubagentStop` and `TeammateIdle` against `teammate-idle-check.py`; same `.architect-team/reviews/<task-id>.json` path + same v6 validation in both modes), `test_dispatch_mode_section.py` (REQ-6 — every pipeline SKILL.md has exactly one `## Dispatch mode` section naming the env var, the `2.1.32` requirement, the `--no-teams` flag, and the teams-mode primitives `Spawn teammate using <role>` / `SendMessage` / `~/.claude/tasks/<slug>/`), `test_no_nested_teams_in_skills.py` (REQ-2 at the skill level — audit-greps the 3 pipeline skill bodies for the now-forbidden "spawn N agents in parallel" / "dispatch X team" / "Spawn 3 reviewers" patterns; flattened patterns must read as Lead-owned task creations), `test_agent_teammate_framing.py` (REQ-5 + REQ-2 Scenario 2.8 — every `agents/*.md` body names "teammate" or "long-lived" or "across multiple tasks within this run"; no body claims to spawn a team; frontmatter still validates against the existing 5-key schema). Net new tests: ~210; total 1417 → **1631 passing + 1 skipped** at v1.0.0. **v1.1.0 adds `tests/test_worktree_state_resolution.py`** (6 cases — `is_worktree` True/False against a real `git worktree add`-created worktree, `shared_state_dir` resolves to main from both main and worktree, `run_state_dir` per-worktree differentiation, cross-worktree lock integration check that acquire-from-worktree with default `locks_dir` blocks an intersecting acquire-from-main); total now **1694 passing + 1 skipped** across **73 test files**.
