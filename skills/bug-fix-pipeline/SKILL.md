...
Same flow as `architect-team-pipeline` Phase 8 — **documentation-currency gate first** (the `doc-updater` agent dispatch + the `system-architect` Documentation Currency Audit + the commit-blocking gate, per v0.9.23 — same dispatch, same audit, same enforcement as the main pipeline), then completion audit gate, default-branch guard (the work lands on `architect-team/<bug-slug>` feature branch unless `--allow-push-to-default`), commit with the standard message template, push to the feature branch, recommend a PR. **End-of-run worktree finalize (v3.6.0):** after the push and before the auto-compact prompt — when this run created a worktree — call `finalize_run_worktree(Path('${WORKTREE_PATH}'))` via the polyglot Python pattern (same invocation as `architect-team-pipeline` Phase 8's `### End-of-run worktree finalize` section): it removes the worktree + branch if the branch is merged into `origin/main`, otherwise leaves the folder and returns a `warning` the orchestrator prints verbatim. Unmerged work is never auto-deleted. Auto-compact prompt at the very end (unless `--no-compact`).
