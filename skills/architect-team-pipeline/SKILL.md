...
Teammate writes its **self-review** — the 11 top-level fields of `<cwd>/.architect-team/reviews/<task-id>.json` per the schema (v5) in `team-spawning-and-review-gates` — BEFORE any `TaskUpdate` flips its task to `completed`. If any check fails, the teammate re-engages on implementation.
