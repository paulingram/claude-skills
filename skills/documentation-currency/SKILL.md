...
| `<codebase>/docs/INTERACTION_INTUITION_MAP.md` (per-frontend-codebase, when present) | added / changed user verdicts during a Phase −1D bulk-verify, or shipped a frontend change that altered interactive elements, their intuited actions, or their candidate endpoints | every entry reflects the current user_verdicts; the frontmatter freshness field (`last_intuited`) is current; no element row references an interactive element the code no longer contains |
...
| `phenotypes/README.md` (if a phenotype store exists) | added / removed / renamed a phenotype, or changed a seed's variation points, scaffold parameters, or quick-start workflow | the seed table lists exactly the `phenotypes/<label>/` dirs on disk (count + rows); the quick-starts match the scaffolds' `post_emit_notes`; the authoring-paths section reflects the shipped absorb capability *(added v3.13.2 — the v3.13.0 release shipped a fourth phenotype while this doc still said "All three production seeds"; it ships inside the plugin but was invisible to this gate)* |
| `phenotypes/SCHEMA.md` (if a phenotype store exists) | changed the `phenotype.json` / `scaffold.manifest.json` contract or the engine validator (`scripts/phenotypes/phenotypes.py`) | the documented schema matches what `validate_phenotype` actually enforces (required keys, value sets, manifest semantics) |
...
- **Bounded Write scope.** The `doc-updater`'s Write allowlist is the inventory paths ONLY (README, CHANGELOG, CLAUDE.md, AGENTS.md, the maps, the per-codebase map variants, and — when a phenotype store exists — `phenotypes/README.md` + `phenotypes/SCHEMA.md`). It NEVER writes source code, tests, openspec/* artifacts, the per-phenotype records (`phenotypes/<label>/**` — blueprints/scaffolds/manifests are feature artifacts, not currency docs), or the version-source-of-truth JSON files. The Phase 8 commit-audit cross-checks the agent's diff against this scope.
