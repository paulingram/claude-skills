...
- `visual-fidelity-verifier` agent — spawned in Phase 5 (and by `/architect-team:visual-qa`) to independently render the LIVE running app and confirm the visual-fidelity reconciliation actually happened. Every `overall: fail` writes an SR with `origin.kind: "visual-fidelity-drift"` — including the `report-fabricated` case (the reconciliation claimed `perfect` for a screen the live app shows drifted) and `report-incomplete` (a screen the reconciliation skipped). The `acceptance_criteria` mandate that the live app match the DESIGN_MAP Oracle for every drifted screen, re-verified by the verifier against the running app. A verifier `blocked` verdict (the live app would not run) is escalated to the human, not converted to an SR — an unrunnable app is a prerequisite defect, not a fix-team task.
