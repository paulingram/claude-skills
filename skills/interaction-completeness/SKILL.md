...
- **`pending-backend`** (v1.7.0) — the UI element exists in the component code but it is intentionally NOT wired yet, because the endpoint it needs does not yet exist AND the frontend has surfaced a `missing-api-for-frontend-element` SR documenting the contract per `common-pipeline-conventions` `## Frontend missing-API discipline`. **Tell:** the element is rendered (so it's not a `placeholder-page` and not an `unwired-control` gap), it does nothing yet (so it's not `endpoint-backed` or `client-only`), and there is an OPEN solution requirement at `.architect-team/solution-requirements/SR-missing-api-*.json` with `origin.kind: "missing-api-for-frontend-element"` whose `acceptance_criteria` enumerate the endpoint shape. The SR is the wire-up contract; the element flips to `endpoint-backed` once the backend ships the endpoint and the frontend wires it. **SR-linkage rule:** the `interaction-reviewer` accepts `pending-backend` ONLY when the matching open SR exists; without the SR, the element is an `unwired-control` gap (the existing rule — an inert control with no user confirmation and no SR is never a silent pass). **Distinct from `confirmed-stub`:** `confirmed-stub` is intentional + user-authorized + NOT planned for wire-up; `pending-backend` is temporary + SR-authorized + WILL be wired once the backend ships the endpoint. The two never overlap.
...
### Verifying a `pending-backend` element

When the reviewer encounters an inert UI element, the decision tree is: is there a matching open SR with `origin.kind: "missing-api-for-frontend-element"`? If YES, the element is `pending-backend` (record it as such in the converged map; track the SR ID alongside; the element is NOT a gap). If NO, the inert element is an `unwired-control` gap UNLESS the user has explicitly confirmed it as a `confirmed-stub`. The reviewer walks `<cwd>/.architect-team/solution-requirements/` and matches SRs to elements by the element name / file referenced in the SR's `problem_summary` or `affected_screens`. A `pending-backend` element does not require a user-flow test for THIS pass (the element is intentionally inert until the backend ships); when the backend ships and the frontend wires, the next pass re-classifies the element as `endpoint-backed` and the genuine-test requirement applies normally.
