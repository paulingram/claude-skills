...
**If `mempalace` is NOT on PATH** (the CLI was never installed, or installed somewhere off PATH), the wake-up command fails to start. This is **NOT a hard failure** — the orchestrator surfaces ONE user-facing line — *"MemPalace is not on PATH; prior-run recall and auto-mine are disabled for this run. Run `/architect-team:mempalace-install` to enable cross-run memory."* — and then **continues the run with every MemPalace step treated as a no-op** (wake-up returns clean-room state; each `mine` is skipped). The pipeline NEVER blocks, fails, or aborts because MemPalace is absent; memory is an enhancement, not a precondition. The same one-line surface + install suggestion + continue-as-no-ops behavior is the canonical rule the four deferring pipeline bodies (`architect-team-pipeline` `## MemPalace wake-up`, `bug-fix-pipeline`, `mini-architect-team-pipeline`, `common-pipeline-conventions` `## MemPalace wake-up precondition`) point at — they do not re-explain it inline.
