...
2. **Provision immediately at the REAL path (CFP-4).** The endpoint lives exactly where the contract says. Return a payload with every declared field, at the declared type, carrying plausible representative values; make every declared error state reachable through a documented trigger so the frontend can build its error paths in the same pass. Mark the payload provisional (`"_mock": true` or this project's documented equivalent) and record the surface in `<workspace>/.architect-team/contracts/ledger.json` with state `mock-serving`, its owner (you) and its consumer. Then TELL the frontend it is live.
...
4. **Retire the mock before the run can close (CFP-6).** Flip the surface to real data, verify end-to-end through the SAME path the frontend already calls (the frontend needing no change at retirement is the proof the abstraction held), record `verified_by`, and transition the ledger entry to `live`. A run cannot close while your surface is still `mock-serving` — `scripts/contract/interface_contract.py gate` blocks it, and "retire it next run" is the end-of-run deferral your `## No end-of-run deferral discipline` forbids.
