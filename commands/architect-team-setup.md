...
- **The secondary backend is not available (or you say nothing)** → the committed model policy stays exactly as it ships: the v3.43.0 delivery-adversarial split — the **12 delivery + adversarial agents on `model: opus`** (Opus 5) and the **27 planning / validation / review agents on `model: fable`** — with the uniform Opus fallback lever (`python3 scripts/setup/set_default_model.py --model opus`) for a harness that predates the fable alias. With NO signal setup rewrites nothing (your manual lever state is never clobbered); with an explicit `--no-codex` (or the env var set to a falsy value) it applies the uniform fable policy — this moves OFF the shipped delivery split rather than restoring it.
...
python3 scripts/setup/install_gateway.py status --live                  # + probe the RUNNING gateway and confirm it serves the split (models listing + a live spawn-alias /v1/messages completion)
...
### One-call confirmation — CT6 runs the split (v3.39.0, augmented v3.41.0)

A registered `--external-llm` install no longer ends at "steps reported ok" — it ends at **proof**. The installer polls the LIVE gateway's `/v1/models` and asserts the ids the mode needs are actually served (`ct6-secondary` always; `claude-fable-5` additionally in api-key mode — the v3.38.1 field bug, a generated config that passed every install step while rejecting fable, is exactly what this catches). `codex-5.6-sol` is accepted only as a legacy migration alias; new installs and confirmations use `ct6-secondary`. **On top of that listing (v3.41.0), the confirm proves the hop callers actually use** — a model listing proves listing, not serving, which is how a broken completion hop once shipped as "CONFIRMED live". So a passing listing must ALSO produce a real bounded `/v1/messages` completion (a non-empty content block): the spawn alias `claude-haiku-4-5` is tried first and, when the state records one, its completion is MANDATORY — the neutral-alias (`ct6-secondary`) fallback confirms only for legacy states that recorded no spawn alias. The serving deployment is identity-verified against `/model/info` (the response's `x-litellm-model-id` must map to the registry-derived `<route_dialect>/<model>` route, so a request silently answered by Anthropic's real Haiku FAILS with the observed upstream named, not confirmed), and a final auth-enforcement rung requires a deliberately-invalid key be DENIED service (a 4xx is clean; a DB-less gateway's crash-5xx is enforcement with a named warning; only a 200 fails). A stale gateway process serving a pre-regeneration config gets one automatic restart + re-probe. The setup row then states the outcome plainly: **"CONFIRMED live — CT6 runs the split"**, or a fail row with the remediation. Report that confirmation sentence to the user verbatim — it is the answer to "is my team actually running the mixed models?".
