---
description: Spec-to-production multi-agent coding pipeline. Takes a requirements folder (OpenSpec / Superpowers / plain markdown) and drives it end-to-end to tested, integrated production code. Auto-commits and pushes on a clean Phase 8 pass unless invoked with --no-commit / --no-push.
argument-hint: "<path-to-requirements-folder> [--no-commit] [--no-push]"
---
...
- Neither flag → set `AUTO_COMMIT = true`, `AUTO_PUSH = true` (the default).

Also accept natural-language opt-outs in the user's message: phrases like "don't commit", "skip the commit", "no push", "leave the changes uncommitted" — treat these as equivalent to the corresponding flag. When in doubt about a natural-language opt-out, default to opting out (safer) and tell the user which interpretation you took.
