...
![version](https://img.shields.io/badge/version-3.55.2-2563EB?style=flat-square)
![license](https://img.shields.io/badge/license-MIT-3FB950?style=flat-square)
![tests](https://img.shields.io/badge/tests-7020%20passing-3FB950?style=flat-square)
...
The v3.50.0 data-eng lane wired `/architect-team:data-eng` into the Layer-6 Stop-time skill-invocation auditor but omitted `data-eng-pipeline` from the real-time PreToolUse hard-gate — a `/architect-team:data-eng` request could reach build/dispatch tools without the `data-eng-pipeline` Skill ever being engaged, with only the after-the-fact audit to notice. This PATCH adds `data-eng-pipeline` to `hooks/pretool_skill_gate.py`'s `_PIPELINE_SKILLS`, so the data-eng lane is now real-time gated (build/dispatch tools blocked until the Skill is engaged) exactly like the other five pipeline commands — prevention, not just detection. Shipped with a red-first regression test.

| What changed | Detail |
|---|---|
| **The fix** | `data-eng-pipeline` joins `_PIPELINE_SKILLS` (**5 → 6 members**: the 4 run-driving playbooks + `data-eng-pipeline` + `proposal-refiner`) in `hooks/pretool_skill_gate.py` — an unsatisfied `/architect-team:data-eng` request now BLOCKS Edit / Write / Agent / Task* (exit 2) until the `data-eng-pipeline` Skill is engaged, closing the v3.50.0 gap where the lane was covered only by the Stop-time auditor in `skill_invocation_audit.py`. |
| **The regression test** | `test_data_eng_command_gated_and_satisfied_by_dataeng_skill` (red-first, in the existing `tests/test_pretool_skill_gate.py`) — blocked while unsatisfied, satisfied by engaging the `data-eng-pipeline` skill. |
| **Counts + tests** | PATCH — no new skill / agent / command / hook / Layer-3 tool; inventory unchanged (53 / 39 / 25 / 7 / 22). Suite **7019 → 7020 passing + 6 skipped, 0 failed** (239 top-level test files, both encodings; +1 test in an existing file). |

HONEST BOUNDARY: the run-continuity constants (`hooks/run_continuity.py`'s `RUN_DRIVING_SKILLS` / `ENGAGEMENT_SKILLS`) still omit `data-eng-pipeline` — a `/architect-team:data-eng` run is now real-time gated but NOT yet tracked by the `active-run.json` marker / continuation guard / sticky arm. A known follow-up, documented in `docs/INTEGRATION_MAP.md`'s three-constants paragraph, not hidden. Every earlier release remains in full at [`docs/RELEASE_HISTORY.md`](docs/RELEASE_HISTORY.md).
...
```
   ...
   ◆       v3.55.2 ─ data-eng-realtime-gate-fix — /architect-team:data-eng is
           now real-time gated by the PreToolUse skill-gate exactly like the
           other five pipeline commands (data-eng-pipeline joins the gate's
           _PIPELINE_SKILLS); the run-continuity constants still omit the
           lane — a documented follow-up, not hidden
