...
![tests](https://img.shields.io/badge/tests-5710%20passing-3FB950?style=flat-square)
...
### v3.42.0 — credit-exhaustion failover to Claude sign-in (an upstream that is reachable but cannot serve)

When the gateway's upstream ran out of credits, every Claude Code session pointed at it broke and stayed broken. There was **no credit handling anywhere in the gateway surface** — no `402`, no `insufficient_credit`, no `429` classification. A credit-dead gateway keeps binding its port, keeps passing the v3.41.1 TCP liveness probe, and keeps reporting `enabled`, so every surface read healthy while every session failed. The class: **an upstream that is REACHABLE but cannot SERVE was indistinguishable from a healthy one on every signal CT6 collected.** v3.41.1 fixed the client-side wire being silently cut; v3.42.0 fixes the upstream silently being unable to serve.

| Capability | What it is |
|---|---|
| **Only hard-credit errors fail over** | `classify_upstream_error(status, body)` is a PURE function returning `credit-exhausted` / `rate-limited` / `transient` / `other`. HTTP `402` and the hard-credit vocabulary (`insufficient_credit`, `insufficient credits`, `quota_exceeded`, `credit balance is too low`, `billing`, `payment required` — case-insensitive) are `credit-exhausted`; `429` is `rate-limited`; `>= 500` or `overloaded` is `transient`. **Only `credit-exhausted` may trigger a failover.** The `429` status check runs BEFORE the body scan, and that order is load-bearing: some providers return `429` with quota wording for a per-minute burst limit, and misreading that as exhaustion would flap your auth mode on every busy minute. |
| **Detection where CT6 actually sees the upstream** | One bounded 32-token probe over the existing `/v1/messages` completion seam — negligible when credits exist, free when they do not (the failure IS the signal). Log-scanning was rejected with evidence: the generated launcher writes no log by default, so passive scanning would silently detect nothing on a default install. The prober is injectable, so no test opens a socket. |
| **The failover returns you to Claude sign-in, and it sticks** | `failover_to_login()` strips the settings.json env block via the existing merge-preserving `remove_claude_env()` (unrelated keys survive), reverts the role split to uniform fable via the existing lever (login auth cannot serve the secondary alias), and records `activated: false` plus a `failover` record. **The state flip is mandatory, not cosmetic:** a credit-dead gateway still passes the liveness probe, so a failover that only stripped the wire would be silently undone by the v3.41.1 SessionStart heal on your very next session. No third auth mode is invented — the result is the existing subscription/sign-in posture. `enabled` and your stored provider keys are left intact. |
| **Surfaced, reachable, automatic** | `status` gains a `credit-failover` row (when it fired, why, and the remediation) plus a first-class `--json` field; a machine that never failed over keeps byte-identical output. The `failover` subcommand supports detect-then-act, `--check` (probe + report, mutate nothing), and `--force`. `maybe_failover_to_login()` fires automatically at SessionStart, ordered BEFORE the activation heal so a failover that fires is not re-healed in the same run, and fails open on every path. |
| **Coming back** | `install --activate` — it restores the env block, re-applies the split, and clears the failover record. Automatic return is deliberately NOT built: it would mean polling a paid upstream on a timer. |
| **Counts + tests** | +21 tests in `tests/test_credit_failover.py` — one per the 16 coverage-map acceptance criteria, two classifier-negative cases (rate-limited and transient must NOT fail over), the two dev-checkout containment regressions, and the hook-path end-to-end split-revert test. Both crux invariants were **mutation-tested**, not merely asserted: deleting the `429` early return failed 3 tests; flipping the `activated` clear failed 2. Suite 5689 → **5710 passing + 4 skipped** (203 test files), green under both Windows cp1252 AND `PYTHONUTF8=1`, with the v3.41.1 real-state tripwire silent. Counts UNCHANGED (48 / 39 / 23 / 7 / 20); stdlib-only holds. |
...
```
   ...
   ◆       v3.42.0 ─ credit-failover-to-login — an upstream that is REACHABLE but cannot SERVE is no longer invisible: a pure classifier (402/insufficient_credit/quota_exceeded => credit-exhausted; 429 => rate-limited; 5xx/overloaded => transient) with the 429 check ordered BEFORE the body scan so a burst rate-limit can never be misread as exhaustion and flap the auth mode, probe-based detection at the seam CT6 actually observes (the launcher writes no log, so log-scanning was rejected with evidence), and failover_to_login which strips the env block merge-preservingly, reverts the split to uniform fable, and CLEARS recorded activation so v3.41.1's SessionStart heal cannot silently undo it; surfaced on status, reachable as `failover [--check|--force]`, automatic at SessionStart, return via install --activate; suite 5710 passing + 4 skipped (current)
