...
![version](https://img.shields.io/badge/version-0.10.0-2563EB?style=flat-square)
![license](https://img.shields.io/badge/license-MIT-3FB950?style=flat-square)
![tests](https://img.shields.io/badge/tests-1374%20passing-3FB950?style=flat-square)
...
|---|---|
| ▸ **Mini pipeline — rapid feature changes with single-architect drive + auto-merge to main on green QA (v0.10.0)** | A faster sibling to the full `architect-team-pipeline` for the everyday case: a `≤5`-acceptance-criteria change against a familiar codebase where the maps are fresh and the heavyweight reconciliation / master-review / six-team Phase 5 gates are overkill. v0.10.0 ships the new **`mini-architect-team-pipeline`** skill — 9 phases **M0 → M8** — plus a new **`/architect-team:mini`** command and a follow-up **`/architect-team:mini-review-sweep`** command that replays the heavyweight gates against a batch of recent mini-runs when the user wants them. Phase flow: **M0 Intake** (the same two input forms as the main pipeline — folder or plain-language prose); **M1 Maps freshness check** (refresh only when stale); **M2 Architect draft** (one `system-architect` produces proposal/spec/tasks/coverage in a single pass — no 100%-coverage validation loop); **M3 Self-confirm** (the architect re-reads its own draft against the prompt + the maps and answers four binary questions; one `no` loops back to M2, capped at 3 iterations); **M4 Parallel dev** (the existing `frontend` + `backend` agents work non-overlapping slices and **cross-review each other's evidence files** — dev↔dev review, no separate task-reviewer team); **M5 mini-qa** (the new **`mini-qa`** agent — opus — runs unit tests, integration tests against the live dev environment, and **≤ 3 Playwright user-flow tests** that exercise the smallest sufficient happy path for the change; writes a per-cycle verdict at `.architect-team/mini-qa/verdict-<ts>.json`); **M6 Verdict gate** (one of `green` / `red-fix` / `red-rework` — `green` proceeds, `red-fix` loops M4-M5, `red-rework` escalates to the user with the captured evidence); **M7 Auto-merge to main** (commit with the structured **`Mini-Run: <slug>`** trailer the audit hook recognizes, then merge to `main` directly — the discipline that makes mini distinct from the main pipeline's PR-style flow); **M8 Re-eval loop** (after merge, the architect re-evaluates the full prompt against the merged state, cap 3; escalates if the third re-eval still surfaces gaps). Every mini commit carries a `Mini-Run: <slug>` trailer so the existing `pipeline-completion-audit.py` Stop hook recognizes mini runs and the new **`/architect-team:mini-review-sweep`** command can replay the heavyweight gates (visual-fidelity reconciliation, editability completeness, master-review audit, doc-currency audit) against the last `--limit N` mini-runs since `--since <ref>`. The cross-review discipline at M4 is structurally backed by **`test_dev_cross_review_accepted.py`** — confirms the existing `task-reviewer` schema accepts dev-agent reviewers when the reviewer is the OTHER dev agent (frontend reviews backend, backend reviews frontend). Use mini when: ≤5 ACs, familiar codebase, maps fresh, comfort with auto-merge. Use full `/architect-team` when: scope larger, maps stale, design contract changes, OpenSpec proposal review desired, or PR-flow required. Both pipelines run from the same intake-and-mapping foundation and write to the same MemPalace wing — switching is free. |
...
```
   ...
   ◆       v0.10.0 ─ mini pipeline — rapid feature changes (≤5 ACs, familiar codebase) with single-architect drive + auto-merge to main on green QA (current)
