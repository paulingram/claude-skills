...
| **Counts + tests** | +21 tests in `tests/test_credit_failover.py` — one per coverage-map acceptance criterion plus two dev-checkout containment regressions. Both crux invariants were **mutation-tested**, not merely asserted: deleting the `429` early return failed 3 tests; flipping the `activated` clear failed 2. Suite 5689 → **5710 passing + 4 skipped** (203 test files), green under both Windows cp1252 AND `PYTHONUTF8=1`, with the v3.41.1 real-state tripwire silent. Counts UNCHANGED (48 / 39 / 23 / 7 / 20); stdlib-only holds. |
