...
![tests](https://img.shields.io/badge/tests-5707%20passing-3FB950?style=flat-square)
...
| **Counts + tests** | +18 tests in `tests/test_credit_failover.py`, one per coverage-map acceptance criterion. Both crux invariants were **mutation-tested**, not merely asserted: deleting the `429` early return failed 3 tests; flipping the `activated` clear failed 2. Suite 5689 → **5707 passing + 4 skipped** (203 test files), green under both Windows cp1252 AND `PYTHONUTF8=1`, with the v3.41.1 real-state tripwire silent. Counts UNCHANGED (48 / 39 / 23 / 7 / 20); stdlib-only holds. |
...
```
   ...
   ◆       v3.42.0 ─ credit-failover-to-login — an upstream that is REACHABLE but cannot SERVE is no longer invisible: a pure classifier (402/insufficient_credit/quota_exceeded => credit-exhausted; 429 => rate-limited; 5xx/overloaded => transient) with the 429 check ordered BEFORE the body scan so a burst rate-limit can never be misread as exhaustion and flap the auth mode, probe-based detection at the seam CT6 actually observes (the launcher writes no log, so log-scanning was rejected with evidence), and failover_to_login which strips the env block merge-preservingly, reverts the split to uniform fable, and CLEARS recorded activation so v3.41.1's SessionStart heal cannot silently undo it; surfaced on status, reachable as `failover [--check|--force]`, automatic at SessionStart, return via install --activate; suite 5707 passing + 4 skipped (current)
