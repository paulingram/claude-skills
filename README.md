...
![version](https://img.shields.io/badge/version-3.55.1-2563EB?style=flat-square)
...
### v3.55.1 — compact-readme-status-timeline: the README carries only the current release, the full history lives in docs

The README still embedded a one-line release changelog — the `STATUS` section listed every version from `v0.1.0` to the current one (~135 lines that grew every release). v3.47.1 moved the accumulated *narrative* to `docs/RELEASE_HISTORY.md` but kept this timeline; v3.55.1 finishes the job. The embedded timeline moves into `docs/RELEASE_HISTORY.md` as a scannable **Release timeline at a glance** index, and the README's `STATUS` section now names only the current release plus a pointer to the full history — so the README shows only the most recent release's changes and directs you to the docs folder for everything else. Docs-only; the run also refreshed the MemPalace and swept the docs current. **README 1193 → 1066 lines.**

| What changed | Detail |
|---|---|
| **README compacted** | The `STATUS` section's `v0.1.0 → v3.55.0` one-line timeline is gone from the README; the section keeps the house `▰▰▰` frame but names only the current release + a house-style pointer to `docs/RELEASE_HISTORY.md`. The `NEW IN` spotlight (this section) and the top `RELEASE HISTORY` pointer block are unchanged conventions from v3.47.1. |
| **Timeline relocated, nothing lost** | The one-line timeline lands in `docs/RELEASE_HISTORY.md` as a newest-first **Release timeline at a glance** index above the detailed per-release narrative — every release identity preserved (the `test_release_history.py` completeness floor is unchanged). |
| **Currency + memory** | A comprehensive documentation-currency sweep corrected the `docs/CODEBASE_MAP.md` "Currency note" that still pinned the v3.49.0 inventory (→ v3.55.1: 53 skills / 39 agents / 25 commands / 22 Layer-3 tools / 7019 tests); the MemPalace was refreshed with the latest changes. |
| **Counts + tests** | Docs-only PATCH — no new skill / agent / command / hook / Layer-3 tool; inventory unchanged (53 / 39 / 25 / 7 / 22). Suite **7019 passing + 6 skipped, 0 failed** (both encodings), unchanged. |

HONEST BOUNDARY: this relocates documentation and refreshes currency — it changes no runtime behavior. The previous release's headline (v3.55.0 frontend-e2e-loop-exit-gate) and every earlier release remain in full at [`docs/RELEASE_HISTORY.md`](docs/RELEASE_HISTORY.md).
...
```
   ...
   ◆       v3.55.1 ─ compact-readme-status-timeline — the README's embedded
           release timeline moves to docs/RELEASE_HISTORY.md; the README now
           carries only the current release spotlight + a pointer to the full
           history, so the updates and changes live in their own file in docs
